...
int main(int argc,char **argv) {
  ...
  TCLAP::SwitchArg simple_rewriteArg("", "simple_rewrite",
                                     "Uses simple rewriting", false);
  cmd.add(simple_rewriteArg);

  TCLAP::ValueArg<int> branchOnaux(
      "", "branch_on_aux",
      "Branching on auxiliary variables whose domain size is larger than "
      "value; if it is given -1, the solver will branch on all variables",
      false, 0, "int");
  cmd.add(branchOnaux);

  TCLAP::ValueArg<int> countArg("", "count", "count solutions (up to value)",
                                false, 1, "int");
  cmd.add(countArg);

  TCLAP::SwitchArg minimum_outputArg(
      "", "minimum_output",
      "use this option to avoid the output used for the competition", false);
  cmd.add(minimum_outputArg);

  TCLAP::ValueArg<int> recommended_Arg(
      "", "recommended", "use our recommended configurations for search", false,
      1, "int");
  ...
  XCSP3MistralCallbacks cb(
      solver); // my interface between the parser and the solver
  ...
  if (!recommended_Arg.getValue()) {
    ...
    if (solver.objective && solver.objective->is_optimization()) {
      ...
      heuristic = new LastConflict<
          GenericDVO<MinDomainOverWeight, 1, ConflictCountManager>,
          Guided<MinValue>, Guided<MinValue>, 1>(&solver);
    } else {
      // restart = new Luby();
      restart = new Geometric();
      heuristic = new LastConflict<
  ...
  if (countArg.getValue() != 1) {
    ...
    int num_solutions = 0;
    // Outcome res = solver.depth_first_search(cb.variables, heuristic, new
    // NoRestart());
    ...
    while ((countArg.getValue() < 1 || countArg.getValue() > num_solutions) &&
      ...
      if (num_solutions <= 1000 ||
          (num_solutions <= 10000 && (num_solutions % 100 == 0)) ||
          (num_solutions % 1000 == 0)) {
        ...
        ofstream solfile("sols/sol" + int2str(num_solutions) + ".txt",
                         ofstream::out);
