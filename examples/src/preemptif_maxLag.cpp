...
enum model { basic = 0, withouthall = 1};
...
void build_model(Instance &jsp, Solver &solver, VarArray &start_time,
  ...
  for (auto j{0}; j < jsp.nJobs(); ++j) {
    for (auto i{1}; i < jsp.nTasksInJob(j); ++i) {
      solver.add(start_time[jsp.getJobTask(j, i)] >=
                end_time[jsp.getJobTask(j, i - 1)]+jsp.getMinLag(j,i-1));
      if(jsp.getMaxLag(i,j-1) >= 0) 
      {
        solver.add(start_time[jsp.getJobTask(j, i)] <=
                end_time[jsp.getJobTask(j, i - 1)]+ jsp.getMaxLag(j,i-1));
      }      
    }
  }
  ...
  for (auto k{0}; k < jsp.nMachines(); ++k) {
    ...
    for (auto i{0}; i < jsp.nTasksInMachine(k); ++i) {
      ...
      if (jsp.getDuration(o) == 1)
      {
        solver.add(start_time[o] + jsp.getDuration(o) == end_time[o]);
        st.add(start_time[o]);
        et.add(end_time[o]);
      ...
      else 
      {
        ...
        solver.add(start_time[o] + 1 == ets);
        st.add(start_time[o]);
        et.add(ets);
        p.push_back(1);

        solver.add(ste + 1 == end_time[o]);
        st.add(ste);
        et.add(end_time[o]);
        p.push_back(1);

        if (jsp.getDuration(o) >= 2)
        {
          Variable etr(0,ub);
          Variable str(0,ub);
          solver.add(str == ets);
          ...
          st.add(str);
          et.add(etr);
          p.push_back(jsp.getDuration(o)-2);
        }
      }
    }
    ...
    if (model_choice == model::basic)
      solver.add(PreemptiveNoOverlap(st, et, p, makespan,true));
    else
      solver.add(PreemptiveNoOverlap(st, et, p, makespan,false));
