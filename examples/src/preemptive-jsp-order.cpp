...
public:
  ...
  bool operator<=(const TaskInfo &lct) const {
    return due_date <= lct.due_date;
  }
...
void build_model(Instance &jsp, Solver &solver, VarArray &start_time,
                 ...
                 VarArray &time_vars, const int ub, const int model_choice) {
  ...
  if (model_choice == model::compact) {
    ...
    time_vars.initialise(jsp.nTasks() - jsp.nJobs(), 0, ub);
    ...
    for (auto i{0}; i < jsp.nTasks(); ++i) {
      ...
      if (r == 0)
        ...
        start_time.add(time_vars[i - j - 1]);

      if (r == jsp.nMachines() - 1)
        end_time.add(makespan);
      else
        end_time.add(time_vars[i - j]);
    }
...
void model_order(Instance &jsp, Solver &solver, VarArray &start_time,
                 VarArray &end_time, Variable &makespan,
  ...
  for (auto k{0}; k < jsp.nMachines(); ++k) {
    for (auto i{0}; i < jsp.nTasksInMachine(k); ++i) {
      for (auto j{i + 1}; j < jsp.nTasksInMachine(k); ++j) {
        if (end_time[jsp.getMachineTask(k, i)].id() !=
            end_time[jsp.getMachineTask(k, j)].id())
          ordering.add(ReifiedDisjunctive(end_time[jsp.getMachineTask(k, i)],
                                          end_time[jsp.getMachineTask(k, j)], 0,
                                          1));
      }
    }
  }
...
RestartPolicy *restart_factory(std::string rpolicy, const int b,
                               const double f) {
  RestartPolicy *pol;
  if (rpolicy == "luby")
    pol = new Luby(b);
  else if (rpolicy == "geom")
    pol = new Geometric(b, f);
  else
    pol = new NoRestart();
  return pol;
}
...
BranchingHeuristic *heuristic_factory(Solver &solver,
  ...
  if (branching_choice == "minvalue")
    ...
    std::cout << " c Warning: heuristic " << branching_choice
              << " is not handled, using halfsplit instead\n";
    heuristic = new LastConflict<
        GenericDVO<MinDomainOverWeight, 1, ConflictCountManager>, HalfSplit,
        HalfSplit, 1>(&solver);
  }
...
int main(int argc, char **argv) {
  ...
  TCLAP::SwitchArg order_branching(
      "", "order", "Branches on the ordering of end times", false);
  ...
  build_model(jsp, solver, start_time, end_time, origin, makespan, time_vars,
  ...
  if (order_branching.getValue()) {
    ...
    search_vars = time_vars;
  }

  for (int i = 0; i < jsp.nJobs(); ++i) {
    solver.monitor_list << end_time[jsp.getJobTask(i, 0)];
    for (int j = 1; j < jsp.nTasksInJob(i); ++j)
      solver.monitor_list << ", " << end_time[jsp.getJobTask(i, j)];
    solver.monitor_list << "\n";
  }
  ...
  RestartPolicy *restart =
      restart_factory(cmd.get_restart_policy(), solver.parameters.restart_base,
                      solver.parameters.restart_factor);

  BranchingHeuristic *heuristic;
  if (order_branching.getValue()) {
    heuristic = new LastConflict<
        GenericDVO<MinDomainOverWeight, 1, ConflictCountManager>, RandomValue,
        RandomValue, 1>(&solver);
  } else {
    heuristic = heuristic_factory(solver, cmd.get_value_ordering());
  }
