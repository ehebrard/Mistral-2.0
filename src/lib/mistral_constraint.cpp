...
Mistral::PropagationOutcome Mistral::ConstraintReifiedDisjunctive::propagate() {
  ...
  std::cout << std::endl << "propagate [" << id << "] " << this << " (init)" << std::endl;
...
Mistral::PropagationOutcome Mistral::ConstraintReifiedDisjunctive::propagate(const int changed_idx, const Event evt) {
  ...
  std::cout << std::endl << "propagate [" << id << "] " << this << " " << event2str(evt) 
	    << " on " << scope[changed_idx] << " in " << scope[changed_idx].get_domain() << " (" << changed_idx << ")"<< std::endl;
  for(unsigned int i=0; i<3; ++i) {
    std::cout << scope[i] << " in " << scope[i].get_domain() << " ";
  }
  std::cout << std::endl;
  ...
  if(changed_idx == 2) {
...
#ifdef _DEBUG_RDISJUNCTIVE
		if(_DEBUG_RDISJUNCTIVE) {
    std::cout << "  -> it was already a precedence " << (*state==2 ? "(<=)" : "(>=)") << std::endl;
	}
#endif
...
#ifdef _DEBUG_RDISJUNCTIVE
		if(_DEBUG_RDISJUNCTIVE) {
    std::cout << (IS_OK(wiped) ? "  => inconsistency" : "  => ok") << std::endl;
	}
#endif	
    ...
    std::cout << "  -> could it be a precedence? (" 
	      << scope[0].get_min() << " + " << processing_time[0] 
	      << " > " <<  scope[1].get_max() << " || "
...
double Mistral::ConstraintIncrementalWeightedBoolSumInterval::weight_conflict(double unit, Vector<double>& weights)
{
...
#ifdef _PWBS_WC_ALT
double Mistral::PredicateWeightedBoolSum::weight_conflict(double unit, Vector<double>& weights)
{
  int i, idx, arity=weight.size;
...
#ifdef _ELT_WC
double Mistral::PredicateElement::weight_conflict(double unit, Vector<double>& weights)
{
...
#ifdef _CNE_WC
double Mistral::ConstraintCliqueNotEqual::weight_conflict(double unit, Vector<double>& weights)  {
