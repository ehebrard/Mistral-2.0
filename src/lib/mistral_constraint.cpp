...
#ifdef _DEBUG_RELAX
  if (_DEBUG_RELAX) {
    std::cout << "[" << std::setw(4) << id << "]: trigger on: " << x << " in "
              << x.get_domain() << ": " << x.is_ground() << " " << t << "/"
              << _NEVER_ << std::endl;
...
#ifdef _DEBUG_RELAX
    if (_DEBUG_RELAX) {
      std::cout << "  -> link to cons graph " << x.id() << "/"
                << get_solver()->constraint_graph.size << std::endl;
    }
#endif
...
bool Mistral::ConstraintImplementation::is_triggered_on(const int i, const int t) {
  ...
  // 	    << (on[i] >= &(get_solver()->constraint_graph[_scope[i].id()].on[t])
  // ? " yes" : " no") << std::endl;
...
void Mistral::GlobalConstraint::initialise_vars(Solver *s) {
  for (unsigned int i = 0; i < scope.size; ++i) {
    ...
    // std::cout << "INITIALISE " << scope[i] << " => " <<
    // get_solver()->variables[scope[i].id()] << std::endl;
  }
}
...
void Mistral::ConstraintImplementation::initial_post(Solver *s) {
  ...
  // Also, if there is only one active variable and the constraint enforces
  // nfc1, we do not post it at all.
  ...
  // Now we post the constraint on the active variables (provided that there are
  // at least 2)
  if (!enforce_nfc1 || nb_actives > 1) {
    ...
    for (unsigned int i = 0; i < on.size; ++i) {
      // if(!(_scope[i].is_ground())) {
      // std::cout << _scope[i] << " "  << (on[i] != NULL) << " " <<
      // _scope[i].get_min() << ".." << _scope[i].get_max() << std::endl;
    ...
    //
    // std::cout << "TRY TO POST A CONSTRAINT WITH ONLY ONE ACTIVE VARIABLE, WE
    // NEED TO PROPAGATE IT AT LEAST ONCE!" << std::endl;
    //
  ...
  if (_DEBUG_RELAX) {
    std::cout << std::endl;
    // std::cout << ((int*)(on.stack_)) << " " << ((int*)(on[14])) << " " <<
    // on[14] << std::endl;
...
#ifdef _DEBUG_GENPROPAG
    std::cout << " - (bc) revise " << scope[revise_idx] << " in "
              << scope[revise_idx].get_domain() << " w.r.t. "
              << scope[changed_idx] << std::endl;
#endif
...
#ifdef _DEBUG_GENPROPAG
      std::cout << "  *find a support for " << scope[revise_idx] << " = "
                << vali << std::endl;
#endif

      if (!(scope[changed_idx].contain(
              support[revise_idx][vali][changed_idx])) &&
...
#ifdef _DEBUG_GENPROPAG
  std::cout << " - revise " << scope[revise_idx] << " in "
            << scope[revise_idx].get_domain() << " w.r.t. "
            << scope[changed_idx] << std::endl;
#endif
...
#ifdef _DEBUG_GENPROPAG
    std::cout << "  *find a support for " << scope[revise_idx] << " = " << vali
              << std::endl;
#endif
...
bool Mistral::BinaryConstraint::find_support(const int revise_idx, const int vli) 
{
  int changed_idx = 1 - revise_idx,
      vali = -INFTY; //, *solution = support[revise_idx][vli];
  ...
  while (vali < solution[changed_idx] && check(solution)) {
    ...
    // if(vali<solution[changed_idx]) std::cout << "   " << solution[0] << " "
    // << solution[1] << std::endl;
  }
...
bool Mistral::BinaryConstraint::find_bound_support(const int revise_idx, const int vli) 
{
  ...
  for (solution[changed_idx] = scope[changed_idx].get_min();
       // print_sol(solution, scope[revise_idx], vli, scope[changed_idx], id,
       // this) &&
...
Mistral::PropagationOutcome Mistral::TernaryConstraint::propagate(const int changed_idx, const Event evt) {
...
#ifdef _DEBUG_GENPROPAG
  std::cout << " propagate " << event2str(evt) << " on " << scope[changed_idx]
            << std::endl;
#endif
  ...
  for (int i = 1; i < 3; ++i) {
    ...
    std::cout << " -revise " << scope[revise_idx] << " in "
              << scope[revise_idx].get_domain() << " w.r.t. "
              << scope[changed_idx] << " & " << scope[other_idx] << std::endl;
#endif
    ...
    do {
      ...
      if ((x == NOVAL || y == NOVAL || !scope[changed_idx].contain(x) ||
...
#ifdef _DEBUG_GENPROPAG
  std::cout << " bound propagate " << event2str(evt) << " on "
            << scope[changed_idx] << std::endl;
#endif
...
bool Mistral::TernaryConstraint::find_support(const int revise_idx, const int vli) 
{
  ...
  int valx = scope[x_idx].get_min(),
      valy; //, *solution = support[revise_idx][vli];
...
bool Mistral::TernaryConstraint::find_bound_support(const int revise_idx, const int vli) 
{
  ...
  for (solution[x_idx] = scope[x_idx].get_min();
       no_support && solution[x_idx] <= max_x_val; ++solution[x_idx]) {
    for (solution[y_idx] = min_y_val;
         no_support && solution[y_idx] <= max_y_val; ++solution[y_idx]) {
...
Mistral::GlobalConstraint::GlobalConstraint(std::vector< Variable > scp) {
  for (std::vector<Variable>::iterator vi = scp.begin(); vi != scp.end(); ++vi)
    scope.add(*vi);
}
...
#ifdef _DEBUG_GENPROPAG
        std::cout << "find a support for " << scope[i] << " = " << vali
                  << std::endl;
#endif
  ...
  while (!changes.empty()) {
    ...
    for (i = 0; IS_OK(wiped) && i < scope.size; ++i) {
      if (i != j) {
...
#ifdef _DEBUG_GENPROPAG
        if (_DEBUG_GENPROPAG) {
          std::cout << "revise " << scope[i] << " in " << scope[i].get_domain()
                    << std::endl;
        }
#endif
        ...
        // 	  std::cout << "find a support for " << scope[i] << " = " <<
        // vali << std::endl;
        // 	  // }
        // 	  }
        // #endif
  ...
  for (i = 0; IS_OK(wiped) && i < scope.size; ++i) {
    if ((int)i != changed_idx) {
      ...
      while (!supported && IS_OK(wiped) && vali <= valmax) {
        ...
        // std::cout << "find support for " << scope[i] << "=" << vali <<
        // std::endl;
...
#ifdef _DEBUG_GENPROPAG
    std::cout << " propagate " << event2str(event_type[j]) << " on " << scope[j]
              << std::endl;
#endif
...
#ifdef _DEBUG_GENPROPAG
      std::cout << " - (bc) revise " << scope[i] << " in "
                << scope[i].get_domain() << " w.r.t. " << scope[j] << std::endl;
#endif
...
bool Mistral::GlobalConstraint::first_support(const int vri, const int vli) 
{
  ...
  if (supports && supports[vri][vli][0] != NOVAL) {
    ...
    while (j--)
      if (vri != j)
        if (!scope[j].contain(supports[vri][vli][j]))
          break;
    if (j < 0)
      return true;
  }
...
#ifdef _DEBUG_BUILD
  std::cout << "[" << std::setw(4) << id << "]: initialise (global)"
            << std::endl;
#endif
...
Mistral::PropagationOutcome Mistral::ConstraintNotEqual::rewrite() {
  ...
  if (scope[0].is_ground() || scope[1].is_ground() ||
    ...
    if (scope[0].get_min() == 0 && scope[1].get_min() == 0 &&
        scope[0].get_max() == 1 && scope[1].get_max() == 1 &&
        scope[0].is_expression() && scope[1].is_expression()) {
...
Mistral::PropagationOutcome Mistral::ConstraintNotEqual::propagate() {
  ...
  // std::cout.flush();
  // std::cout << scope[0].get_domain() << " " << scope[1].get_domain() <<
  // std::endl;

  PropagationOutcome wiped_idx = CONSISTENT;
  if (!active) {
    if (scope[0].get_first() == scope[1].get_first())
      wiped_idx = FAILURE(0);
  } else {
    ...
    // 		<< scope[active-1] << " in " << scope[active-1].get_domain() <<
    // std::endl;
    // }
    ...
    if (active != 3 &&
        (scope[active - 1].remove(scope[2 - active].get_first()) == FAIL_EVENT))
      wiped_idx = FAILURE(active - 1);
  }
  return wiped_idx;
}

Mistral::PropagationOutcome
Mistral::ConstraintNotEqual::propagate(const int changed_idx, const Event evt) {

  // std::cout << "\n propagate changes on " << scope[changed_idx] << " to " <<
  // scope[1-changed_idx]
  ...
  PropagationOutcome wiped_idx =
      (scope[var].remove(scope[changed_idx].get_first()) == FAIL_EVENT
           ? FAILURE(var)
           : CONSISTENT);
...
#ifdef _DEBUG_REWRITE
      std::cout << "    relax constraints on " << scope[j] /*.get_var()*/
                << " and post them on " << scope[1 - j]    /*.get_var()*/
                << std::endl;
#endif
...
Mistral::PropagationOutcome Mistral::ConstraintEqual::propagate() {
  ...
  PropagationOutcome wiped = CONSISTENT;
  if (active) {
    if (scope[1].set_domain(scope[0]) == FAIL_EVENT)
      wiped = FAILURE(1);
    if (IS_OK(wiped) && scope[0].set_domain(scope[1]) == FAIL_EVENT)
      wiped = FAILURE(0);
...
Mistral::PropagationOutcome Mistral::ConstraintEqual::propagate(const int changed_idx, const Event evt) {
  return (scope[1 - changed_idx].set_domain(scope[changed_idx]) == FAIL_EVENT
              ? FAILURE(1 - changed_idx)
              : CONSISTENT);
}
...
Mistral::PropagationOutcome Mistral::PredicateUpperBound::propagate() {
  ...
  if (scope[1].is_ground()) {
    ...
    if (scope[0].get_min() > bound) {
      if (FAILED(scope[1].set_domain(0)))
        return FAILURE(1);
    } else if (scope[0].get_max() <= bound) {
      if (FAILED(scope[1].remove(0)))
        return FAILURE(1);
    }
  }
...
Mistral::PropagationOutcome Mistral::PredicateLess::propagate() {
  ...
  if (scope[2].is_ground()) {
    ...
    if (scope[0].get_min() + offset > scope[1].get_max()) {
      if (FAILED(scope[2].set_domain(0)))
        return FAILURE(2);
    } else if (scope[0].get_max() + offset <= scope[1].get_min()) {
      if (FAILED(scope[2].remove(0)))
        return FAILURE(2);
    }
  }
...
Mistral::PropagationOutcome Mistral::PredicateLess::propagate(const int changed_idx, const Event evt) {
  ...
  // std::cout << "HERE: "  << event2str(evt) << " on " << scope[changed_idx] <<
  // " in " << scope[changed_idx].get_domain() << std::endl;
  ...
  if (changed_idx == 2) {
    ...
    if (scope[2].is_ground()) {
      ...
      if (scope[2].get_min()) {
        ...
        if (changed_idx) {
          if (UB_CHANGED(evt) &&
              FAILED(scope[0].set_max(scope[1].get_max() - offset)))
            wiped = FAILURE(0);
        } else {
          if (LB_CHANGED(evt) &&
              FAILED(scope[1].set_min(scope[0].get_min() + offset)))
            wiped = FAILURE(1);
        }
      } else {

        // std::cout << "APPLY " << scope[0] << " in " << scope[0].get_domain()
        // << " + " << offset
        // 	  << " > "
        ...
        if (changed_idx) {
          if (LB_CHANGED(evt) &&
              FAILED(scope[0].set_min(scope[1].get_min() - offset + 1)))
            wiped = FAILURE(0);
        } else {
          if (UB_CHANGED(evt) &&
              FAILED(scope[1].set_max(scope[0].get_max() + offset - 1)))
            wiped = FAILURE(1);
        }
      }
    } else {
...
// //   }
...
#ifdef _DEBUG_LESS
  if (_DEBUG_LESS) {
    std::cout << "init propagate " << scope[0] << " in "
              << scope[0].get_domain() << " + " << offset << " < " << scope[1]
              << " in " << scope[1].get_domain() << std::endl;
  }
#endif
  ...
  if (scope[1].set_min(scope[0].get_min() + offset) == FAIL_EVENT)
    wiped = FAILURE(1);
  if (IS_OK(wiped) &&
      scope[0].set_max(scope[1].get_max() - offset) == FAIL_EVENT)
    wiped = FAILURE(0);

#ifdef _DEBUG_LESS
  if (_DEBUG_LESS) {
    std::cout << "init propagated " << scope[0] << " in "
              << scope[0].get_domain() << " + " << offset << " < " << scope[1]
              << " in " << scope[1].get_domain() << std::endl;
  }
#endif
...
Mistral::PropagationOutcome Mistral::ConstraintLess::propagate(const int changed_idx, const Event evt) {
  ...
  if (changed_idx == 0) {
    if (LB_CHANGED(evt) &&
        scope[1].set_min(scope[0].get_min() + offset) == FAIL_EVENT)
      wiped = FAILURE(1);
  } else {
    if (UB_CHANGED(evt) &&
        scope[0].set_max(scope[1].get_max() - offset) == FAIL_EVENT)
...
Mistral::ConstraintDisjunctive::ConstraintDisjunctive(Variable x, Variable y,
                                                      const int p0,
                                                      const int p1)
...
Mistral::ConstraintDisjunctive::ConstraintDisjunctive(Vector<Variable> &scp,
                                                      const int p0,
                                                      const int p1)
...
Mistral::ConstraintDisjunctive::ConstraintDisjunctive(
    std::vector<Variable> &scp, const int p0, const int p1)
    : BinaryConstraint(scp) {
  processing_time[0] = p0;
  processing_time[1] = p1;
  enforce_nfc1 = false;
...
#ifdef _DEBUG_RELAX
  if (_DEBUG_RELAX) {
    std::cout << "[" << std::setw(4) << id << "](" << name() << "): force relax"
              << std::endl;
  }
#endif
...
Mistral::PropagationOutcome
Mistral::ConstraintDisjunctive::propagate(const int changed_idx,
                                          const Event evt) {
  int hold = 3;
  ...
  //   std::cout << "propagate " ;
  //   display(std::cout);
  //   std::cout << " because of " << event2str(evt) << " on " <<
  //   scope[changed_idx] << std::endl;
  // }

  // if(id == 11)
  //   std::cout << scope[1].get_min() << " + " << processing_time[1] << " ?> "
  //   << scope[0].get_max() << std::endl;
...
std::ostream& Mistral::ConstraintDisjunctive::display(std::ostream& os) const {
  os << precedence[0] << " or " << precedence[1];
  return os;
}

Mistral::ConstraintReifiedDisjunctive::ConstraintReifiedDisjunctive(
    Variable x, Variable y, Variable z, const int p0, const int p1)
...
Mistral::ConstraintReifiedDisjunctive::ConstraintReifiedDisjunctive(
    Vector<Variable> &scp, const int p0, const int p1)
...
Mistral::PropagationOutcome Mistral::ConstraintReifiedDisjunctive::propagate() {
  ...
  if (_DEBUG_RDISJUNCTIVE) {
    std::cout << std::endl
              << "propagate [" << id << "] " << this << " (init)" << std::endl;
  ...
  if (scope[0].get_min() + processing_time[0] > scope[1].get_max()) {
    ...
    /// BUGGY WITH DYNAMIC DOMAIN TYPE CHANGE!
    // else if( scope[0].set_min( *min_t1_ptr+processing_time[1] ) ==
    // FAIL_EVENT) wiped = FAILURE(0);
    // else if( scope[1].set_max( *max_t0_ptr-processing_time[1] ) ==
    // FAIL_EVENT) wiped = FAILURE(1);
    else if (scope[0].set_min(scope[1].get_min() + processing_time[1]) ==
             FAIL_EVENT)
      wiped = FAILURE(0);
    else if (scope[1].set_max(scope[0].get_max() - processing_time[1]) ==
             FAIL_EVENT)
      wiped = FAILURE(1);
    ...
    if (scope[2].set_domain(1) == FAIL_EVENT)
      wiped = FAILURE(2);
    // else if( scope[0].set_max( *max_t1_ptr-processing_time[0] ) ==
    // FAIL_EVENT) wiped = FAILURE(0);
    // else if( scope[1].set_min( *min_t0_ptr+processing_time[0] ) ==
    // FAIL_EVENT) wiped = FAILURE(1);
    else if (scope[0].set_max(scope[1].get_max() - processing_time[0]) ==
             FAIL_EVENT)
      wiped = FAILURE(0);
    else if (scope[1].set_min(scope[0].get_min() + processing_time[0]) ==
             FAIL_EVENT)
      wiped = FAILURE(1);
  ...
  if (IS_OK(wiped) && *state != 3) {
    if (*state == 2) {
      //   if( scope[0].set_max( *max_t1_ptr-processing_time[0] ) == FAIL_EVENT)
      //   wiped = FAILURE(0);
      //   else if( scope[1].set_min( *min_t0_ptr+processing_time[0] ) ==
      //   FAIL_EVENT) wiped = FAILURE(1);
      // } else {
      //   if( scope[0].set_min( *min_t1_ptr+processing_time[1] ) == FAIL_EVENT)
      //   wiped = FAILURE(0);
      //   else if( scope[1].set_max( *max_t0_ptr-processing_time[1] ) ==
      //   FAIL_EVENT) wiped = FAILURE(1);
      if (scope[0].set_max(scope[1].get_max() - processing_time[0]) ==
          FAIL_EVENT)
        wiped = FAILURE(0);
      else if (scope[1].set_min(scope[0].get_min() + processing_time[0]) ==
               FAIL_EVENT)
      ...
      if (scope[0].set_min(scope[1].get_min() + processing_time[1]) ==
          FAIL_EVENT)
        wiped = FAILURE(0);
      else if (scope[1].set_max(scope[0].get_max() - processing_time[1]) ==
               FAIL_EVENT)
...
Mistral::PropagationOutcome Mistral::ConstraintReifiedDisjunctive::propagate(const int changed_idx, const Event evt) {
  ...
  if (_DEBUG_RDISJUNCTIVE) {
    std::cout << std::endl
              << "propagate [" << id << "] " << this << " " << event2str(evt)
              << " on " << scope[changed_idx] << " in "
              << scope[changed_idx].get_domain() << " (" << changed_idx << ")"
              << std::endl;
    for (unsigned int i = 0; i < 3; ++i) {
      std::cout << scope[i] << " in " << scope[i].get_domain() << " ";
    }
  ...
  if (changed_idx == 2) {
    ...
    // it becomes a precedence, we must enforce both rules no matter what and
    // there cannot be further changes.
...
#ifdef _DEBUG_RDISJUNCTIVE
    if (_DEBUG_RDISJUNCTIVE) {
      std::cout << "  -> it was already a precedence "
                << (*state == 2 ? "(<=)" : "(>=)") << std::endl;
    }
#endif
    ...
    if (*state == 2) {
      if ( // events.contain(1)
          changed_idx == 1 && UB_CHANGED(evt)) {
        if (scope[0].set_max(scope[1].get_max() - processing_time[0]) ==
            FAIL_EVENT)
          wiped = FAILURE(0);
      }
      if (IS_OK(wiped) && // events.contain(0)
          changed_idx == 0 && LB_CHANGED(evt)) {
        if (scope[1].set_min(scope[0].get_min() + processing_time[0]) ==
            FAIL_EVENT)
          wiped = FAILURE(1);
      }
    } else {
      if ( // events.contain(0)
          changed_idx == 0 && UB_CHANGED(evt)) {
        if (scope[1].set_max(scope[0].get_max() - processing_time[1]) ==
            FAIL_EVENT)
          wiped = FAILURE(1);
      }
      if (IS_OK(wiped) && // events.contain(1)
          changed_idx == 1 && LB_CHANGED(evt)) {
        if (scope[0].set_min(scope[1].get_min() + processing_time[1]) ==
            FAIL_EVENT)
          wiped = FAILURE(0);
      }
...
#ifdef _DEBUG_RDISJUNCTIVE
    if (_DEBUG_RDISJUNCTIVE) {
      std::cout << (IS_OK(wiped) ? "  => inconsistency" : "  => ok")
                << std::endl;
    }
#endif
...
#ifdef _DEBUG_RDISJUNCTIVE
  if (_DEBUG_RDISJUNCTIVE) {
    std::cout << "  -> could it be a precedence? (" << scope[0].get_min()
              << " + " << processing_time[0] << " > " << scope[1].get_max()
              << " || " << scope[1].get_min() << " + " << processing_time[1]
              << " > " << scope[0].get_max() << ")" << std::endl;
  }
#endif
...
std::ostream& Mistral::ConstraintReifiedDisjunctive::display(std::ostream& os) const {
  if (scope[2].is_ground()) {
    if (!scope[2].get_max()) {
      os << scope[1] << " + " << processing_time[1]
         << " <= " << scope[0] /*.get_var()*/;
    } else {
      os << scope[0] << " + " << processing_time[0]
         << " <= " << scope[1] /*.get_var()*/;
    }
  } else {
    os << scope[0] /*.get_var()*/ << " + " << processing_time[0]
       << " <= " << scope[1] /*.get_var()*/ << " or " << scope[1] /*.get_var()*/
       << " + " << processing_time[1] << " <= " << scope[0] /*.get_var()*/;
  }
  os << "*";
  return os;
}
...
Mistral::PropagationOutcome Mistral::PredicateEqual::rewrite() {
  ...
  // if(id==49) {
  //   std::cout << is_posted << " rewrite (" << scope[0] << " in " <<
  //   scope[0].get_domain()
  // 	      << " == " << scope[1] << " in " << scope[1].get_domain()
  // 	      << ") <-> " << scope[2] << " in " << scope[2].get_domain() <<
  // std::endl;
  // }
  ...
  if (scope[2].is_ground()) {
    ...
    get_solver()->add(
        Constraint(new PredicateConstantEqual(new_scope, val, spin)));
  }
  //#endif
...
Mistral::PropagationOutcome Mistral::PredicateEqual::propagate() {
  ...
  if (scope[2].is_ground()) {
    if ((spin + scope[2].get_min()) == 1) {
      ...
      if (FAILED(scope[0].set_domain(scope[1])))
        wiped = FAILURE(0);
      else if (FAILED(scope[1].set_domain(scope[0])))
        wiped = FAILURE(1);
    }
  } else {
    if (!(scope[0].intersect(scope[1]))) {
      if (FAILED(scope[2].remove(spin)))
        wiped = FAILURE(2);
    } else {
      if (scope[0].is_ground() && scope[1].is_ground()) {
        if (FAILED(scope[2].set_domain(spin)))
          wiped = FAILURE(2);
      }
    }
  }
...
Mistral::PropagationOutcome Mistral::PredicateEqual::propagate(const int changed_idx, const Event evt) {
  ...
  if (_DEBUG_PEQ) {
    ...
    if (spin)
      std::cout << scope[0] << scope[0].get_domain() << " == " << scope[1]
                << scope[1].get_domain();
    else
      std::cout << scope[0] << scope[0].get_domain() << " =/= " << scope[1]
                << scope[1].get_domain();
    std::cout << ") -> change on " << scope[changed_idx] << " ("
              << event2str(evt) << ")\n";
  }
#endif
  ...
  if (changed_idx == 2) {
    if (LB_CHANGED(evt) == spin) {
      ...
      if (scope[0].is_ground() && FAILED(scope[1].remove(scope[0].get_min())))
        wiped = FAILURE(1);
      else if (scope[1].is_ground() &&
               FAILED(scope[0].remove(scope[1].get_min())))
        wiped = FAILURE(0);
    }
  } else {
    if (scope[2].is_ground()) {
      if ((spin != scope[2].get_min())) {
        if (scope[changed_idx].is_ground() &&
            FAILED(scope[1 - changed_idx].remove(scope[changed_idx].get_min())))
...
Mistral::PropagationOutcome Mistral::PredicateConstantEqual::propagate() {
  ...
  if (_DEBUG_PCEQ) {
    ...
    if (spin)
      std::cout << scope[0] << ":" << scope[0].get_domain() << " == " << value;
    else
      std::cout << scope[0] << ":" << scope[0].get_domain() << " =/= " << value;
  ...
  if (scope[1].is_ground()) {
    if ((spin + scope[1].get_min()) == 1) {
      if (FAILED(scope[0].remove(value)))
        wiped = FAILURE(0);
    } else {
      if (FAILED(scope[0].set_domain(value)))
        wiped = FAILURE(0);
    }
  } else {
    if (!(scope[0].contain(value))) {
      if (FAILED(scope[1].remove(spin)))
        wiped = FAILURE(1);
    } else if (scope[0].is_ground()) {
      if (FAILED(scope[1].set_domain(spin)))
        wiped = FAILURE(1);
    }
  }
...
#ifdef _DEBUG_PCEQ
    if (_DEBUG_PCEQ) {
      std::cout << spin << ", " << value << " in "
                << ":" << scope[0].get_domain() << ": " << (LB_CHANGED(evt))
                << std::endl;
    }
#endif
...
Mistral::PropagationOutcome Mistral::PredicateIntervalMember::propagate() {
  ...
  if (scope[1].is_ground()) {
    if ((spin + scope[1].get_min()) == 1) {
      if (FAILED(scope[0].remove_interval(lower_bound, upper_bound)))
        wiped = FAILURE(0);
    } else {
      // if( FAILED(scope[0].set_domain(lower_bound, upper_bound)) ) wiped =
      // FAILURE(0);
      if (FAILED(scope[0].set_min(lower_bound)))
        wiped = FAILURE(0);
      else if (FAILED(scope[0].set_max(upper_bound)))
        wiped = FAILURE(0);
    }
  } else {

    // std::cout << scope[0].get_domain() << " <> [" << lower_bound << ".." <<
...
std::ostream& Mistral::PredicateIntervalMember::display(std::ostream& os) const {
  os << scope[1] /*.get_var()*/ << " <=> (";
  if (spin)
    os << scope[0] /*.get_var()*/ << " in [" << lower_bound << ".."
       << upper_bound << "]";
  else
    os << scope[0] /*.get_var()*/ << " in ..]" << lower_bound << ","
       << upper_bound << "[..";
...
Mistral::PropagationOutcome Mistral::PredicateSetMember::propagate() {
  ...
  if (scope[1].is_ground()) {
    if ((spin + scope[1].get_min()) == 1) {
      if (FAILED(scope[0].set_domain(non_values)))
        wiped = FAILURE(0);
    } else {
      if (FAILED(scope[0].set_domain(values)))
        wiped = FAILURE(0);
    }
  } else {
...
#ifdef _DEBUG_OFFSET
  if (_DEBUG_OFFSET) {
    std::cout << scope[0].get_domain() << " + " << offset << " = "
              << scope[1].get_domain() << std::endl;
  }
#endif

  if (FAILED(scope[0].set_min(scope[1].get_min() - offset)))
    wiped = FAILURE(0);
  else if (FAILED(scope[0].set_max(scope[1].get_max() - offset)))
    wiped = FAILURE(0);
  else if (FAILED(scope[1].set_min(scope[0].get_min() + offset)))
    wiped = FAILURE(1);
  else if (FAILED(scope[1].set_max(scope[0].get_max() + offset)))
    wiped = FAILURE(1);
...
#ifdef _DEBUG_OFFSET
    if (_DEBUG_OFFSET) {
      std::cout << "react to " << event2str(ecur) << " on " << scope[changed]
                << std::endl;
    }
#endif
...
#ifdef _DEBUG_OFFSET
    if (_DEBUG_OFFSET) {
      std::cout << " ====> " << event2str(enxt) << " on " << scope[changed]
                << std::endl;
    }
#endif
...
Mistral::PropagationOutcome Mistral::PredicateFactor::propagate() {
  ...
  } else if (FAILED(scope[0].set_max(
                 (factor > 0 ? scope[1].get_max() : scope[1].get_min()) /
                 factor))) {
    wiped = FAILURE(0);

  } else if (FAILED(scope[1].set_min(
                 (factor > 0 ? scope[0].get_min() : scope[0].get_max()) *
                 factor))) {
    wiped = FAILURE(1);

  } else if (FAILED(scope[1].set_max(
                 (factor > 0 ? scope[0].get_max() : scope[0].get_min()) *
                 factor))) {
    wiped = FAILURE(1);
  }
...
Mistral::PredicateFactor::propagate(const int changed_idx, const Event evt) {
  ...
  if (_DEBUG_FACTOR) {
    std::cout << std::endl
              << scope[0].get_domain() << " * " << factor << " = "
              << scope[1].get_domain() << " id=" << changed_idx
              << " lvl=" << get_solver()->level << std::endl;
  }
#endif
...
#ifdef _DEBUG_SQ
  if (_DEBUG_SQ) {
    std::cout << "prop " << scope[0].get_domain()
              << "^2 = " << scope[1].get_domain() << std::endl;
  }
#endif

  if (FAILED(scope[1].set_min(0)))
    wiped = FAILURE(1);
  else {
    ...
    do {
      ...
      // compute the bounds of the square, given the bounds of x [taking into
      // account a hole centered on 0]
      if (lb_sr >= 0) {
...
#ifdef _DEBUG_SQ
      if (_DEBUG_SQ) {
        std::cout << "[" << lb_sr << ".." << ub_sr << "]^2 = [" << lb_sq << ".."
                  << ub_sq << "]" << std::endl;
      }
#endif

      if (FAILED(scope[1].set_max(ub_sq)) || FAILED(scope[1].set_min(lb_sq)))
        wiped = FAILURE(1);
      else {

#ifdef _DEBUG_SQ
        if (_DEBUG_SQ) {
          std::cout << scope[0].get_domain() << "^2 = ["
                    << scope[1].get_domain() << "]" << std::endl;
        }
#endif
        ...
        if (cur_ub_xsq < ub_sq) {
          ...
          if (FAILED(scope[0].set_max(ub_sr)))
            wiped = FAILURE(0);
          if (FAILED(scope[0].set_min(-ub_sr)))
            wiped = FAILURE(0);
        }
        if (IS_OK(wiped)) {
          ...
          if (cur_lb_xsq > lb_sq) {
            ...
            // std::cout << "\n\nHEREHERE" << std::endl;
            // std::cout << cur_ub_xsq << " " << (sqrt((double)(cur_ub_xsq))) <<
            // " " << ceil(sqrt((double)(cur_ub_xsq))) << " " << lb_sr <<
            // std::endl;
            // exit(1);

            if (lb_sr > 0) {
              if (FAILED(scope[0].remove_interval(1 - lb_sr, lb_sr - 1)))
                wiped = FAILURE(0);
            }
...
#ifdef _DEBUG_ABS
  if (_DEBUG_ABS) {
    std::cout << "*abs(" << scope[0].get_domain()
              << ") = " << scope[1].get_domain() << std::endl;
  }
#endif
...
Mistral::PropagationOutcome Mistral::PredicateAbs::propagate_change_on_absX(const Event evt) {
  ...
  if (scope[1].is_range()) {
    if (FAILED(scope[0].set_domain(-scope[1].get_max(), scope[1].get_max())))
      wiped = FAILURE(0);
    else {

      // std::cout << "scope[1].get_min() > 0 : " << (scope[1].get_min() > 0) <<
      // std::endl;
      // std::cout << "[" << (1-scope[1].get_min()) << "," <<
      // (scope[1].get_min()-1) << "]\n";

      if (scope[1].get_min() > 0 &&
          FAILED(scope[0].remove_interval(1 - scope[1].get_min(),
                                          scope[1].get_min() - 1)))
        wiped = FAILURE(0);
    }
  } else {
    ...
    if (FAILED(scope[0].set_domain(buffer)))
      wiped = FAILURE(0);
  }
...
Mistral::PropagationOutcome Mistral::PredicateAbs::propagate() {
  ...
  if (IS_OK(wiped))
    wiped = propagate_change_on_absX(LB_EVENT | UB_EVENT);
...
#ifdef _DEBUG_ABS
  if (_DEBUG_ABS) {
    std::cout << get_solver()->level << ": change on (" << scope[changed_idx]
              << "=" << scope[changed_idx].get_domain() << ")" << std::endl;
  }
#endif

  if (changed_idx)
    wiped = propagate_change_on_absX(evt);
  else
    wiped = propagate_change_on_X(evt);

  return wiped;
}

std::ostream& Mistral::PredicateAbs::display(std::ostream& os) const {
  os << ": abs(" << scope[0] /*.get_var()*/
     << ") == " << scope[1] /*.get_var()*/;
  return os;
}
  ...
  if (scope[0].is_ground() || scope[1].is_ground()) {
    ...
    for (int var = 0; hold && var < 2; ++var) {
      ...
      for (Event trig = 0; can_negate && trig < 3; ++trig) {
        for (int i = get_solver()->constraint_graph[idx].on[trig].size;
             can_negate && i--;) {
          con = get_solver()->constraint_graph[idx].on[trig][i];

          // std::cout << con << (con.absorb_negation(con.index()) ? " can" : "
          // cammot")
          // 	    << " absorb negation of " << con.get_scope()[con.index()] <<
          // std::endl;
...
Mistral::PropagationOutcome Mistral::PredicateNot::propagate(const int changed_idx, const Event evt) {
  ...
  //  std::cout << std::endl << scope[0].get_domain() << " " <<
  //  scope[1].get_domain() << std::endl;
  ...
  if (FAILED(scope[1 - changed_idx].set_domain(UB_CHANGED(evt))))
    wiped = FAILURE(1 - changed_idx);
...
Mistral::PropagationOutcome Mistral::PredicateNeg::propagate() {
  ...
  for (int changed_idx = 0; changed_idx < 2; ++changed_idx) {
    if (FAILED(scope[1 - changed_idx].set_max(-scope[changed_idx].get_min())))
      wiped = FAILURE(1 - changed_idx);
    if (FAILED(scope[1 - changed_idx].set_min(-scope[changed_idx].get_max())))
      wiped = FAILURE(1 - changed_idx);
    ...
    if (scope[changed_idx].get_size() < scope[1 - changed_idx].get_size()) {
      ...
      do {
        vali = vnxt;
        if (!scope[changed_idx].contain(-vali) &&
            FAILED(scope[1 - changed_idx].remove(vali)))
          wiped = FAILURE(1 - changed_idx);
...
Mistral::PropagationOutcome Mistral::PredicateNeg::propagate(const int changed_idx, const Event evt) {
  ...
  if (ASSIGNED(evt)) {
    if (FAILED(
            scope[1 - changed_idx].set_domain(-scope[changed_idx].get_min())))
      wiped = FAILURE(1 - changed_idx);
  } else {
    if (LB_CHANGED(evt)) {
      if (FAILED(scope[1 - changed_idx].set_max(-scope[changed_idx].get_min())))
...
Mistral::PropagationOutcome
Mistral::PredicateAnd::propagate(const int changed_idx, const Event evt) {
  Mistral::PropagationOutcome wiped = CONSISTENT;
  ...
  if (changed_idx == 2) {
    ...
    if (scope[2].equal(0)) {
      ...
      if (LB_CHANGED(evt)) {
        ...
        if (FAILED(scope[1 - changed_idx].set_domain(0)))
          wiped = FAILURE(1 - changed_idx);
      }
    } else if (scope[2].equal(1)) {
      // it is a (x and y) constraint
      if (UB_CHANGED(evt) || FAILED(scope[1 - changed_idx].set_domain(1)))
        wiped = FAILURE(1 - changed_idx);
    } else {
      ...
      if (UB_CHANGED(evt)) {
        ...
        if (FAILED(scope[2].set_domain(scope[1 - changed_idx].get_value())))
          wiped = FAILURE(2);
      }
    }
  }
...
Mistral::PropagationOutcome
Mistral::PredicateOr::propagate(const int changed_idx, const Event evt) {
  Mistral::PropagationOutcome wiped = CONSISTENT;
  ...
  if (changed_idx == 2) {
    ...
    if (scope[2].is_ground()) {
      if (scope[2].get_min()) {
        ...
        if (UB_CHANGED(evt)) {
          if (FAILED(scope[1 - changed_idx].remove(0)))
            wiped = FAILURE(1 - changed_idx);
        }
      } // otherwise, it is a "NOR" constraint and that has been dealt with when
        // scope[2] changed
    } else {
      if (LB_CHANGED(evt)) {
        if (FAILED(scope[2].remove(0)))
        ...
        if (FAILED(scope[2].set_domain(scope[1 - changed_idx].get_min())))
          wiped = FAILURE(2);
      }
    }
  }
...
std::ostream& Mistral::PredicateOr::display(std::ostream& os) const {
  os << scope[2] /*.get_var()*/ << " <=> (" << scope[0] /*.get_var()*/ << " or "
     << scope[1] /*.get_var()*/ << ")";
  return os;
}
...
Mistral::PropagationOutcome
Mistral::ConstraintAnd::propagate(const int changed_idx, const Event evt) {
  Mistral::PropagationOutcome wiped = CONSISTENT;
...
Mistral::PropagationOutcome Mistral::ConstraintNotAnd::propagate() {
  ...
  if (scope[1].get_min()) {
    if (FAILED(scope[0].remove(1)))
      wiped = FAILURE(0);
  } else if (scope[0].get_min()) {
    if (FAILED(scope[1].remove(1)))
      wiped = FAILURE(1);
  }
...
Mistral::PropagationOutcome
Mistral::ConstraintNotAnd::propagate(const int changed_idx, const Event evt) {
  Mistral::PropagationOutcome wiped = CONSISTENT;

  if (LB_CHANGED(evt) && FAILED(scope[1 - changed_idx].remove(1)))
    wiped = FAILURE(1 - changed_idx);
...
Mistral::PropagationOutcome
Mistral::ConstraintOr::propagate(const int changed_idx, const Event evt) {
  Mistral::PropagationOutcome wiped = CONSISTENT;

  if (UB_CHANGED(evt) && FAILED(scope[1 - changed_idx].remove(0)))
    wiped = FAILURE(1 - changed_idx);
...
Mistral::PropagationOutcome Mistral::ConstraintLex::propagate() {
  ...
  1: IF b1=0 THEN x0 = x1     ACTIVATION: change on b1 or state&1 and change on
  x0 or x1
  2: IF x0 =/= x1 THEN b1=1   ACTIVATION: change on x0 or x1

  4: IF b0<b1 THEN x0<x1      ACTIVATION: change on b0 or b1 or state&4 and
  change on x0 or x1
  8: IF x0>=x1 THEN b0=b1     ACTIVATION: change on x0 or x1 or state&8 and
  change on b0 or b1
  ...
  */
  ...
  int state =
      (!scope[3].get_max() + 4 * (scope[2].get_max() < scope[3].get_min()) +
...
#ifdef _DEBUG_LEX
    std::cout << " change on " << scope[evt] << " in "
              << scope[evt].get_domain() << " state: ( ";
    if (state & 1)
      std::cout << scope[3] << "=1 ";
    if (state & 4)
      std::cout << scope[2] << "<" << scope[3] << " ";
    if (state & 8)
      std::cout << scope[0] << ">=" << scope[1] << " ";
    std::cout << ")" << std::endl;
#endif
...
std::ostream& Mistral::ConstraintLex::display(std::ostream& os) const {
  os << "(" << scope[0] /*.get_var()*/ << " lex " << scope[1] /*.get_var()*/
     << " - " << scope[2] /*.get_var()*/ << "." << scope[3]   /*.get_var()*/
     << ")";
  return os;
}
...
#ifdef _DEBUG_ADD
  if (_DEBUG_ADD) {
    std::cout << "init prop " << scope[0].get_domain() << " + "
              << scope[1].get_domain() << " = " << scope[2].get_domain()
              << std::endl;
  }
#endif
...
#ifdef _DEBUG_ADD
  if (_DEBUG_ADD) {
    std::cout << scope[0] << " + " << scope[1] << " = " << scope[2]
              << std::endl;
    std::cout << scope[0].get_domain() << " + " << scope[1].get_domain()
...
Mistral::PropagationOutcome Mistral::PredicateModConstant::filter() {
...
#ifdef _DEBUG_MOD
  if (_DEBUG_MOD) {
    std::cout << scope[0] << " % " << modulo << " = " << scope[1] << std::endl;
    std::cout << scope[0].get_domain() << " % " << modulo << " = "
              << scope[1].get_domain() << std::endl
              << on[0] << std::endl
              << on[1] << std::endl;
  }
  ...
  if (FAILED(scope[1].set_max(Y.max)))
    wiped = FAILURE(1);
  else if (FAILED(scope[1].set_min(Y.min)))
    wiped = FAILURE(1);
  ...
  if (IS_OK(wiped)) {
    ...
    if (lb <= ub) {
      while (IS_OK(wiped) && lb <= scope[0].get_max()) {
        ...
        // std::cout << "remove interval [" << lb << "," << ub << "]" <<
        // std::endl;

        if (FAILED(scope[0].remove_interval(lb, ub)))
          wiped = FAILURE(0);
        lb += incr;
        ub += incr;
      }
  ...
  if (_DEBUG_MOD) {
    std::cout << on[0] << std::endl
              << on[1] << std::endl
              << scope[0].get_domain() << " % " << modulo << " = "
              << scope[1].get_domain() << (IS_OK(wiped) ? " ok" : " fail!")
              << std::endl
              << std::endl;
  }
#endif

  // if(FAILED(scope[1].set_max(max_modulo(scope[0].get_min(),
  // scope[0].get_max(), modulo)))) wiped = FAILURE(1);
  // if(FAILED(scope[1].set_min(min_modulo(scope[0].get_min(),
  // scope[0].get_max(), modulo)))) wiped = FAILURE(1);
  // if(FAILED(scope[0].set_min(min_antimodulo(scope[0].get_min(),
  // 					     scope[1].get_min(),
  // 					     scope[1].get_max(), modulo)))) wiped =
  // FAILURE(0);
...
Mistral::PropagationOutcome Mistral::PredicateCModConstant::filter() {
...
#ifdef _DEBUG_CMOD
  if (_DEBUG_CMOD) {
    std::cout << " => prune " << scope[1] << ": " << scope[1].get_domain()
              << " <- " << Y << std::endl;
  }
#endif
  ...
  if (IS_OK(wiped)) {
    ...
    if (IS_OK(wiped)) {
      ...
      if (min_y > 0) { // the image of the modulo is positive, so the domain
                       // must be positive

#ifdef _DEBUG_CMOD
        if (_DEBUG_CMOD) {
          std::cout << "   => " << scope[1] << " in " << scope[1].get_domain()
                    << " is positive, hence " << scope[0]
                    << " must be positive " << std::endl;
        }
#endif
        if (FAILED(scope[0].set_min(1)))
          wiped = FAILURE(0);
      ...
      } else if (max_y < 0) { // the image of the modulo is negative, so the
                              // domain must be negative
        if (FAILED(scope[0].set_max(-1)))
          wiped = FAILURE(0);

#ifdef _DEBUG_CMOD
        if (_DEBUG_CMOD) {
          std::cout << "   => " << scope[1] << " in " << scope[1].get_domain()
                    << " is negative, hence " << scope[0]
...
// std::cout << "\n ^^^" << std::endl;
  ...
  while (vnext > modulo) {
    ...
    if (scope[2].get_min() > Y.max || scope[2].get_max() < Y.min) {
      ...
      if (FAILED(scope[1].remove(modulo)))
        wiped = FAILURE(1);
    } else {
      ...
      if (IS_OK(wiped)) {
        ...
        if (modulo > 0) {
          left_lb.min =
              ...
              /*move to the next 0 on the left*/ -
              Mistral::__modulo_fct__(min_x, modulo);
          ...
          left_ub.max =
              /*start from min(x)*/ min_x
              /*move to the next mod-1 on the right*/ -
              Mistral::__modulo_fct__(min_x, modulo) + modulo - 1;
          left_ub.min = left_ub.max + max_y - modulo + 2;

          right_ub.max =
              /*start from max(x)*/ max_x
              /*move to the next mod-1 on the right*/ -
              Mistral::__modulo_fct__(max_x, modulo) + modulo - 1;
...
// #endif
...
#ifdef _DEBUG_MOD
          if (_DEBUG_MOD) {
            std::cout << " all pruning for this round: " << cur_intervals
                      << std::endl;
          ...
          left_lb.min =
              /*start from min(x)*/ min_x
              /*move to the next 1+mod on the left*/ -
              Mistral::__modulo_fct__(min_x, modulo) + modulo + 1;
          ...
          left_ub.max =
              /*start from min(x)*/ min_x
              /*move to the next 0 on the right*/ -
              Mistral::__modulo_fct__(min_x, modulo);
...
#ifdef _DEBUG_MOD
          if (_DEBUG_MOD) {
            std::cout << " -> " << left_lb << "/" << left_ub
                      << " min(x) = " << min_x << std::endl;
          }
// exit(1);
#endif

          if (!left_lb.empty() && left_lb.min <= min_x &&
              left_lb.max >= min_x) {
            min_x = left_lb.max + 1;
            shrink = true;
          }
          if (!left_ub.empty() && left_ub.min <= min_x &&
              left_ub.max >= min_x) {
            min_x = left_ub.max + 1;
            shrink = true;
          }
...
#ifdef _DEBUG_MOD
          if (_DEBUG_MOD) {
            std::cout << " -> " << right_lb << "/" << right_ub
                      << " max(x) = " << max_x << std::endl;
          }
// exit(1);
#endif
          shrink = false;

          if (!right_lb.empty() && right_lb.max >= max_x &&
              right_lb.min <= max_x) {
            max_x = right_lb.min - 1;
            shrink = true;
          }
          if (!right_ub.empty() && right_ub.max >= max_x &&
              right_ub.min <= max_x) {
...
#ifdef _DEBUG_MOD
    if (_DEBUG_MOD) {
      std::cout << " => X in [" << minmin_x << "," << maxmax_x << "], Y in ["
                << minmin_y << "," << maxmax_y << "]\n"
                << std::endl;
    }
#endif
  ...
  // if(IS_OK(wiped) && minmin_x <  INFTY && FAILED(scope[0].set_min(minmin_x)))
  // wiped = FAILURE(0);
  if (IS_OK(wiped) && minmin_y < INFTY && FAILED(scope[2].set_min(minmin_y)))
    wiped = FAILURE(2);
  // if(IS_OK(wiped) && maxmax_x > -INFTY && FAILED(scope[0].set_max(maxmax_x)))
  // wiped = FAILURE(0);
  if (IS_OK(wiped) && maxmax_y > -INFTY && FAILED(scope[2].set_max(maxmax_y)))
    wiped = FAILURE(2);

#ifdef _DEBUG_MOD
  if (_DEBUG_MOD) {
    std::cout << " +++> remove " << forbidden_intervals << " from "
              << scope[0].get_domain() << " / set domain to [" << minmin_x
              << "," << maxmax_x << "]\n";
  }
#endif

  for (unsigned int i = 0; IS_OK(wiped) && i < forbidden_intervals.size; ++i) {
    if (FAILED(scope[0].remove_interval(forbidden_intervals[i].min,
                                        forbidden_intervals[i].max)))
      wiped = FAILURE(0);
  }
...
// else if(FAILED(scope[2].set_domain(minmin_y, maxmax_y))) wiped = FAILURE(2);
  ...
  if (_DEBUG_MOD) {
    std::cout << on[0] << std::endl
              << on[1] << std::endl
              << on[2] << std::endl
              << scope[0].get_domain() << " % " << scope[1].get_domain()
              << " = " << scope[2].get_domain()
              << (IS_OK(wiped) ? " ok" : " fail!") << std::endl
              << std::endl;
  }
...
Mistral::PropagationOutcome
Mistral::PredicateMod::propagate(const int changed_idx, const Event evt) {
  ...
  //     //compute the min and max possible values for x%modulo, and update the
  //     current lb/ub of z accordingly
  ...
  //     //compute the min and max possible values for x given that x%modulo =
  //     z, and update the current lb/ub of x accordingly
  ...
  //     if(modmin>max_z || modmax<min_z || antimodmin>max_x ||
  //     antimodmax<min_x) {
  //       // forbidden value
  ...
  //   // // 	  vali = vnxt;
  //   // // 	  k = (vali % modulo);
  //   // // 	  if(!scope[2].contain( k ) && FAILED(scope[0].remove( vali )))
  //   wiped = FAILURE(0);
  ...
  //   // // 	//consistent = scope[2]->setDomain( (scope[0].get_min() %
  //   scope[1].get_min()) );
  //   // // 	if(FAILED(scope[2].set_domain( (scope[0].get_value() %
  //   scope[1].get_value()) ))) wiped = FAILURE(2);
  //   // //     } else {
  ...
  // 	      << scope[0].get_domain() << " % "
  // 	      << scope[1].get_domain() << " = "
  // 	      << scope[2].get_domain() << (IS_OK(wiped) ? " ok" : " fail!") <<
  // std::endl << std::endl;
  //   }
  // #endif
        ...
        if (min_y > 0) { // the image of the modulo is positive, so the domain
          ...
          if (lb <= ub) {
            while (IS_OK(wiped) && lb <= max_x) {
              // #ifdef _DEBUG_CMOD
              // 	    if(_DEBUG_CMOD) {
              // 	      std::cout << "    -> remove interval [" << lb <<
              // "," << ub << "]" << std::endl;
              // 	    }
              // #endif
              // if(FAILED(scope[0].remove_interval(lb, ub))) wiped =
              // FAILURE(0);
...
Mistral::PropagationOutcome
Mistral::PredicateCMod::propagate(const int changed_idx, const Event evt) {
  return filter();
}
...
Mistral::PropagationOutcome Mistral::PredicateDiv::filter() {
  ...
  if (FAILED(scope[2].set_domain(Z)))
    wiped = FAILURE(2);
  ...
  if (IS_OK(wiped)) {
    ...
    X = Z.anti_div_X(Y);
    if (FAILED(scope[0].set_domain(X)))
      wiped = FAILURE(0);
    ...
    if (IS_OK(wiped)) {
      ...
      if (_DEBUG_DIV) {
        ...
        std::cout << "filter " << scope[1].get_domain() << " = " << X << "/"
                  << Z << std::endl;
      }
#endif
      ...
      Y = Z.anti_div_Y(X);
      // Y = X.anti_mul(Z);
      if (FAILED(scope[1].set_domain(Y)))
        wiped = FAILURE(1);
    }
  }
...
Mistral::PropagationOutcome
Mistral::PredicateDiv::propagate(const int changed_idx, const Event evt) {
...
#ifdef _DEBUG_DIV
  if (_DEBUG_DIV) {
    std::cout << scope[0] << " / " << quotient << " = " << scope[1]
              << std::endl;
    std::cout << scope[0].get_domain() << " / " << quotient << " = "
              << scope[1].get_domain() << std::endl
              << on[0] << std::endl
              << on[1] << std::endl;
  }
    ...
    for (; i < 2; ++i)
      if (scope[i].is_ground()) {
        ...
        if (scope[i].get_min() == 1) {
          ...
          get_solver()->add(
              Constraint(new PredicateFactor(tmp, scope[i].get_min())));
        }
      }
  }
  return wiped;
...
Mistral::PropagationOutcome Mistral::PredicateMul::revise_division(const int X, const int Y, const int Z) {
  ...
  // we start with all bounds at their previous values, and update them if
  // necessary
  // (in which case we set up the pruning flag)
  int lb_pos = min_pos[Z], ub_pos = max_pos[Z], lb_neg = min_neg[Z],
      ub_neg = max_neg[Z], nlb1, nlb2, nub1, nub2;
  // lb_aux, ub_aux;
  bool pruning_flag = false, pzero = false, ppos = max_pos[Z] <= 0,
       pneg = min_neg[Z] >= 0;
  ...
  // if(lb_neg != ub_pos) {
  if (!zero[X] ||
      !zero[Y]) { // if X and Y can both be 0, we cannot deduce anything
    if (IS_OK(wiped)) {
      if (max_pos[Z] > 0) {
        ...
        if (lb_pos > max_pos[Z] || ub_pos < min_pos[Z])
          ppos = true;
      ...
      if (min_neg[Z] < 0) {
        ...
        if (lb_neg > max_neg[Z] || ub_neg < min_neg[Z])
          pneg = true;
...
Mistral::PropagationOutcome
Mistral::PredicateMul::prune(const int lb_neg, const int ub_neg,
                             const int lb_pos, const int ub_pos,
                             const bool pzero, const int Z) {
  ...
  if (ub_pos < lb_neg)
    ...
    if (IS_OK(wiped) && (lb_pos >= min_neg[Z] || ub_neg <= max_pos[Z])) {
      if (lb_pos - 1 > ub_neg &&
          (pzero ||
           (!zero[Z] && (min_pos[Z] < lb_pos || max_neg[Z] > ub_neg)))) {
  ...
  if ((min_neg[Z] > 0 && min_neg[Z] < min_pos[Z]) || (!min_neg[Z] && !zero[Z]))
    min_neg[Z] = min_pos[Z];

  if ((max_pos[Z] < 0 && max_pos[Z] > max_neg[Z]) || (!max_pos[Z] && !zero[Z]))
    max_pos[Z] = max_neg[Z];
  ...
  if (_DEBUG_MUL) {
    ...
    std::cout << " now in [" << min_neg[Z] << ".." << max_neg[Z] << "|"
              << (zero[Z] ? "{0}" : "_") << "|" << min_pos[Z] << ".."
              << max_pos[Z] << "]" << std::endl;
...
Mistral::PropagationOutcome Mistral::PredicateMul::propagate() {
  ...
  if (!is_ground) {
    while (IS_OK(wiped) && !changes.empty()) {
      ...
      if (_DEBUG_MUL) {
        ...
        std::cout << std::endl
                  << "react to " << scope[evt_idx] /*.get_var()*/ << " in "
      ...
      // x0 * x1 = x2
      if (evt_idx == 2) {
        for (int i = 0; IS_OK(wiped) && i < 2;
             ++i) // once for scope[0], once for scope[1]
        ...
        // update x[1-evt_idx]
        if (IS_OK(wiped))
          wiped = revise_division(2, evt_idx, 1 - evt_idx);
      }
    }

  } else {

    if (min_neg[0] * min_neg[1] != min_neg[2])
      wiped = 0;
  }
  ...
  if (_DEBUG_MUL) {
    ...
    if (IS_OK(wiped)) {
      ...
      std::cout << "  ===> " << scope[0] /*.get_var()*/ << " in "
                << scope[0].get_domain() << " * " << scope[1] /*.get_var()*/
                << " in " << scope[1].get_domain() << " = "
                << scope[2] /*.get_var()*/ << " in " << scope[2].get_domain()
                << std::endl;
...
#ifdef _OS_PRUNING
      if (values[i].empty()) {
        support_lists[n].initialise(solver, 0, table.size, 8, false);
      } else {
...
#else
      if (values[i].empty()) {
        support_of[i][val].initialise(solver, 0, table.size, 8, false);
      } else {
  ...
  for (unsigned int xi = 0; xi < arity; ++xi) {
    ...
    for (Domain::iterator xit = dom_xi.end(); xit != xstop;) {
      ...
      if (support_of[xi][valj]->empty()) {
        if (FAILED(scope[xi].remove(valj)))
          get_solver()->fail();
      }
#else
      if (support_of[xi][valj].empty()) {
...
Mistral::PropagationOutcome Mistral::ConstraintGAC4::propagate() 
{
  ...
  for (sk = pruned_lists.size; --sk >= 0;) {
    ...
    if (support_lists[tk].empty()) {
      ...
      if (FAILED(scope[var_map[tk]].remove(val_map[tk]))) {
        wiped = FAILURE(var_map[tk]);
        goto FAIL;
      }
    }
...
bool Mistral::ConstraintGAC4::check_integrity( ) const 
{
  ...
  for (xi = 0; xi < arity; ++xi) {
    ...
    for (Domain::iterator xit = dom_xi.begin(); xit != xend; ++xit) {
      ...
      ReversibleSet::iterator past =
          first + init_support_size[list_map[xi][vali]];
      ...
      if (!entailed)
        for (ReversibleSet::iterator sit = first; sit != last; ++sit) {
          tk = *sit;
          for (xj = 0; xj < arity; ++xj) {
            if (!(scope[xj].contain(table[tk][xj]))) {
              std::cout << this << " " << id << ": tuple " << tk << ": <"
                        << table[tk][0];
...
void Mistral::ConstraintTable::add(const int *tuple) { table.add(tuple); }
...
void Mistral::ConstraintGAC2001::initialise() {
  ...
  for (i = 0; i < arity; ++i) {
    ...
    // std::cout << scope[i] << " in " << scope[i].get_domain() << "[" <<
    // themins[i] << ".." << scope[i].get_initial_max() << "] = " << nval <<
    // std::endl;
  ...
  for (i = 0; i < arity; ++i) {
    ...
    supportList[i] =
        new Vector<const int *>[scope[i].get_initial_max() - themins[i] + 1];
...
Mistral::ConstraintGAC2001::~ConstraintGAC2001() 
{
  ...
  // std::cout << id << " ";
  //   for(i=0; i<arity; ++i) {
  // 	std::cout << " [" << scope[i].get_initial_min() << ".." <<
  // scope[i].get_initial_max() << "]:" ;
  // 	std::cout << table << "-";
  // 	std::cout << table[0] << "-";
  // 	std::cout << table[0][i];
  //   }
    ...
    for (i = 0; IS_OK(wiped) && i < arity; ++i) {
      ...
      if (oi != changedIdx && (changes.contain(oi) || active.contain(oi))) {
        ...
        while (xit != xend && IS_OK(wiped)) {
          ...
          if (valid)
            firstSupport[oi][j] = index - 1;
          else {
#ifdef _DEBUG_TABLE
            if (_DEBUG_TABLE) {
              std::cout << "remove " << j << " from " << scope[oi].get_domain()
                        << " " << scope[oi].get_size() << std::endl;
            }
#endif
...
void Mistral::ConstraintGAC3::initialise() {
  ...
  var_sizes[arity - 2] = (scope[arity - 1].get_initial_max() -
                          scope[arity - 1].get_initial_min() + 1);
  for (i = arity - 3; i >= 0; --i) {
    var_sizes[i] = var_sizes[i + 1] * (scope[i + 1].get_initial_max() -
                                       scope[i + 1].get_initial_min() + 1);
  }
...
Mistral::ConstraintGAC3::~ConstraintGAC3() { delete[] var_sizes; }
...
Mistral::ConstraintBoolSumInterval::ConstraintBoolSumInterval(
    Vector<Variable> &scp, const int l, const int u)
    : GlobalConstraint(scp) {
  priority = 1;
  lower_bound = l;
  upper_bound = u;
...
#ifdef _CBSI_WC
double Mistral::ConstraintBoolSumInterval::weight_conflict(double unit, Vector<double>& weights)
{
...
max_ = _max_;
  ...
  if (min_ > upper_bound || max_ < lower_bound) {
    ...
    for (unsigned int i = 0; i < active.size; ++i) {
      ...
      // std::cout << "scope[" << j << "].equal(1): " < scope[j].equal(1) <<
      // std::endl;
...
void Mistral::ConstraintBoolSumInterval::initialise_activity(double *lact, double *vact, double norm) {
  ...
  //   //std::cout << "ratio #sol[x=0]: " << ratio_0 << " weight: " <<
  //   (1-ratio_0) << std::endl;

  //   //std::cout << "ratio #sol[x=1]: " << ratio_1 << " weight: " <<
  //   (1-ratio_1) << std::endl;

  //   double incr_0 = norm*(1-ratio_0);
  //   double incr_1 = norm*(1-ratio_1);

  //   // std::cout << "ratio #sol[x=0]: " << ratio_0 << " weight: " << incr_0
  //   << std::endl;

  //   // std::cout << "ratio #sol[x=1]: " << ratio_1 << " weight: " << incr_1
  //   << std::endl;
...
Mistral::Explanation::iterator Mistral::ConstraintBoolSumInterval::get_reason_for(const Atom a, const int lvl, Explanation::iterator& end) {
  ...
  if (a != NULL_ATOM) {
    if (get_solver()->variables[a].get_value()) {
      ...
      while (i--) {
        if (!(scope[i].get_max()))
          explanation.add(literal(scope[i], true));
      }
    } else {
      // the literal we try to explain is positive
      while (i--) {
        if (scope[i].get_min())
          explanation.add(literal(scope[i], false));
      }
    }
  } else {
    if (min_ > upper_bound) {
...
#ifdef _PBS_WC
double Mistral::PredicateBoolSum::weight_conflict(double unit, Vector<double>& weights)
{
  ...
  std::cout << ") = " << scope[scope.size - 1] << " in "
            << scope[scope.size - 1].get_domain() << std::endl;
#endif
...
Mistral::PropagationOutcome Mistral::PredicateBoolSum::propagate() 
{
  ...
  if (_min_ > scope[n].get_max() || _max_ < scope[n].get_min())
    wiped = FAILURE(n);
  else {
    if (FAILED(scope[n].set_min(_min_)))
      wiped = FAILURE(n);
    else if (FAILED(scope[n].set_max(_max_)))
      wiped = FAILURE(n);
...
Mistral::PropagationOutcome Mistral::PredicateWeightedSum::rewrite() {
  ...
  // std::cout << scope.size << " " << wpos << " " << wneg << " " << lower_bound
  // << " " << upper_bound << std::endl;

  // check if it can be rewritten as an ADD predicate
  if (scope.size == 3 && wpos == 1 && wneg == 1 && lower_bound == 0 &&
      upper_bound == 0) {
    ...
    relax();
    get_solver()->add(
        Constraint(new PredicateAdd(scope[1], scope[2], scope[0])));
  }
    ...
    if (lower_bound == upper_bound) {
      ...
      if (lower_bound == 0) {
        ...
        get_solver()->add(
            Constraint(new PredicateOffset(scope[1], scope[0], lower_bound)));
      }
    } else {
      if (upper_bound == INFTY) {
        ...
        relax();
        get_solver()->add(
            Constraint(new ConstraintLess(scope[1], scope[0], -lower_bound)));
      }
    }
  }
...
#ifdef _PWS_WC
double Mistral::PredicateWeightedSum::weight_conflict(double unit, Vector<double>& weights)
{
#ifdef _DEBUG_WEIGHT_CONFLICT
  std::cout << "\nWEIGHT " << this << " (\n";
    ...
    for (i = 0; i < wneg; ++i) {
      ...
      if (idx >= 0 && scope[i].get_max() < scope[i].get_initial_max()) {
        ...
        std::cout << " >> does not weight " << scope[i] << " because "
                  << scope[i].get_max() << " = " << scope[i].get_initial_max()
                  << std::endl;
      }
#endif
    }
  ...
  for (i = wpos; i < wneg; ++i) {
    ...
    if (idx >= 0) {
      ...
      weight_contributed[i] +=
          unit * ((double)(weight[i])) / ((double)(scope.size));
#endif
      if (the_max < weights[idx])
        the_max = weights[idx];
    }
...
Mistral::PropagationOutcome Mistral::PredicateWeightedSum::propagate() 
{
  ...
  for (i = 0; i < wpos; ++i) {
    ...
    if (_DEBUG_WEIGHTEDSUM) {
      if (i)
        std::cout << " + [" << lo_bound[i] << "," << up_bound[i] << "] = ["
                  << smin << "," << smax << "] ";
      else
        std::cout << "[" << smin << "," << smax << "] ";
    }
  ...
  while (IS_OK(wiped) && !events.empty()) {
    ...
    if (lower_bound == upper_bound) {
      ...
      while (j--) {
        ...
        // std::cout << i << ": " << (span[i]) << " " <<
        // (unknown_parity.contain(i)) << std::endl;
      ...
      if (_DEBUG_WEIGHTEDSUM) {
        display(std::cout);
        std::cout << std::endl
                  << unknown_parity << ": " << (parity ? "odd" : "even")
                  << std::endl;
      }
#endif
      ...
      if (unknown_parity.size == 0) {
        ...
        if (parity != 0)
          wiped = FAILURE(arity - 1);
      } else if (unknown_parity.size ==
                 1) { // it needs to be of parity "parity"
        i = unknown_parity[0];

#ifdef _DEBUG_WEIGHTEDSUM
        if (_DEBUG_WEIGHTEDSUM) {
          std::cout << "parity pruning: " << (lo_bound[i] % 2) << " " << parity
                    << std::endl;
        }
#endif
        ...
        while (IS_OK(wiped) && (lo_bound[i] % 2 == 0) != (parity == 0)) {
          ...
          tmin = lo_bound[i];
          if (FAILED(scope[i].set_min(++lo_bound[i])))
            wiped = FAILURE(i);
          else {
            lo_bound[i] = scope[i].get_min();
            if (i < wneg)
              smin += ((lo_bound[i] - tmin) * weight[i]);
            else
              smax += ((lo_bound[i] - tmin) * weight[i]);
          }
        ...
        while (IS_OK(wiped) && (up_bound[i] % 2 == 0) != (parity == 0)) {
          ...
          tmin = up_bound[i];
          if (FAILED(scope[i].set_max(--up_bound[i])))
            wiped = FAILURE(i);
          else {
            up_bound[i] = scope[i].get_max();
            if (i < wneg)
              smax -= ((tmin - up_bound[i]) * weight[i]);
            else
              smin -= ((tmin - up_bound[i]) * weight[i]);
          }
    ...
    if (IS_OK(wiped)) {
      ...
      if (smax < lower_bound || smin > upper_bound)
        wiped = FAILURE(arity - 1);
      else {
        ...
        for (i = 0; IS_OK(wiped) && i < wpos; ++i) {
          ...
          if (tmin < (up_bound[i] - lo_bound[i])) {
            ...
            if (_DEBUG_WEIGHTEDSUM) {
              std::cout << scope[i] << " in " << scope[i].get_domain()
                        << " <= " << (lo_bound[i] + tmin) << std::endl;
            }
#endif

            if (FAILED(scope[i].set_max(lo_bound[i] + tmin)))
              wiped = FAILURE(i);
            else {
        ...
        for (i = wpos; IS_OK(wiped) && i < wneg; ++i) {
          ...
          if (tmin < (up_bound[i] - lo_bound[i]) * weight[i]) {
            ...
            if (_DEBUG_WEIGHTEDSUM) {
              std::cout << scope[i] << " in " << scope[i].get_domain()
                        << " <= " << (lo_bound[i] + tmin / weight[i])
                        << std::endl;
            }
#endif

            if (FAILED(scope[i].set_max(lo_bound[i] + tmin / weight[i])))
              wiped = FAILURE(i);
        ...
        for (i = wneg; IS_OK(wiped) && i < arity; ++i) {
          ...
          if (tmin < (lo_bound[i] - up_bound[i]) * weight[i]) {
            ...
            if (_DEBUG_WEIGHTEDSUM) {
              std::cout << scope[i] << " in " << scope[i].get_domain()
                        << " >= " << (up_bound[i] + tmin / weight[i])
                        << std::endl;
            }
#endif

            if (FAILED(scope[i].set_min(up_bound[i] + tmin / weight[i])))
              wiped = FAILURE(i);
        ...
        for (i = 0; IS_OK(wiped) && i < wpos; ++i) {
          ...
          if (tmax < (up_bound[i] - lo_bound[i])) {
            ...
            if (_DEBUG_WEIGHTEDSUM) {
              std::cout << scope[i] << " in " << scope[i].get_domain()
                        << " >= " << (up_bound[i] - tmax) << std::endl;
            }
#endif

            if (FAILED(scope[i].set_min(up_bound[i] - tmax)))
              wiped = FAILURE(i);
        ...
        for (i = wpos; IS_OK(wiped) && i < wneg; ++i) {
          ...
          if (tmax < (up_bound[i] - lo_bound[i]) * weight[i]) {
            ...
            if (_DEBUG_WEIGHTEDSUM) {
              std::cout << scope[i] << " in " << scope[i].get_domain()
                        << " >= " << (up_bound[i] - tmax / weight[i])
                        << std::endl;
            }
#endif

            if (FAILED(scope[i].set_min(up_bound[i] - tmax / weight[i])))
              wiped = FAILURE(i);
        ...
        for (i = wneg; IS_OK(wiped) && i < arity; ++i) {
          ...
          if (tmax < (lo_bound[i] - up_bound[i]) * weight[i]) {
            ...
            if (_DEBUG_WEIGHTEDSUM) {
              std::cout << scope[i] << " in " << scope[i].get_domain()
                        << " <= " << (lo_bound[i] - tmax / weight[i])
                        << std::endl;
            }
#endif

            if (FAILED(scope[i].set_max(lo_bound[i] - tmax / weight[i])))
              wiped = FAILURE(i);
...
Mistral::PropagationOutcome Mistral::ConstraintParity::propagate() 
{
  ...
  if (active.size <= 1) {
...
#ifdef _DEBUG_PARITY
      if (_DEBUG_PARITY) {
        std::cout << " : " << scope[i].get_domain() << " -> " << parity
                  << std::endl;
      }
#endif
    }
    if (active.size) {
      ...
      if (FAILED(scope[active.back()].set_domain(parity != target_parity)))
        wiped_idx = FAILURE(active.back());
    } else {

#ifdef _DEBUG_PARITY
      if (_DEBUG_PARITY) {
        std::cout << " all assigned: "
                  << (parity == target_parity ? "ok" : "fail!") << std::endl;
      }
#endif
...
Mistral::ConstraintWeightedBoolSumInterval::ConstraintWeightedBoolSumInterval(
    Vector<Variable> &scp, Vector<int> &wgt, const int L, const int U)
    : GlobalConstraint(scp), lower_bound(L), upper_bound(U) {
  priority = 1;
  for (unsigned int i = 0; i < scope.size; ++i) {
    weight.add(wgt[i]);
...
Mistral::ConstraintWeightedBoolSumInterval::ConstraintWeightedBoolSumInterval(
    std::vector<Variable> &scp, std::vector<int> &wgt, const int L, const int U)
    : GlobalConstraint(scp), lower_bound(L), upper_bound(U) {
  priority = 1;
  for (unsigned int i = 0; i < scope.size; ++i) {
    weight.add(wgt[i]);
...
#ifdef _CWBSI_WC
double Mistral::ConstraintWeightedBoolSumInterval::weight_conflict(double unit, Vector<double>& weights)
{
...
Mistral::Explanation::iterator Mistral::ConstraintWeightedBoolSumInterval::get_reason_for(const Atom a, const int lvl, Explanation::iterator& end) {
  ...
  for each pruning, the constraint has stored the reason with the following
  code:
  1 -> parity
  2 -> lb
  3 -> ub

  case 1: find the variables with odd coefficients that are not variables[a].
  They are necessarily bound to prune a
  case 2: sum(max(w_x)) - w_a < lb ->
                  find a set of x, assigned before lvl and such that
  sum(max(w_x)) - w_a < lb


  in fact, only parity pruning must be stored. Otherwise, we can deduce it from
  the value of a (and the sign of its weight).
  ...
  */
  ...
  /*
  if(a != NULL_ATOM) {
  std::cout << "extract a reason for " << get_solver()->variables[a] << " = " <<
  get_solver()->variables[a].get_value() << " from: " << this << " with: " <<
  std::endl;
  } else {
  std::cout << "extract a reason for failure of: " << this << " with: " <<
  std::endl;
  }
  ...
  std::cout << std::endl;
  for(int i=0; i<scope.size; ++i) {
  if(!(IS_GROUND(domains[i])) || (a == NULL_ATOM) ||
  get_solver()->assignment_order.stack_[a] >
  get_solver()->assignment_order.stack_[scope[i].id()])
  std::cout << std::setw(5) << scope[i].get_domain();
  else
  std::cout << "{0,1}";
  }
  ...
  */
  ...
  while (i--) {
    ...
    if ((Atom)idx == a) {
      ...
      // a=1 and coeff is negative => 1 <=> pushed toward the lower value
      // (positive_contributors are the reason)

      // a=1 and coeff is positive or
      // a=0 and coeff is negative => 0 <=> pushed toward the upper value
      // (negative_contributors are the reason)
    ...
    // std::cout << "+ " << weight[i] << "." << scope[i].get_domain() << ": ["
    // << bounds[0] << "," << bounds[1] << "]\n";
}
...
if (a == NULL_ATOM) {
  if (bounds[0] > upper_bound)
    direction = true;
  else if (bounds[1] < lower_bound)
    direction = false;
  else {
...
//     if(total + (1-vala)*weight[va] <= upper_bound || total + vala*weight[va]
//     > upper_bound) {
...
//     for(i=0; i<positive_contributors.size; ++i) {
// 	std::cout << std::setw(5) <<
// get_solver()->variables[positive_contributors[i]/2];
//     }
//     std::cout << std::endl;

//     std::cout << " when all these guys are set in this way, we have " <<
//     (bounds[0]-total) << " + " << total ;
//     if(va != INFTY) {
// 	if(GET_VAL(domains[va]))
// 	  std::cout << " + 0 > " << upper_bound << " hence we need " <<
// scope[va] << " to be " << weight[va] << std::endl;
// 	else
// 	  std::cout << " + " << weight[va] << " > " << upper_bound << " hence we
// need " <<  scope[va] << " to be 0" << std::endl;
//     } else {

// 	std::cout << " hence we need one of them to be assigne differently" <<
// std::endl;
...
//     if(total + (1-vala)*weight[va] >= lower_bound || total + vala*weight[va]
//     < lower_bound) {
// 	std::cout << "ERROR!!" << std::endl;
// 	error = true;
//     }
...
//     for(i=0; i<negative_contributors.size; ++i) {
// 	std::cout << std::setw(5) <<
// get_solver()->variables[negative_contributors[i]/2];
//     }
//     std::cout << std::endl;

//     std::cout << " when all these guys are set in this way, we have " <<
//     (bounds[1]-total) << " + " << total ;
//     if(va != INFTY) {
// 	if(GET_VAL(domains[va]))
// 	  std::cout << " + 0 < " << lower_bound << " hence we need " <<
// scope[va] << " to be " << weight[va] << std::endl;
// 	else
// 	  std::cout << " + " << weight[va] << " < " << lower_bound << " hence we
// need " <<  scope[va] << " to be 0" << std::endl;
//     } else {
...
// 	std::cout << "NOGOOD INVOLVES A VARIABLE FIXED LATER THAN THE ATOM!" <<
// std::endl;
// 	exit(1);
//     }
...
return (direction ? positive_contributors.begin()
                  : negative_contributors.begin());
}

Mistral::PropagationOutcome
Mistral::ConstraintWeightedBoolSumInterval::propagate() {
...
// span[i] = (up_bound[i]-lo_bound[i]);
    ...
    if (_DEBUG_WEIGHTEDBOOLSUM) {
      if (i)
        std::cout << " + [" << GET_MIN(domains[i]) << "," << GET_MAX(domains[i])
                  << "] = [" << smin << "," << smax << "] ";
      else
        std::cout << "[" << smin << "," << smax << "] ";
    }
  ...
  for (i = wpos; i < wneg; ++i) {
    smax +=
        weight[i] * GET_MAX(domains[i]); //(up_bound[i] = scope[i].get_max());
    smin +=
        weight[i] * GET_MIN(domains[i]); //(lo_bound[i] = scope[i].get_min());
// span[i] = weight[i] * (up_bound[i]-lo_bound[i]);
  ...
  while (IS_OK(wiped) && !events.empty()) {
    ...
    if (lower_bound == upper_bound) {
      ...
      if (unknown_parity.size == 0) {
        ...
        // #ifdef _DEBUG_WEIGHTEDBOOLSUM
        // 	if(_DEBUG_WEIGHTEDBOOLSUM) {
        std::cout << "parity pruning: " << (GET_MIN(domains[i]) % 2) << " "
                  << parity << std::endl;
// 	}
// #endif
...
std::ostream &
Mistral::ConstraintWeightedBoolSumInterval::display(std::ostream &os) const {
#ifdef _GET_SUM_NAME
  os << " cwbsi: ";
#endif
...
Mistral::ConstraintIncrementalWeightedBoolSumInterval::
...
Mistral::ConstraintIncrementalWeightedBoolSumInterval::
    ~ConstraintIncrementalWeightedBoolSumInterval() {
#ifdef _DEBUG_MEMORY
  std::cout << "c delete weightedsum constraint" << std::endl;
#endif
  delete[] domains;
...
void Mistral::ConstraintIncrementalWeightedBoolSumInterval::initialise_activity(
    double *lact, double *vact, double norm) {
  ...
  exponent +=
      (double)(real_max - real_min + 1) / (double)(bound_[1] - bound_[0] + 1);
...
double Mistral::ConstraintIncrementalWeightedBoolSumInterval::weight_conflict(
    double unit, Vector<double> &weights) {
...
Mistral::Explanation::iterator
Mistral::ConstraintIncrementalWeightedBoolSumInterval::get_reason_for(
    const Atom a, const int lvl, Explanation::iterator &end) {

#ifdef _DEBUG_REASONIWBS
  if (_DEBUG_REASONIWBS) {
    std::cout << "compute reason for " << a << " with respect to " << this
              << std::endl;
    for (int i = 0; i < scope.size; ++i) {
  ...
  if (a != NULL_ATOM) {
    ...
    if (!a_exists) {
      std::cout << "request reason for " << a << " not constrained by " << this
                << std::endl;
      exit(1);
    }
  }
...
// }
  ...
  if (_DEBUG_REASONIWBS) {
    ...
    std::cout << "[" << bound_[0] << ".." << bound_[1] << "] vs ["
              << lower_bound << ".." << upper_bound << "]\n";

    if (a != NULL_ATOM) {
      if (direction) {
        std::cout << scope[i] << " was set to a high value ("
                  << (GET_VAL(domains[i]) * weight[i])
                  << ") because otherwise the maximum would be too low " << a
                  << std::endl;
      } else {
        std::cout << scope[i] << " was set to a low value ("
                  << (GET_VAL(domains[i]) * weight[i])
                  << ") because otherwise the minimum would be too high " << a
                  << std::endl;
      }
    } else {
      if (direction) {
...
Mistral::PropagationOutcome
Mistral::ConstraintIncrementalWeightedBoolSumInterval::propagate() {
...
bound_[1] = _max_;
...
#ifdef _DEBUG_IWEIGHTEDBOOLSUM
      if (_DEBUG_IWEIGHTEDBOOLSUM) {
        std::cout << "\nprocessing events: " << event2strc(event_type[i])
                  << " on " << scope[i] << " in " << scope[i].get_domain()
                  << std::endl;
      }
#endif

      // the commented way is faster, but the initial propagate might be
      // triggered with invalid bound information
      // if(LB_CHANGED(event_type[i])) {
  ...
  if (_DEBUG_IWEIGHTEDBOOLSUM) {
    ...
    if (active_min != bound_[0]) {
      std::cout << "(1) mismatch on lower bound: active = " << active_min
                << ", incremental = " << bound_[0] << std::endl;
      exit(1);
    }

    if (active_max != bound_[1]) {
      std::cout << "(1) mismatch on upper bound: active = " << active_max
                << ", incremental = " << bound_[1] << std::endl;
      exit(1);
    }

    if (_DEBUG_IWEIGHTEDBOOLSUM) {
      std::cout << "\ntarget bounds: [" << lower_bound << ".." << upper_bound
                << "]" << std::endl;
    }
  }
#endif
      ...
      if (i >= 0) {
        ...
        if (_DEBUG_IWEIGHTEDBOOLSUM) {
          std::cout << "\nnext index: " << i << ": " << scope[i] << " in "
                    << scope[i].get_domain() << " * " << weight[i] << std::endl;
        }
#endif
...
int Mistral::ConstraintIncrementalWeightedBoolSumInterval::check(
    const int *s) const {
...
std::ostream &Mistral::ConstraintIncrementalWeightedBoolSumInterval::display(
  ...
  if (upper_bound < INFTY)
    os << " <= " << upper_bound << " (" << bound_[0] << ".." << bound_[1]
       << ") " << active;
...
Mistral::PredicateWeightedBoolSum::PredicateWeightedBoolSum(
    Vector<Variable> &scp, const int o)
    : GlobalConstraint(scp) {
  priority = 1;
  offset = o;
  init_prop = true;
...
Mistral::PredicateWeightedBoolSum::PredicateWeightedBoolSum(
    Vector<Variable> &scp, Vector<int> &wgt, const int o)
    : GlobalConstraint(scp) {
  priority = 1;
  offset = o;
  init_prop = true;
...
Mistral::PredicateWeightedBoolSum::PredicateWeightedBoolSum(
    std::vector<Variable> &scp, std::vector<int> &wgt, const int o)
    : GlobalConstraint(scp) {
  priority = 1;
  offset = o;
  init_prop = true;
...
void Mistral::PredicateWeightedBoolSum::initialise_activity(double *lact,
                                                            double *vact,
                                                            double norm) {
  ...
  //   // std::cout << "bounds [" << scope[n].get_min() << " .. " <<
  //   scope[n].get_max() << "]\n";
  ...
  // std::cout << "weights: " << incr_0 << " " << incr_1 << " " << avg_weight <<
  // std::endl;
  ...
  (double)(std::abs(weight.back()));

  // std::cerr << 11111 << "  " << (scope[i]) << " " <<
  (2*scope[i].id()+(weight[i]<0)) << std::endl ;

  // std::cerr << lact << " " << lact[(2*scope[i].id()+(weight[i]<0))] <<
  std::endl;
  ...
  std::cout << scope[i] << " * " << weight[i] << ": " << ac_i << " (" <<
  skew*ac_i << "/" << (1-skew)*ac_i << ")" << std::endl;
  }
  */
}
...
#ifdef _PWBS_WC_ALT
double
Mistral::PredicateWeightedBoolSum::weight_conflict(double unit,
...
Mistral::Explanation::iterator
Mistral::PredicateWeightedBoolSum::get_reason_for(const Atom a, const int lvl,
...
// Explanation **reason = get_solver()->reason_for.stack_;
  ...
  if (_DEBUG_REASONRIWBS) {
    ...
    std::cout << "[" << bound_[0] << "," << bound_[1] << "] in "
              << scope[weight.size].get_domain() << std::endl;
  }
#endif
...
#ifdef _DEBUG_REASONRIWBS
  if (_DEBUG_REASONRIWBS) {
    std::cout << " ==> " << total_nog << " in? ["
              << scope[weight.size].get_min() << ","
              << scope[weight.size].get_max() << "]\n";
    std::cout << explanation << std::endl << std::endl;

    // if(explanation.size == (32 + (a == NULL_ATOM)) && explanation[0] == 465
    // && explanation[1] == 541)
    exit(1);
  }
#endif
...
std::ostream &
Mistral::PredicateWeightedBoolSum::display(std::ostream &os) const {
#ifdef _GET_SUM_NAME
  os << " pwbs: (" << id << ") ";
#endif
...
Mistral::PredicateElement::PredicateElement(std::vector<Variable> &scp,
                                            const int o)
    : GlobalConstraint(scp) {
  offset = o;
  priority = 1;
...
void Mistral::PredicateElement::initialise() {
  ...
  aux_dom.initialise(std::min(0, scope[n].get_min()),
                     std::max(n, scope[n].get_max()), BitSet::empt);
...
Mistral::PropagationOutcome Mistral::PredicateElement::propagate() {
  ...
  if (_DEBUG_ELEMENT) {
    std::cout << std::endl
              << std::endl
              << (get_solver()->statistics.num_propagations) << " " << id
              << " X: " << scope[0].get_domain();
    for (i = 1; i < n; ++i) {
      std::cout << " " << scope[i].get_domain();
    }
    std::cout << "[" << scope[n].get_domain() << "-" << offset
              << "] = " << scope[n + 1].get_domain() << std::endl;
  }
#endif
  ...
  while (IS_OK(wiped) && update_V) {
    ...
    while (IS_OK(wiped) && !changes.empty()) {
...
#ifdef _DEBUG_ELEMENT
      if (_DEBUG_ELEMENT) {
        std::cout << "react to " << scope[evt] << " in "
                  << scope[evt].get_domain() << std::endl;
      }
#endif
      if (evt < n && scope[n].contain(evt + offset)) {
...
#ifdef _DEBUG_ELEMENT
        if (_DEBUG_ELEMENT) {
          std::cout << "  update " << scope[n] << " in "
                    << scope[n].get_domain() << ": " << scope[n + 1] << " in "
                    << scope[n + 1].get_domain() << " inter " << scope[evt]
                    << " in " << scope[evt].get_domain() << "?" << std::endl;
        }
#endif
        update_V = true;
        ...
        if (_DEBUG_ELEMENT) {
          std::cout << "  update " << scope[n] << " in "
                    << scope[n].get_domain() << ": " << scope[n + 1] << " in "
                    << scope[n + 1].get_domain() << " inter " << std::endl;
        }
#endif
...
#ifdef _DEBUG_ELEMENT
          if (_DEBUG_ELEMENT) {
            std::cout << "  " << scope[i - offset] << " in "
                      << scope[i - offset].get_domain() << "?";
          }
#endif
    ...
    if (IS_OK(wiped) && update_V) {
      ...
      do {
        ...
        if (_DEBUG_ELEMENT) {
          std::cout << " + " << scope[i - offset] << " in "
                    << scope[i - offset].get_domain() << ": " << aux_dom
                    << std::endl;
        }
#endif
      } while (i < nxt);
...
#ifdef _DEBUG_ELEMENT
      if (_DEBUG_ELEMENT) {
        std::cout << "     " << scope[n + 1] << " in "
                  << scope[n + 1].get_domain() << " = " << aux_dom << std::endl;
      }
#endif
...
#ifdef _ELT_WC
double Mistral::PredicateElement::weight_conflict(double unit,
                                                  Vector<double> &weights) {
  ...
  for (int i = 0; i < n; ++i) {
    std::cout << " " << scope[i] << " in " << scope[i].get_domain()
              << (scope[n].contain(i + offset) ? "*" : " ") << std::endl;
  }
  std::cout << ")[" << scope[n].get_domain()
            << "] = " << scope[n + 1].get_domain() << "\n";
#endif
...
std::ostream &Mistral::PredicateElement::display(std::ostream &os) const {
  ...
  os << ")[" << scope.back(2) /*.get_var()*/
     << "] == " << scope.back() /*.get_var()*/;
  return os;
}

Mistral::ConstraintCliqueNotEqual::ConstraintCliqueNotEqual(
    Vector<Variable> &scp, const int except)
    : GlobalConstraint(scp), exception(except) {
  priority = 2;
}
...
Mistral::PropagationOutcome Mistral::ConstraintCliqueNotEqual::propagate() {
  ...
  for (i = 0; i < changes.size; ++i) {
    ...
    if (value != exception) {
#ifdef _DEBUG_CLIQUENOTEQUAL
      std::cout << "react to " << scope[changes[i]] << "=" << value
                << std::endl;
      std::cout << "  check pairs of recently assigned vars: " << std::endl;
#endif
      for (j = i + 1; j < changes.size; ++j) {
#ifdef _DEBUG_CLIQUENOTEQUAL
        std::cout << "    " << scope[changes[j]] << "="
                  << scope[changes[j]].get_min() << " ";
#endif
      ...
      for (j = n; j; --j) {
        ...
        if (active_var != changes[i]) {
          ...
          if (evt == FAIL_EVENT) {
            ...
            std::cout << "OK! " << scope[active_var].get_domain() << std::endl;
#endif
        }
      }
    }
...
#ifdef _DEBUG_CLIQUENOTEQUAL
        std::cout << "rreact to " << scope[active[i]] << "=" << value
                  << std::endl;
        std::cout << "  check pairs of recently assigned vars: " << std::endl;
#endif
        for (j = i + 1; j < n; ++j) {
...
std::ostream &
Mistral::ConstraintCliqueNotEqual::display(std::ostream &os) const {
  os << "=/=(" << scope[0] /*.get_var()*/;
  for (unsigned int i = 1; i < scope.size; ++i)
    os << ", " << scope[i] /*.get_var()*/;
...
#ifdef _CNE_WC
double
Mistral::ConstraintCliqueNotEqual::weight_conflict(double unit,
                                                   Vector<double> &weights) {
  // std::cout << std::endl << "WEIGHT AFTER FAILURE ON CLIQUENOTEQUAL " <<
  // get_solver()->prev_wiped_idx << " <> " << culprit << "\n";
  // std::cout << scope[get_solver()->prev_wiped_idx] << " in " <<
  // scope[get_solver()->prev_wiped_idx].get_domain()
  // 	    << " == " << scope[culprit] << " in " << scope[culprit].get_domain()
  // << std::endl;
...
bool Mistral::ConstraintMultiAtMostSeqCard::greedy_assign(int *w,
  ...
  for (k = 0; k < _k; ++k) {
    ...
    for (i = 0; i < _q[k] && i < arity; ++i) {
      ...
      if (cardinality[k][i] > max_cardinality[k])
        max_cardinality[k] = cardinality[k][i];
    }
  }
  ...
  for (i = 0; i < arity; ++i) {
    ...
    if (_DEBUG_AMSC) {
      ...
      for (k = 0; k < _k; ++k) {
        ...
        for (j = 0; j < _q[k]; ++j)
          _card[k][(j * arity + i)] =
              cardinality[k][(i + j) % _q[k]] + cumulated[i + 1];
    ...
    for (k = 0; k < _k; ++k) {
      ...
      if (o_c[k] != n_c[k]) {
        ...
        // 	    << id << " n_c[k]=" << n_c[k] << " / [" << (-arity) << ","
        // << _p[k] << "]" << std::endl << std::endl;
        // }

        ++occurrences[k][n_c[k]];
        if ((n_c[k] + cumulated[i + 1]) > max_cardinality[k])
          ++max_cardinality[k];
        if (!(--occurrences[k][o_c[k]]) &&
            (o_c[k] + cumulated[i + 1]) == max_cardinality[k])
          --max_cardinality[k];
      }
    }
  }
  ...
  if (count < _d && greedy_assign(wl, lcumulated, scope)) {
    ...
    if (lcumulated[arity] + count < _d) {
      ...
      for (int i = 0; // IS_OK(wiped) &&
           i < arity; ++i) {
        if (!scope[i].is_ground() &&
            lcumulated[i] + rcumulated[arity - i - 1] + count < _d) {
        ...
        if (!scope[i].is_ground() &&
            lcumulated[i + 1] + rcumulated[arity - i] + count <= _d) {
      ...
      if (_DEBUG_AMSC) {
        if (pruning) {
          std::cerr << "pruning " << _d - count << " " << _p << "/" << _q
                    << std::endl;
          for (int i = 0; i < arity; ++i) {
            if (scope[i].is_ground())
              std::cerr << scope[i].get_min();
...
Mistral::Explanation::iterator
Mistral::ConstraintMultiAtMostSeqCard::get_reason_for(const Atom a,
                                                      const int lvl,
                                                      iterator &end) {
  ...
  if (a != NULL_ATOM) {
    ...
    idx = arity;
    arity = scope.size;
    // l_pruning represents the last index to consider in the disentailement
    // rules
    l_pruning = idx - 1;
    ...
    std::cout << "l_pruning = " << l_pruning
              << "because x_i =" << scope[idx].get_value() << std::endl;
    std::cout << "r_pruning = " << r_pruning
              << "because x_i =" << scope[idx].get_value() << std::endl;
    ...
    for (int i = 0; i < last_r; i++)
      if (TMP[i] != max_equal_to_p[i]) {
        std::cout << "\n \n \n \n Difference at index " << i
                  << " ; set_max_equal_to_p_at_rank max_equal_to_p  \n";
        for (int i = 0; i < last_r; i++)
          std::cout << ' ' << TMP[i];
...
// std::cout << " " << i << " " << SIGN(literal__);
    ...
    std::cout << "intersection from index :" << last_l << " to " << last_r - 1
              << std::endl;
#endif
...
//	std::cout << "Prepare Intersection from " << last_l<< "TO  " << last_r-1
//<<  std::endl;

#ifdef _DEBUG_AMSC_NOGOOD
    std::cout << "intersection size :" << left_right_intersection.size
              << std::endl;
#endif
...
// std::cout << " " << INVERSE(arity,i) << " " << SIGN(literal__);
    ...
    //	std::cout << "Right from " <<INVERSE(arity,0) << "TO  " <<
    //INVERSE(arity, (last_r-1)) <<  std::endl;
    ...
    if (last_r > arity)
      last_r = arity;
#ifdef _DEBUG_AMSC_NOGOOD
    std::cout << "AGAIN \n last_l : " << last_l << "because q =" << _q[0]
              << std::endl;
    std::cout << "last_r : " << last_r << "because q =" << _q[0] << std::endl;
    std::cout << "intersection size :" << left_right_intersection.size
              << std::endl;
    std::cout << "(should be equal to intersection size )last_r - last_l +1= "
              << last_r - last_l << std::endl;
#endif
    ...
    for (int i = last_l; i < last_r; ++i) {
      ...
      if (scope[i].is_ground()) {
        tmp_idx = scope[i].id();
        if (rank[tmp_idx] < a_rank) {
          if (left_right_intersection[i - last_l] ==
              max_equal_to_p[INVERSE(arity, i)]) {
...
// std::cout << " " << i << " " << SIGN(literal__);
    ...
    //	std::cout << "Final Intersection from " << last_l<< "TO  " << last_r-1
    //<<  std::endl;
    //	std::cout <<  std::endl;
    //	std::cout <<  std::endl;
...
// std::cout << " " << arity << " " << SIGN(literal__);
  ...
  for (int i = 0; i < explanation.size; i++) {
    ...
    if (j == 0)
      if (naive_explanation[0] != explanation[i]) {
        std::cout << "Error, literal " << explanation[i]
                  << "not contained in the naive explanation \n"
                  << std::endl;
        exit(1);
      }
  }
  ...
  //   ((sol->statistics.avg_amsc_expl_size *
  //   sol->statistics.num_amsc_explanations) + explanation.size)/
  //   (++sol->statistics.num_amsc_explanations);

  // std::cout << " + " << (explanation.size) << " : " <<
  // sol->statistics.num_amsc_explanations << " x " <<
  // sol->statistics.avg_amsc_expl_size << std::endl;
...
void Mistral::ConstraintMultiAtMostSeqCard::set_max_equal_to_p_at_rank(
    int __rank, int __size, Vector<Variable> &X) {
  //__size=4;
  if (__size > (int)(X.size))
    __size = X.size;
...
// int sizeOfX = X.size;
  ...
  std::cout
      << "The sequence of variables at the moment of calling the conflict : \n";
  for (int i = 0; i < __size; ++i)
    if (X[i].is_ground())
  ...
  std::cout << "\n A snapshot of the sequence at rank : " << __rank
            << std::endl;
  ...
  std::cout << "\n The initialization of greedy on the snapshot : " << __rank
            << std::endl;
  for (int i = 0; i < __size; ++i)
...
//	}
    ...
    card[i % _q[0]] = card[(i + _q[0] - 1) % _q[0]] - sequence_image[i] +
                      sequence_image[i + _q[0]];
  }
  ...
  for (int i = 0; i < sequence_image.size; ++i)
    std::cout << "sequence_image" << sequence_image[i] << std::endl;
#endif
}
void Mistral::ConstraintMultiAtMostSeqCard::greedy_assign_for_explanation(
    Vector<Variable> &X, int __size, int __rank) {
  ...
  for (i = 0; i < arity; ++i) {
    ...
    if (_DEBUG_AMSC) {
      ...
      for (k = 0; k < _k; ++k) {
        ...
        for (j = 0; j < _q[k]; ++j)
          _card[k][(j * arity + i)] =
              cardinality[k][(i + j) % _q[k]] + lcumulated[i + 1];
        for (j = 0; j <= arity + _p[k]; ++j) {
    ...
    for (k = 0; k < _k; ++k) {
      ...
      if (o_c[k] != n_c[k]) {
        ...
        ++occurrences[k][n_c[k]];
        if ((n_c[k] + lcumulated[i + 1]) > max_cardinality[k])
          ++max_cardinality[k];
        if (!(--occurrences[k][o_c[k]]) &&
            (o_c[k] + lcumulated[i + 1]) == max_cardinality[k])
          --max_cardinality[k];
      }
    }
  }
...
// Mistral::Explanation::iterator
// Mistral::ConstraintMultiAtMostSeqCard::get_reason_for(const Atom a, const int
// lvl, Explanation::iterator& end) {
//   explanation.clear();
//   int i=scope.size;
...
std::ostream &
Mistral::ConstraintMultiAtMostSeqCard::display(std::ostream &os) const {
  os << "(" << scope[0] /*.get_var()*/;
  for (unsigned int i = 1; i < scope.size; ++i)
    os << " + " << scope[i] /*.get_var()*/;
...
Mistral::ConstraintStretch::ConstraintStretch() : GlobalConstraint() {}

Mistral::ConstraintStretch::ConstraintStretch(Vector<Variable> &scp,
                                              std::vector<int> &type,
                                              std::vector<int> &lb,
                                              std::vector<int> &ub,
...
Mistral::ConstraintStretch::ConstraintStretch(std::vector<Variable> &scp,
                                              std::vector<int> &type,
                                              std::vector<int> &lb,
                                              std::vector<int> &ub,
                                              std::vector<int> &transitions)
...
void Mistral::ConstraintStretch::init_struct(std::vector<int> &type,
                                             std::vector<int> &lb,
                                             std::vector<int> &ub,
                                             std::vector<int> &T) {
...
{
  for (int i = 0; i < 2; ++i)
    std::fill(backward[scope.size - i], backward[scope.size - i] + stype.size(),
              i);
...
Mistral::PropagationOutcome Mistral::ConstraintStretch::prune() {
  ...
  for (int j = 0; IS_OK(wiped) && j < (int)(stype.size()); ++j) {
    ...
    for (int i = scope.size; i >= 0; --i) {
      ...
      if (forward[i + 1][j] - forward[i][j] == 0)
        continue;
...
int Mistral::ConstraintStretch::check( const int* s ) const 
{
  ...
  for (size_t i = 1; ok && i < scope.size; ++i) {
    if (s[i] != cur_val) {
      ...
      if (sindex[cur_val] == INFTY || slb[sindex[cur_val]] > cur_span ||
          sub[sindex[cur_val]] < cur_span)
        ok = false;
      else {
        cur_val = s[i];
        cur_span = 1;
...
void Mistral::ConstraintAllDiff::sortit() 
{
  ...
  //   assert(minsorted[i-1]->min <= minsorted[i]->min);
  //   assert(maxsorted[i-1]->max <= maxsorted[i]->max);
  // }
  ...
  for (i = j = nb = 0;;) { // merge minsorted[] and maxsorted[] into bounds[]
    if (i < (int)(scope.size) &&
        min <= max) { // make sure minsorted exhausted first
      if (min != last)
        bounds[++nb] = last = min;
  ...
  if (_DEBUG_ALLDIFF) {
    std::cout << "\nmin:";
    for (auto q{0}; q < scope.size; ++q) {
      std::cout << std::setw(5) << minsorted[q]->min;
    }
    std::cout << "\n    ";
    for (auto q{0}; q < scope.size; ++q) {
      std::cout << std::setw(5) << minsorted[q]->max;
    }
    std::cout << "\nmax:";
    for (auto q{0}; q < scope.size; ++q) {
      std::cout << std::setw(5) << maxsorted[q]->min;
    }
    std::cout << "\n    ";
    for (auto q{0}; q < scope.size; ++q) {
      std::cout << std::setw(5) << maxsorted[q]->max;
    }
    std::cout << std::endl;
  }
#endif
...
int Mistral::ConstraintAllDiff::filterlower() 
{
  ...
  if (_DEBUG_ALLDIFF) {
    ...
    std::cout << "\nbound:";
    for (auto q{0}; q <= nb + 1; ++q) {
      std::cout << std::setw(5) << bounds[q];
    }
    std::cout << "\ndiffs:";
    for (auto q{0}; q <= nb + 1; ++q) {
      std::cout << std::setw(5) << d[q];
    }
    std::cout << "\ntreel:     ";
    for (auto q{1}; q <= nb + 1; ++q) {
      std::cout << std::setw(5) << t[q];
    }
    std::cout << "\nhalll:     ";
    for (auto q{1}; q <= nb + 1; ++q) {
      std::cout << std::setw(5) << h[q];
    }
    std::cout << std::endl;
  }
#endif

  for (i = 0; i < (int)(scope.size);
       i++) { // visit Intervals in increasing max order
...
#ifdef _DEBUG_ALLDIFF
    if (_DEBUG_ALLDIFF) {
      std::cout << "process [" << maxsorted[i]->min << ".."
                << (maxsorted[i]->max + 1) << ") p=1"
                << " -> reduce difference of interval [" << bounds[j] << ","
                << bounds[z] << ")"
                << "\n";
    }
#endif
...
#ifdef _DEBUG_ALLDIFF
      if (_DEBUG_ALLDIFF) {
        std::cout << "overload on interval [" << bounds[j] << "," << bounds[y]
    ...
    if (d[z] == bounds[z] - bounds[y]) {
      ...
      std::cout << "no hall interval [" << bounds[j] << "," << bounds[y]
                << ") because " << bounds[z] << " - " << bounds[y]
                << " != " << d[z] << " \n";
    }
#endif
...
#ifdef _DEBUG_ALLDIFF
    if (_DEBUG_ALLDIFF) {
      std::cout << "process [" << minsorted[i]->min << ".."
                << (minsorted[i]->max + 1) << ") p=1"
                << " -> reduce difference of interval [" << bounds[y] << ","
                << bounds[j] << ")"
                << "\n";
    }
#endif
...
Mistral::PropagationOutcome Mistral::ConstraintAllDiff::propagate() 
{
  ...
  if ((status_lower == INCONSISTENT) || (status_upper == INCONSISTENT)) {
    ...
    while (i < scope.size) {
      ...
      if (scope[i].set_max(iv[i].max) == FAIL_EVENT) {
        expl_note = -(i + 1);
        return FAILURE(i);
      }
      i++;
...
#ifdef _ALLDIFF_WC
double Mistral::ConstraintAllDiff::weight_conflict(double unit, Vector<double>& weights)  {
  ...
  if (expl_note >= 0) {
    ...
    // TODO: this thing is not "correct" (we don't care it's a heuristic, but
    // still)
...
void Mistral::ConstraintAllDiff::print_structs() {
  ...
  std::cout << std::endl;
  for (size_t i = 0; i < scope.size; ++i) {
    std::cout << i << " [" << minsorted[i]->min << ".." << minsorted[i]->max
              << "] [" << maxsorted[i]->min << ".." << maxsorted[i]->max
              << "] \n";
  }
  // for(int i=0; i<2*scope.size+2; ++i) {
  //   std::cout << i << " bounds=" << bounds[i] << " t=" << t[i] << " d=" <<
  //   d[i] << " h=" << h[i] << std::endl;
  // }
}
...
Mistral::PropagationOutcome Mistral::PredicateVertexCover::propagate() {
  ...
  for (int i = 0; i < changes.size; ++i) {
    ...
    std::cout << " -change: " << scope[var] << " in " << scope[var].get_domain()
              << " (" << event2str(evt) << ")" << std::endl;
  }
...
std::ostream& Mistral::PredicateVertexCover::display(std::ostream& os) const {
  os << scope.back() << ":" << scope.back().get_domain()
     << " == | vertex cover(G) = " << scope[0] /*.get_var()*/ << ":"
     << scope[0].get_domain();
  for (unsigned int i = 1; i < scope.size - 1; ++i) {
...
void Mistral::PredicateFootrule::initialise() {
  ...
  int dmax =
      std::min(scope[scope.size - 1].get_max(), uncorrelated_distance * 2);
  ...
  for (int i = 0; i < 2 * N; ++i) {
    domains[i].initialise(scope[i].get_min(), scope[i].get_max(),
                          scope[i].get_max() - scope[i].get_min() + 1, false);
...
int Mistral::PredicateFootrule::max_md(const int n, const int k) const {
  // return the maximum manhattan distance over k coordinates of 2
  // n-permutations
...
#ifndef _SUM_ONLY
  if (values[sol[i]] & 1 || values[sol[N + i]] & 2)
    repeat = true;
  else {
    values[sol[i]] |= 1;
    values[sol[N + i]] |= 2;
...
// void used_values
Mistral::PropagationOutcome
Mistral::PredicateFootrule::prune_from_transitions(const int i) {
  PropagationOutcome wiped = CONSISTENT;

#ifdef _DEBUG_FRDP
  if (_DEBUG_FRDP) {
    std::cout << "look for inconsistent values in " << scope[i] << " and "
              << scope[N + i] << " in " << scope[i].get_domain() << " x "
              << scope[N + i].get_domain()
              << "\n   because the only possible transitions are " << util_stack
              << std::endl;
  }
#endif
...
#endif
      if (scope[i + N].contain(abs(util_stack[j] - val)) ||
          scope[i + N].contain(abs(util_stack[j] + val))) {
#ifdef _DEBUG_FRDP
        if (_DEBUG_FRDP) {
          std::cout << " yes b/c " << scope[N + i].get_domain() << " contains "
                    << abs(util_stack[j] - val) << " or "
                    << abs(util_stack[j] + val) << std::endl;
        }
#endif
        not_supported = false;
      } else {
#ifdef _DEBUG_FRDP
        if (_DEBUG_FRDP) {
          std::cout << " no b/c " << scope[N + i].get_domain()
                    << " does not contain " << abs(util_stack[j] - val)
                    << " nor " << abs(util_stack[j] + val) << std::endl;
        }
#endif
      }
    }
...
#ifdef _DEBUG_FRDP
      if (_DEBUG_FRDP) {
        std::cout << "  => remove " << val << " from " << scope[i].get_domain()
                  << std::endl;
      }
#endif
      if (FAILED(scope[i].remove(val)))
        wiped = FAILURE(i);
    }
    vnxt = scope[i].next(val);
  ...
  if (IS_OK(wiped)) {
    ...
    do {
      ...
      for (size_t j = 0; not_supported && j < util_stack.size; ++j) {
        ...
        if (scope[i].contain(abs(util_stack[j] - val)) ||
            scope[i].contain(abs(util_stack[j] + val))) {

#ifdef _DEBUG_FRDP
          if (_DEBUG_FRDP) {
            std::cout << " yes b/c " << scope[i].get_domain() << " contains "
                      << abs(util_stack[j] - val) << " or "
                      << abs(util_stack[j] + val) << std::endl;
          }
#endif
...
#ifdef _DEBUG_FRDP
          if (_DEBUG_FRDP) {
            std::cout << " no b/c " << scope[i].get_domain()
                      << " does not contain " << abs(util_stack[j] - val)
                      << " nor " << abs(util_stack[j] + val) << std::endl;
          }
#endif
...
#ifdef _DEBUG_FRDP
        if (_DEBUG_FRDP) {
          std::cout << "  => remove " << val << " from "
                    << scope[N + i].get_domain() << std::endl;
        }
#endif

        if (FAILED(scope[N + i].remove(val)))
          wiped = FAILURE(N + i);
      }
...
Mistral::PropagationOutcome
Mistral::PredicateFootrule::compute_DP_from_scratch() {
  PropagationOutcome wiped = CONSISTENT;
...
#ifdef _DEBUG_FRDP
  if (_DEBUG_FRDP) {
    std::cout << get_solver()->statistics.num_propagations
              << " compute DP from scratch on:\n";
    for (int i = 0; i < N; ++i) {
    ...
    std::cout << std::endl
              << "compute final state from the domain of " << scope.back()
              << ": " << scope.back().get_domain() << std::endl;
  }
#endif
          ...
          if (i < N - 1) {
            ...
            if (idx < (int)(state[N].size) && idx >= ns) {
              ...
              if (_DEBUG_FRDP) {
                std::cout << "  ¬" << d << " (" << state[i][s] << "+"
                          << distance[i][t] << " & |" << d << "-"
                          << uncorrelated_distance << "| in "
                          << scope.back().get_domain() << ")\n";
              }
#endif
            }
          }
    ...
    for (size_t s = 0; s < state[i].size; ++s) {
      for (size_t t = 0; t < distance[i - 1].size; ++t) {
        ...
        if (idx < (int)(state[i - 1].size)) {
          if (!util_stack.contain(distance[i - 1][t]))
            util_stack.add(distance[i - 1][t]);
...
void Mistral::PredicateFootrule::print_automaton() const {
  for (int i = 0; i < N; ++i) {
    std::cout << "s" << std::left << std::setw(2) << (i + 1) << " "
              << std::setw(30) << state[i].to_str() << " | "
...
std::ostream& Mistral::PredicateFootrule::display(std::ostream& os) const {
  os << scope.back() << ":" << scope.back().get_domain() << " == |Manhattan(["
     << scope[0] /*.get_var()*/ << ":" << scope[0].get_domain();
  for (int i = 1; i < N; ++i) {
    os << ", " << scope[i] /*.get_var()*/ << ":" << scope[i].get_domain();
  }
  os << "] vs [" << scope[N] /*.get_var()*/ << ":" << scope[N].get_domain();
  for (int i = 1; i < N; ++i) {
    os << ", " << scope[i + N] /*.get_var()*/ << ":"
       << scope[i + N].get_domain();
  }
...
void Mistral::PredicateMin::react_to(PropagationOutcome& wiped, const int changed_idx, const Event evt) {
...
#ifdef _DEBUG_MIN
  if (_DEBUG_MIN) {
    std::cout << "react to " << event2str(evt) << " on " << scope[changed_idx]
              << " in " << scope[changed_idx].get_domain() << std::endl;
  }
#endif
  ...
  if (changed_idx < n) {
    ...
    if (candidates.contain(changed_idx)) {
#ifdef _DEBUG_MIN
      if (_DEBUG_MIN) {
        std::cout << "  " << scope[changed_idx] << " in "
                  << scope[changed_idx].get_domain() << " is a candidate"
                  << std::endl;
      }
#endif
...
#ifdef _DEBUG_MIN
        if (_DEBUG_MIN) {
          std::cout << "    => the only one! (enforce equality) (1)"
                    << std::endl;
          for (i = 0; i < n; ++i) {
            if (i != candidates.back() &&
                scope[i].get_min() <= scope[n].get_max()) {
        ...
        if (LB_CHANGED(evt)) {
          ...
          if (scope[n].get_max() < scope[changed_idx].get_min()) {
#ifdef _DEBUG_MIN
            if (_DEBUG_MIN) {
              std::cout << "    => update " << scope[changed_idx]
                        << "'s possibility" << std::endl;
            }
#endif
            ...
            if (_DEBUG_MIN) {
              ...
              for (i = 0; i < n; ++i) {
                // if(candidates.contain(i) && scope[i].get_min() >
                // scope[n].get_max()) {
                //   std::cout << "ERROR CANDIDATES (2)!\n";
                //   exit(1);
                // }
                if (!candidates.contain(i) &&
                    scope[i].get_min() <= scope[n].get_max()) {
                  std::cout << "ERROR CANDIDATES (2)!\n";
                  exit(1);
                }
          ...
          if (scope[n].get_min() < scope[changed_idx].get_min()) {
#ifdef _DEBUG_MIN
            if (_DEBUG_MIN) {
              std::cout << "    => " << scope[changed_idx]
                        << " was (maybe) the witness for " << scope[n]
                        << "'s min" << std::endl;
            }
#endif
            ...
            if (_DEBUG_MIN) {
              for (i = 1; i < n; ++i) {
                if (scope[i].get_min() < real_min)
                  real_min = scope[i].get_min();
              }
            }
#endif
      ...
      if (_DEBUG_MIN) {
        std::cout << "  " << scope[changed_idx] << " in "
                  << scope[changed_idx].get_domain() << " is not a candidate"
                  << std::endl;
      }
    }
#endif
    ...
    if (_DEBUG_MIN) {
      std::cout << "  " << scope[changed_idx] << " in "
                << scope[changed_idx].get_domain() << " is the MIN"
                << std::endl;
    }
#endif
    if (candidates.size == 1) { // equality
#ifdef _DEBUG_MIN
      if (_DEBUG_MIN) {
        std::cout << "    => only one candidate (enforce equality) (2)"
                  << std::endl;
        for (i = 0; i < n; ++i) {
          // if(candidates.contain(i) && scope[i].get_min() >
          // scope[n].get_max()) {
      ...
      if (_DEBUG_MIN) {
        if (IS_OK(wiped)) {
          std::cout << "    => " << scope[candidates.back()] << " in "
                    << scope[candidates.back()].get_domain() << std::endl;
        } else {
          std::cout << "    => fail!" << std::endl;
        }
      }
      ...
      if (LB_CHANGED(evt)) {
        ...
        for (i = candidates.size; IS_OK(wiped) && i;) {
          ...
          if (_DEBUG_MIN) {
            if (IS_OK(wiped)) {
              std::cout << "    => " << scope[var] << " in "
                        << scope[var].get_domain() << std::endl;
            } else {
              std::cout << "    => fail!" << std::endl;
            }
          }
...
#ifdef _DEBUG_MIN
            if (_DEBUG_MIN) {
              std::cout << "      => remove " << scope[var] << " in "
                        << scope[var].get_domain() << " ("
                        << scope[var].get_min() << " > " << ub << ")"
                        << std::endl;
            }
#endif
...
#ifdef _DEBUG_MIN
    if (_DEBUG_MIN) {
      std::cout << "  => only one candidate! (enforce equality) (3)"
                << std::endl;
      for (i = 0; i < n; ++i) {
...
// Mistral::PropagationOutcome Mistral::PredicateMin::propagate(const int
// changed_idx, const Event evt) {
//   PropagationOutcome wiped = CONSISTENT;
//   react_to(wiped, changed_idx, evt);
//   return wiped;
// }

// 'candidates' stores the indices of the variables that could possibly take the
// minimum value
...
Mistral::PropagationOutcome Mistral::PredicateMin::propagate() {
  ...
  while (!fix_point && !is_failed) {
    ...
    if (alone >= 0) {
      if (!is_failed) {
        if (lb_x[alone] > ub_min || ub_x[alone] < lb_min)
          is_failed = true;
...
std::ostream& Mistral::PredicateMin::display(std::ostream& os) const {
  os << scope.back() << ":" << scope.back().get_domain() << " == min("
     << scope[0] /*.get_var()*/ << ":" << scope[0].get_domain();
  for (unsigned int i = 1; i < scope.size - 1; ++i) {
    os << ", " << scope[i] /*.get_var()*/ << ":" << scope[i].get_domain();
  }
...
void Mistral::PredicateMax::react_to(PropagationOutcome& wiped, const int changed_idx, const Event evt) {
  ...
  if (changed_idx < n) {
    ...
    if (_DEBUG_MAX) {
      std::cout << "  " << scope[changed_idx] << " in "
                << scope[changed_idx].get_domain() << " is the MAX"
                << std::endl;
    }
#endif
    if (candidates.size == 1) { // equality
#ifdef _DEBUG_MAX
      if (_DEBUG_MAX) {
        std::cout << "    => only one candidate (enforce equality)"
                  << std::endl;
      }
#endif
      FILTER3(candidates.back(), set_domain(scope[n]));
...
Mistral::PropagationOutcome Mistral::PredicateMax::propagate() {
  ...
  while (!fix_point && IS_OK(wiped)) {
    ...
    for (i = 0; IS_OK(wiped) && i < n; ++i) {
      ...
      if (__max_min__ > scope[i].get_min())
        __max_min__ = scope[i].get_min();
      if (__max_max__ > scope[i].get_max())
        __max_max__ = scope[i].get_max();
    }
  ...
  //   std::cout << std::endl << active << std::endl << candidates << " " <<
  //   changes << std::endl;
  //   }
  // #endif
  ...
  // 	// if the upper bound has been changed, we need to update the ub of
  ...
  // 		{
  // 		  std::cout << " => " << scope[var] << " in " <<
  // scope[var].get_domain() << " <= " << val << std::endl;
  // 		}
  // #endif
  ...
  // 		if(_DEBUG_MAX)
  // 		  {
  // 		    std::cout << "    " << scope[var] << " has changed: " <<
  // scope[var].get_domain() << std::endl;
  // 		  }
  // #endif
  // 		changes.add(var);
  // 		if((scope[var].get_max() < scope[n].get_min()) &&
  // candidates.contain(var)) {
  // 		  candidates.reversible_remove(var);
  // #ifdef _DEBUG_MAX
  // 		  if(_DEBUG_MAX)
  // 		    {
  // 		      std::cout << "    " << scope[var] << " can no longer be max" <<
  // std::endl;
  // 		    }
  // #endif
  // 		}
  ...
  // 	// if the lower bound has been changed, some active variables may become
  // unactive
  ...
  //   if(_DEBUG_MAX)
  //   {
  // 	  std::cout << " relax the constraint from vars lower than " << val <<
  // std::endl;
  //   }
  // #endif
  ...
  //   if(_DEBUG_MAX)
  //   {
  // 	      std::cout << "    relax1 " << this << " from " << scope[var] <<
  // std::endl;
  //   }
  // #endif
  ...
  //   if(_DEBUG_MAX)
  //   {
  // 	      std::cout << "    relax2 " << this << " from " << scope[evt] <<
  // std::endl;
  //   }
  // #endif
  ...
  //   if(_DEBUG_MAX)
  //   {
  // 	    std::cout << " no need to update " << scope[n] << "'s max" <<
  // std::endl;
  //   }
  // #endif
  ...
  //   if(_DEBUG_MAX)
  //   {
  //     std::cout << scope[candidates[0]] << " is the last max var " <<
  //     std::endl;
  //   }
  // #endif

  //     if(scope[candidates[0]].set_domain(scope[n]) == FAIL_EVENT) wiped =
  //     FAILURE(candidates[0]);
  //     if(scope[n].set_domain(scope[candidates[0]]) == FAIL_EVENT) wiped =
  //     FAILURE(n);
...
Mistral::Explanation::iterator
Mistral::ConstraintLeftExplanationMultiAtMostSeqCard::get_reason_for(
    const Atom a, const int lvl, iterator &end) {
...
//}
  ...
  Mistral::ConstraintSimplifiedExplanationMultiAtMostSeqCard::
      ConstraintSimplifiedExplanationMultiAtMostSeqCard(
          Vector<Variable> & scp, const int k, const int d, const int *p,
          const int *q)
      : ConstraintMultiAtMostSeqCard(scp, k, d, p, q) {}

  Mistral::Explanation::iterator
  Mistral::ConstraintSimplifiedExplanationMultiAtMostSeqCard::get_reason_for(
      const Atom a, const int lvl, iterator &end) {
    ...
    if (a != NULL_ATOM) {
      ...
      last_r = l_pruning + _q[0] - 1;
#ifdef _DEBUG_AMSC_NOGOOD
      std::cout << "last_l is the first left index to consider in the "
                   "intersection \n";
      std::cout << "last_r is the last index to consider in the intersection\n";
      std::cout << "arity " << arity << std::endl;
      std::cout << "index of a = " << idx << std::endl;
      ...
      std::cout << "last_r : " << last_r << "because q =" << _q[0] << std::endl;
#endif
      // the total length to consider in the set_max_equal_to_p_at_rank is
      // last_r + q
      // increment last_r!
      last_r++;
      if (last_r > arity)
      ...
      //		std::cout << "last considered " << last_l-1 <<
      //std::endl;
      //	left_right_intersection.clear();
...
//	if (last_r>arity) last_r= arity;
      ...
      if (rank[tmp_idx] < a_rank)
      {
      if (left_right_intersection[i-last_l] == max_equal_to_p[INVERSE(arity,
      i)])
      {
      if (max_equal_to_p[INVERSE(arity,i)] == scope[i].get_value())
      {
...
ConstraintOccurrences::ConstraintOccurrences(
    Vector<Variable> & scp, const int firstDomainValue,
    const int lastDomainValue, const int *minOccurrences,
...
* called as: filterLowerMin(t, d, h, stableInterval, potentialStableSets,
* newMin);
*/
int ConstraintOccurrences::filterLowerMin(
    int *tl, int *c, int *stableAndUnstableSets, int *stableInterval,
    int *potentialStableSets, int *newMin) {
  int i, j, w, x, y, z, v;
  int changes = 0, arity = scope.size;

  for (w = i = nb + 1; i > 0; i--) {
    // c[i] = sum(l, bounds[potentialStableSets[i]=stableInterval[i]=i-1],
    // bounds[i]-1);
  ...
  for (i = 0; i < arity; i++) { // visit intervals in increasing max order
    ...
    if (c[z] <= sum(l, bounds[y], bounds[z] - 1)) {
      ...
      if (c[z] == sum(l, bounds[y], bounds[z] - 1)) {
        if (stableAndUnstableSets[y] >
            y) // Consider stable and unstable sets beyong y
          y = stableAndUnstableSets[y]; // Equivalent to pathmax since the path
                                        // is fully compressed
        pathset(stableAndUnstableSets, stableAndUnstableSets[y], j - 1,
  ...
  // For all variables that are not a subset of a stable set, shrink the lower
  // bound
  for (i = arity - 1; i >= 0; i--) {
    x = maxsorted[i]->minrank;
    y = maxsorted[i]->maxrank;
...
*/
int ConstraintOccurrences::filterUpperMin(int *tl, int *c,
                                          int *stableAndUnstableSets,
                                          int *stableInterval, int *newMax) {
  ...
  for (i = arity; --i >= 0;) { // visit intervals in decreasing min order
    ...
    if (c[z] > sum(l, bounds[z], bounds[y] - 1)) {
      ...
      if (stableAndUnstableSets[x] < x) {
        newMax[i] = w =
            pathmin(stableAndUnstableSets, stableAndUnstableSets[x]);
        pathset(stableAndUnstableSets, x, w, w); // path compression
      } else {
        newMax[i] = x;
      }
  ...
  // For all variables that are not subsets of a stable set, shrink the lower
  // bound
  for (i = arity - 1; i >= 0; i--) {
...
//_vars.getManager().fail();
      ...
      if ((statusLowerMax == CHANGES) || (statusLowerMin == CHANGES) ||
        ...
        i = scope.size;
        while (i--)
          if (FAILED(scope[i].set_min(iv[i].min) |
                     scope[i].set_max(iv[i].max))) {
            wiped = FAILURE(i);
            return wiped;
          }
      } // if
...
// with a weight of 1.
partialSum *ConstraintOccurrences::initializePartialSum(
    const int firstValue, int count, const int *elements) {
  int i, j;
...
int ConstraintOccurrences::maxValue(partialSum * p) { return p->lastValue - 2; }
...
int ConstraintOccurrences::check( const int* s ) const 
{
  ...
  i = nval;
  while (i--) {
    if (occ[i] < (getlb(i + l->firstValue + 3)) ||
        occ[i] > (getub(i + l->firstValue + 3)))
      return 1;
  }
...
// Mistral::ConstraintPreemptiveNoOverlap::ConstraintPreemptiveNoOverlap(
...
//   max_duration = *std::max_element(duration.begin(), duration.end());

//   iv.resize(numVars());
//   minsorted.resize(numVars());
//   maxsorted.resize(numVars());
//   bounds.resize(2 * numVars() + 2, 0);

//   for (auto i{0}; i < numVars(); ++i) {
//     minsorted[i] = maxsorted[i] = &iv[i];
//     iv[i].min = iv[i].max = NOVAL;
//     iv[i].duration = duration[i];
//   }

//   tree_link.resize(2 * numVars() + 2);
//   diff.resize(2 * numVars() + 2);
//   hall_link.resize(2 * numVars() + 2);
// }

// void Mistral::ConstraintPreemptiveNoOverlap::initialise() {
//   ConstraintImplementation::initialise();
//   for (unsigned int i = 0; i < scope.size; ++i) {
//     trigger_on(_RANGE_, scope[i]);
//   }
...
// void Mistral::ConstraintPreemptiveNoOverlap::mark_domain() {
//   // for (unsigned int i = scope.size; i;) {
//   //   // get_solver()->mark_non_convex(scope[i].id());
//   //   get_solver()->forbid(scope[--i].id(), RANGE_VAR);
//   // }
...
// void Mistral::ConstraintPreemptiveNoOverlap::sortit() {
//   int i, j, nb;
//   int min, max, last;
//   int n{static_cast<int>(numVars())};

//   std::sort(minsorted.begin(), minsorted.end(),
//             [](const NO_Interval *a, const NO_Interval *b) {
//               return a->min < b->min;
//             });
//   std::sort(maxsorted.begin(), maxsorted.end(),
//             [](const NO_Interval *a, const NO_Interval *b) {
//               return a->max < b->max;
//             });

//   min = (*(minsorted.begin()))->min;
//   max = (*(maxsorted.begin()))->max + 1;
//   // MODIF: to have undominated intervals we need to be a bit more careful
//   bounds[0] = last = min - 1 - max_duration;
...
//     }
//   }
//   ConstraintPreemptiveNoOverlap::nb = nb;
//   // MODIF: to have undominated intervals we need to be a bit more careful
...
//     }
//     std::cout << "\nmax:";
//     for (auto q{0}; q < maxsorted.size(); ++q) {
//       std::cout << std::setw(5) << maxsorted[q]->min;
//     }
//     std::cout << "\n    ";
//     for (auto q{0}; q < maxsorted.size(); ++q) {
...
// int Mistral::ConstraintPreemptiveNoOverlap::filterlower() {

//   int i, j;
//   int w, x, y, z, _changes_ = 0;

//   for (i = 1; i <= nb + 1; i++)
//     diff[i] = bounds[i] - bounds[tree_link[i] = hall_link[i] = i - 1];
...
//     std::cout << "\ndiffs:     ";
//     for (auto q{1}; q <= nb + 1; ++q) {
//       std::cout << std::setw(5) << diff[q];
//     }
//     std::cout << "\ntreel:     ";
//     for (auto q{1}; q <= nb + 1; ++q) {
//       std::cout << std::setw(5) << tree_link[q];
//     }
//     std::cout << "\nhalll:     ";
//     for (auto q{1}; q <= nb + 1; ++q) {
...
//                 << bounds[maxsorted[i]->maxrank]
//                 << "] p=" << maxsorted[i]->duration
//                 << " -> reduce difference of interval [" << bounds[j] << ","
//                 << bounds[z] << ") by " << maxsorted[i]->duration << "\n";
//     }
// #endif

//     // MODIF: the difference decreases by the duration of the task (number of
//     // unit tasks) if (--diff[z] == 0)

//     // bool multi{true};

//     // if (multi) {
//       auto load{maxsorted[i]->duration};
...
//           load -= diff[z];
//           diff[z] = 0;
//           z = pathmax(&tree_link[0], tree_link[z] = z + 1);
//           length += 1;
//         } else {
//           diff[z] -= load;
//           load = 0;
...
// //         tree_link[z = pathmax(&tree_link[0], tree_link[z] = z + 1)] = j;
// //       }
// //     }
...
//       pathset(&hall_link[0], x, w, w); // path compression
//       _changes_ = 1;
//     }
//     if (diff[z] == bounds[z] - bounds[y]) {
...
// int Mistral::ConstraintPreemptiveNoOverlap::filterupper() {
//   int i, j;
//   int w, x, y, z, _changes_ = 0;
...
//   for (i = numVars(); --i >= 0;) { // visit Intervals in decreasing min order
//     x = minsorted[i]->maxrank;
//     y = minsorted[i]->minrank;
...
//                 << bounds[minsorted[i]->maxrank]
//                 << "] p=" << minsorted[i]->duration
//                 << " -> reduce difference of interval [" << bounds[z] << ","
//                 << bounds[j] << ") by " << minsorted[i]->duration << "\n";
//     }
// #endif
...
//     auto load{minsorted[i]->duration};
...
//         load -= diff[z];
//         diff[z] = 0;
//         z = pathmin(&tree_link[0], tree_link[z] = z - 1);
//         length += 1;
//       } else {
//         diff[z] -= load;
//         load = 0;
...
//     // diff[z] -= minsorted[i]->duration;
//     // if (diff[z] <= 0)
//     //   tree_link[z = pathmin(&tree_link[0], tree_link[z] = z - 1)] = j;

//     pathset(&tree_link[0], x - 1, z, z);
//     if (diff[z] < bounds[y] - bounds[z]) {
...
//       // std::cout << "here hall_link[y]=" << hall_link[y] << ", j+1=" << (j + 1)
//       //           << ", y=" << y << "\n";
...
// Mistral::PropagationOutcome
// Mistral::ConstraintPreemptiveNoOverlap::propagate() {
...
// Mistral::PropagationOutcome
// Mistral::ConstraintPreemptiveNoOverlap::propagate_hall() {
...
// Mistral::PropagationOutcome
// Mistral::ConstraintPreemptiveNoOverlap::propagate_edge() {
...
// int Mistral::ConstraintPreemptiveNoOverlap::check(const int *s) const {
//   // int i = scope.size, j;
//   // while (--i) {
//   //   j = i;
//   //   while (j--)
...
// std::ostream &Mistral::ConstraintPreemptiveNoOverlap::display(std::ostream & os)
//     const {
...
Mistral::ConstraintPreemptiveNoOverlapHall::ConstraintPreemptiveNoOverlapHall(
    Vector<Variable> & scp, const std::vector<int> &d)
    : GlobalConstraint(scp), duration(d) {
  ...
  // max_duration = std::max(duration);
  max_duration = *std::max_element(duration.begin(), duration.end());

  // std::cout << min_duration << " " << max_duration << std::endl;
...
void Mistral::ConstraintPreemptiveNoOverlapHall::initialise() {
  ConstraintImplementation::initialise();
  for (unsigned int i = 0; i < scope.size; ++i) {
    trigger_on(_RANGE_, scope[i]);
  }
...
Mistral::ConstraintPreemptiveNoOverlapHall::
    ~ConstraintPreemptiveNoOverlapHall() {
#ifdef _DEBUG_MEMORY
  std::cout << "c delete no overlap constraint" << std::endl;
#endif
}

void Mistral::ConstraintPreemptiveNoOverlapHall::sortit() {
  int i, j, nb;
  int min, max, last;
  int n{static_cast<int>(numVars())};
  ...
  ConstraintPreemptiveNoOverlapHall::nb = nb;
  // MODIF: to have undominated intervals we need to be a bit more careful
  bounds[nb + 1] = bounds[nb] + 1 + max_duration;
...
int Mistral::ConstraintPreemptiveNoOverlapHall::filterlower() {
...
int Mistral::ConstraintPreemptiveNoOverlapHall::filterupper() {
  int i, j;
  int w, x, y, z, _changes_ = 0;
...
Mistral::PropagationOutcome
Mistral::ConstraintPreemptiveNoOverlapHall::propagate() {
...
int Mistral::ConstraintPreemptiveNoOverlapHall::check(const int *s) const {
  return 0;
}

std::ostream &Mistral::ConstraintPreemptiveNoOverlapHall::display(std::ostream &
                                                                  os) const {
  os << "NoOverlap (hall) (" << scope[0] << ".." << scope[duration.size()]
     << "(" << duration[0] << ")";
  for (unsigned int i = 1; i < duration.size(); ++i)
    os << ", " << scope[i] << ".." << scope[duration.size() + i] << "("
       << duration[i] << ")";
...
//// NOOVERLAP BASED ONN EDGE

Mistral::ConstraintPreemptiveNoOverlapEdge::ConstraintPreemptiveNoOverlapEdge(
    Vector<Variable> & scp, const std::vector<int> &d)
    : GlobalConstraint(scp), duration(d), T(d.size()) {
...
void Mistral::ConstraintPreemptiveNoOverlapEdge::initialise() {
  ConstraintImplementation::initialise();
  for (unsigned int i = 0; i < scope.size; ++i) {
    trigger_on(_RANGE_, scope[i]);
  }
...
void Mistral::ConstraintPreemptiveNoOverlapEdge::mark_domain() {
  // for (unsigned int i = scope.size; i;) {
  //   // get_solver()->mark_non_convex(scope[i].id());
  //   get_solver()->forbid(scope[--i].id(), RANGE_VAR);
  // }
...
int Mistral::ConstraintPreemptiveNoOverlapEdge::check(const int *s) const {
  return 0;
}

std::ostream &Mistral::ConstraintPreemptiveNoOverlapEdge::display(std::ostream & os)
    const {
  os << "NoOverlap (edge) (" << scope[0] << ".." << scope[duration.size()] << "("
     << duration[0] << ")";
  for (unsigned int i = 1; i < duration.size(); ++i)
    os << ", " << scope[i] << ".." << scope[duration.size() + i] << "("
       << duration[i] << ")";
