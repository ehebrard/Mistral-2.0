...
public:
  ...
  // NOT IMPLEMENTED! 4 -> check that the solution can be extended to all
  // variables
  int checked;
...
public:
  ...
  */
  ...
  //   triggers[higher_priority].empty());
  // }
...
public:
  /*!@name Parameters*/
  //@{
  /// The current level in the search tree (number of left branches from the
  /// root)
  /*! The current level in the search tree (number of left branches from the
  root) level -1: state of the domains as stated when modelling level 0: level
  at wich constraints are added (+ initial propagation) level k: after k
  decisions
  */
  // int level;
  bool search_started;
  ...
  /// The set of variables, in the initial order, that is as loaded from the
  /// model
  Vector<Variable> variables;
  ...
  // /// For each level, the list of reversible objects that changed at this
  // level,
  // /// and will need to be restored
  ...
  Constraint culprit;
  /// the constraint-index of the last wiped_out variable in the culprit
  /// constraint (-1 otherwise)
  int wiped_idx;
  /// keep the index of the variable that produced the last conflict
  int prev_wiped_idx;
  /// the overall-index of the variable responsible for the last trigger before
  /// a failure
  int wiper_idx;
  ...
  // //statistics.pseudo_boolean &= (x.is_boolean() || (objective &&
  ...
  void forbid(const int var, const int m) { domain_types[var] &= (~m); }
  void mark_non_convex(const int var) { domain_types[var] &= (~RANGE_VAR); }
  // void add(Vector<Literal>& clause);
  //@}
  ...
  //   ++level;
  // }
  /// called by reversible objects when they want to be restored when backtrack
  /// to this level
  ...
  /// called when the var'th variable of constraint cons changes (with event
  /// type evt)
  // void trigger_event(const int var, const Event evt);

  /// achieve propagation closure
  PropagationOutcome propagate(Constraint c, const bool ft = true,
                               const bool ts = false);
  PropagationOutcome checker_propagate(Constraint c, const bool ft = true,
                                       const bool ts = false);
  PropagationOutcome bound_checker_propagate(Constraint c, const bool ft = true,
                                             const bool ts = false);
  bool propagate();
  ...
  void branch_left();
  /// undo the previous branching decision and enforce the complementary
  /// constraint
  ...
  Returns an outcome (SAT/UNSAT/OPT/UNKNOWN) and undo all decisions before
  returning.
  */
  Outcome depth_first_search(Vector<Variable> &seq, BranchingHeuristic *heu,
                             RestartPolicy *pol, Goal *goal = NULL,
                             bool _restore_ = true);
  /*!
  ...
  returning.
  */
  Outcome depth_first_search(BranchingHeuristic *heu, RestartPolicy *pol,
                             Goal *goal = NULL, bool _restore_ = true);
  ...
  */
...
#ifdef _PARALLEL
  // used only with parallelization to indicate an exit because a solution is
  ...
  BranchingHeuristic *heuristic_factory(std::string var_ordering,
  ...
  // 	if(branching == "minval") {
  // 	  heu = new GenericHeuristic < GenericWeightedDVO < FailureCountManager,
  // MinDomainOverWeight >, MinValue > (this); 	} else if(branching == "maxval")
  // { 	  heu = new GenericHeuristic < GenericWeightedDVO < FailureCountManager,
  // MinDomainOverWeight >, MaxValue > (this); 	} else if(branching ==
  // "halfsplit") { 	  heu = new GenericHeuristic < GenericWeightedDVO <
  // FailureCountManager, MinDomainOverWeight >, HalfSplit > (this); 	} else
  // if(branching == "randminmax") { 	  heu = new GenericHeuristic <
  // GenericWeightedDVO < FailureCountManager, MinDomainOverWeight >, RandMinMax
  // > (this); 	} else if(branching == "minweight") { 	  heu = new
  // GenericHeuristic < GenericWeightedDVO < FailureCountManager,
  // MinDomainOverWeight >, MinWeightValue > (this);
  // 	}
  //   } else if(var_ordering == "dom/activity") {
  // 	if(branching == "minval") {
  // 	  heu = new GenericHeuristic < GenericWeightedDVO < PruningCountManager,
  // MinDomainOverWeight >, MinValue > (this); 	} else if(branching == "maxval")
  // { 	  heu = new GenericHeuristic < GenericWeightedDVO < PruningCountManager,
  // MinDomainOverWeight >, MaxValue > (this); 	} else if(branching ==
  // "halfsplit") { 	  heu = new GenericHeuristic < GenericWeightedDVO <
  // PruningCountManager, MinDomainOverWeight >, HalfSplit > (this); 	} else
  // if(branching == "randminmax") { 	  heu = new GenericHeuristic <
  // GenericWeightedDVO < PruningCountManager, MinDomainOverWeight >, RandMinMax
  // > (this); 	} else if(branching == "minweight") { 	  heu = new
  // GenericHeuristic < GenericWeightedDVO < PruningCountManager,
  // MinDomainOverWeight >, MinWeightValue > (this);
  // 	}
  //   }
  //   return heu;
...
public:
  ...
  // 	// }
  //   }
  //   std::cout << print_sol << print_mod << print_sta << print_ins <<
  //   print_par << std::endl;

  // }

public:
  SolverCmdLine(const std::string &message, const char delimiter = ' ',
  ...
  void initialise() //  {
      ...
      //   // INPUT FILE
      //   fileArg = new TCLAP::ValueArg<std::string>("f","file","instance
      //   file",true,"data/example.opb","string"); add( *fileArg );

      //   // DECAY FACTOR
      //   timeArg = new TCLAP::ValueArg<double>("t","time_limit","time
      //   limit",false,0,"double"); add( *timeArg );
      ...
      //   // printArg = new TCLAP::MultiArg<std::string>("p","print","objects
      //   to print",false,&p_allowed);
      //   // add( *printArg );

      //   // VERBOSITY LEVEL
      //   verbosityArg = new TCLAP::ValueArg<int>("v","verbosity","verbosity
      //   level",false,1,"int"); add( *verbosityArg );

      //   // RANDOM SEED
      //   seedArg = new TCLAP::ValueArg<int>("s","seed","random
      //   seed",false,12345,"int"); add( *seedArg );

      //   // HEURISTIC RANDOMIZATION
      //   randomizationArg = new
      //   TCLAP::ValueArg<int>("z","randomization","randomization
      //   level",false,0,"int"); add( *randomizationArg );

      //   // WHETHER WE USE REWRITING OPTIONS
      //   rewriteArg = new TCLAP::SwitchArg("w","rewrite","use rewriting during
      //   preprocessing", false); add( *rewriteArg );
      ...
      //   restartArg = new TCLAP::ValueArg<std::string>("r","restart","restart
      //   policy",false,"no",&r_allowed); add( *restartArg );

      //   // RESTART FACTOR
      //   factorArg = new TCLAP::ValueArg<double>("m","factor","restart
      //   factor",false,1.05,"double"); add( *factorArg );

      //   // RESTART BASE
      //   baseArg = new TCLAP::ValueArg<int>("e","base","restart
      //   base",false,200,"int"); add( *baseArg );

      //   // DECAY FACTOR
      //   decayArg = new TCLAP::ValueArg<double>("d","decay","decay
      //   factor",false,.96,"double"); add( *decayArg );

      //   // FORGETFULNESS
      //   forgetArg = new TCLAP::ValueArg<double>("g","forget","clause
      //   forgetfulness",false,.75,"double"); add( *forgetArg );

      //   // ACTIVITY INCREMENT
      //   incrementArg = new TCLAP::ValueArg<double>("i","increment","activity
      //   increment",false,.012,"double"); add( *incrementArg );

      //   // USE CLAUSE LEARNING
      //   learningArg = new TCLAP::SwitchArg("l","learning","Switch on clause
      //   learning (CDCL)", false); add( *learningArg );

      // // // PRINT SOLUTION
      // //   printsolArg = new TCLAP::SwitchArg("p", "print_sol","Print the
      // solution, if found", false);
      // //   add( *printsolArg );

      // // // PRINT MODEL
      // //  printmodArg = new TCLAP::SwitchArg("x", "print_mod","Print the
      // model", false);
      // //   add( *printmodArg );

      // // // PRINT STATISTICS
      // //  printstaArg = new TCLAP::SwitchArg("y", "print_sta","Print the
      // statistics", false);
      // //   add( *printstaArg );

      // // // PRINT INSTANCE
      // //  printinsArg = new TCLAP::SwitchArg("w", "print_ins","Print the
      // instance", false);
      // //   add( *printinsArg );

      // // // PRINT PARAMETERS
      // //  printparArg = new TCLAP::SwitchArg("q", "print_par","Print the
      // parameters", false);
      // //   add( *printparArg );
      ...
      //   // TCLAP::ValueArg<std::string> choiceArg("c","choice","variable
      //   ordering",false,"dom/activity",&vo_allowed);
      //   // add( *choiceArg );
      ...
      //   branchingArg = new
      //   TCLAP::ValueArg<std::string>("b","branching","value
      //   ordering",false,"guided",&bo_allowed); add( *branchingArg );
