...
public:
  ...
  const std::vector<int> &getJobTasks(const int i) const {
    return tasks_in_job[i];
  }
  const std::vector<int> &getMachineTasks(const int i) const {
    return tasks_in_machine[i];
  }
  ...
  int getRankInJob(const int i, const int j = -1) const {
    return get_job_rank(i, (j == -1 ? getJob(i, 0) : j));
  }
...
class SchedulingWeightedDegree
    : public GenericHeuristic<
          GenericDVO<VarComparator, RAND, FailureCountManager>, Branching> {
public:
  ...
  // { 	return GenericHeuristic< GenericDVO< VarComparator, RAND,
  // FailureCountManager >, Branching >::var;
  // }
...
public:
  ...
  C_max_Model(Instance *pb, ParameterList *pr, StatisticList *st)
...
public:
  ...
  L_sum_Model(Instance *pb, ParameterList *pr,
              StatisticList *st // , const int L_sum
              )
      : SchedulingSolver(pb, pr, st) {
...
public:
  ...
  Depth_Model(Instance &prob, ParameterList *params,
              const int Depth) { // setup(prob, params, Depth);
  }
  virtual ~Depth_Model() {}
  ...
  virtual int get_objective();
  // virtual double  get_normalized_objective() {return
  // (double)(get_objective())/(double)(disjuncts.size());}
  virtual int set_objective(const int obj);
...
public:
  ...
  No_wait_Model(Instance &prob, ParameterList *params, const int C_max,
                const int t = 1)
      : C_max_Model() {
    type = t; // setup(prob, params, C_max);
  }
