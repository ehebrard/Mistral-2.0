...
namespace Mistral {
  ...
  */  
  ...
  inline int Variable::get_value() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->get_value();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->get_value();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->get_value();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->get_value();
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::get_min() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->get_min();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->get_min();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->get_min();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->get_min();
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::get_max() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->get_max();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->get_max();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->get_max();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->get_max();
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::get_minCapacity() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->get_minCapacity();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->get_minCapacity();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->get_minCapacity();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->get_minCapacity();
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::get_maxCapacity() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->get_maxCapacity();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->get_maxCapacity();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->get_maxCapacity();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->get_maxCapacity();
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::get_minPosAbs() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->get_minPosAbs();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->get_minPosAbs();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->get_minPosAbs();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->get_minPosAbs();
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::get_minNegAbs() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->get_minNegAbs();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->get_minNegAbs();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->get_minNegAbs();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->get_minNegAbs();
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::next(const int v) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->next(v);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->next(v);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->next(v);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->next(v);
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline int Variable::prev(const int v) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->prev(v);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->prev(v);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->prev(v);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->prev(v);
    else if(variable_type ==   CONST_VAR) return (int)implementation;
  }

  inline bool Variable::isRange() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->isRange();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->isRange();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->isRange();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->isRange();
    else if(variable_type ==   CONST_VAR) return true;
  }

  inline bool Variable::isGround() const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->isGround();
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->isGround();
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->isGround();
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->isGround();
    else if(variable_type ==   CONST_VAR) return true;
  }

  inline bool Variable::equal(const int v) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->equal(v);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->equal(v);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->equal(v);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->equal(v);
    else if(variable_type ==   CONST_VAR) return ((int)implementation == v);
  }

  inline bool Variable::contain(const int v) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->contain(v);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->contain(v);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->contain(v);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->contain(v);
    else if(variable_type ==   CONST_VAR) return ((int)implementation == v);
  }

  inline bool Variable::intersect(const int lo, const int up) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->intersect(lo, up);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->intersect(lo, up);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->intersect(lo, up);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->intersect(lo, up);
    else if(variable_type ==   CONST_VAR) return ((int)implementation >= lo && (int)implementation <= up);
  }

  inline bool Variable::included(const int lo, const int up) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->included(lo, up);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->included(lo, up);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->included(lo, up);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->included(lo, up);
    else if(variable_type ==   CONST_VAR) return ((int)implementation >= lo && (int)implementation <= up);
  }

  inline bool Variable::includes(const int lo, const int up) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->includes(lo, up);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->includes(lo, up);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->includes(lo, up);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->includes(lo, up);
    else if(variable_type ==   CONST_VAR) return ((int)implementation == lo && (int)implementation == up);
  }

  inline bool Variable::intersect(const BitSet& s) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->intersect(s);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->intersect(s);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->intersect(s);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->intersect(s);
    else if(variable_type ==   CONST_VAR) return (s.member((int)implementation));
  }

  inline bool Variable::included(const BitSet& s) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->included(s);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->included(s);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->included(s);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->included(s);
    else if(variable_type ==   CONST_VAR) return (s.member((int)implementation));
  }

  inline bool Variable::includes(const BitSet& s) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->includes(s);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->includes(s);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->includes(s);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->includes(s);
    else if(variable_type ==   CONST_VAR) return (s.size() == 1 && s.member((int)implementation));
  }

  inline bool Variable::intersect(const Variable& x) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->intersect(x);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->intersect(x);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->intersect(x);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->intersect(x);
    else if(variable_type ==   CONST_VAR) return x.contain((int)implementation);
  }

  inline bool Variable::included(const Variable& x) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->included(x);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->included(x);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->included(x);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->included(x);
    else if(variable_type ==   CONST_VAR) return x.contain((int)implementation);
  }

  inline bool Variable::includes(const Variable& x) const {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->includes(x);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->includes(x);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->includes(x);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->includes(x);
    else if(variable_type ==   CONST_VAR) return (x.size() == 1 && x.contain((int)implementation));
  }

  inline void Variable::intersectTo( BitSet& s ) const {
    if     (variable_type ==  BITSET_VAR) ((VariableBitset  *)implementation)->intersectTo(s);
    else if(variable_type ==    LIST_VAR) ((VariableList    *)implementation)->intersectTo(s);
    else if(variable_type ==   RANGE_VAR) ((VariableRange   *)implementation)->intersectTo(s);
    else if(variable_type == VIRTUAL_VAR) ((VariableVirtual *)implementation)->intersectTo(s);
    else if(variable_type ==   CONST_VAR) {
  ...
  inline void Variable::unionTo( BitSet& s ) const {
    if     (variable_type ==  BITSET_VAR) ((VariableBitset  *)implementation)->unionTo(s);
    else if(variable_type ==    LIST_VAR) ((VariableList    *)implementation)->unionTo(s);
    else if(variable_type ==   RANGE_VAR) ((VariableRange   *)implementation)->unionTo(s);
    else if(variable_type == VIRTUAL_VAR) ((VariableVirtual *)implementation)->unionTo(s);
    else if(variable_type ==   CONST_VAR) s.insert((int)implementation);
  }

  inline Event Variable::remove(const int v) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->remove(v);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->remove(v);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->remove(v);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->remove(v);
    else if(variable_type ==   CONST_VAR) return ((int)implementation == v ? FAIL_EVENT : NO_EVENT);
  }

  inline Event Variable::setDomain(const int v) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->setDomain(v);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->setDomain(v);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->setDomain(v);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->setDomain(v);
    else if(variable_type ==   CONST_VAR) return ((int)implementation != v ? FAIL_EVENT : NO_EVENT);
  }

  inline Event Variable::setMin(const int lo) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->setMin(lo);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->setMin(lo);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->setMin(lo);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->setMin(lo);
    else if(variable_type ==   CONST_VAR) return ((int)implementation < lo ? FAIL_EVENT : NO_EVENT);
  }

  inline Event Variable::setMax(const int up) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->setMax(up);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->setMax(up);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->setMax(up);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->setMax(up);
    else if(variable_type ==   CONST_VAR) return ((int)implementation > up ? FAIL_EVENT : NO_EVENT);
  }

  inline Event Variable::setDomain(const BitSet& s) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->setDomain(s);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->setDomain(s);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->setDomain(s);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->setDomain(s);
    else if(variable_type ==   CONST_VAR) return (s.member((int)implementation) ? NO_EVENT : FAIL_EVENT);
  }

  inline Event Variable::setDomain(Variable& x) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->setDomain(x);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->setDomain(x);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->setDomain(x);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->setDomain(x);
    else if(variable_type ==   CONST_VAR) return (x.contain((int)implementation) ? NO_EVENT : FAIL_EVENT);
  }

  inline Event Variable::removeSet(const BitSet& s) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->removeSet(s);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->removeSet(s);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->removeSet(s);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->removeSet(s);
    else if(variable_type ==   CONST_VAR) return (s.member((int)implementation) ? FAIL_EVENT : NO_EVENT);
  }

  inline Event Variable::removeRange(const int lo, const int up) {
    if     (variable_type ==  BITSET_VAR) return ((VariableBitset  *)implementation)->removeRange(lo, up);
    else if(variable_type ==    LIST_VAR) return ((VariableList    *)implementation)->removeRange(lo, up);
    else if(variable_type ==   RANGE_VAR) return ((VariableRange   *)implementation)->removeRange(lo, up);
    else if(variable_type == VIRTUAL_VAR) return ((VariableVirtual *)implementation)->removeRange(lo, up);
    else if(variable_type ==   CONST_VAR) return (((int)implementation < lo || (int)implementation > up) ? NO_EVENT : FAIL_EVENT);
  }

  std::ostream& Variable::display(std::ostream& os) const {
    if(variable_type ==   CONST_VAR) os << (int)implementation;
