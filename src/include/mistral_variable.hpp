...
public:
  ...
  /// The linked solver
  Environment *solver;
  /// unique identifier, corresponds to its rank in the variables list of the
  /// solver
  int id;
  //@}
  ...
  virtual int
  get_solution_int_value() const; //{ return solver->last_solution_lb[id] } ;
  // get value in solution (as a string)
  virtual std::string
  get_solution_str_value() const; //{ return solver->last_solution_lb[id] } ;
  // get min value in solution (same as value if assigned)
  virtual int
  get_solution_min() const; //{ return solver->last_solution_lb[id] } ;
  // get max value in solution (same as value if assigned)
  virtual int
  get_solution_max() const; //{ return solver->last_solution_ub[id] } ;
  // used when creating the model
  bool is_initialised() const { return id != -1; }
  //@}
...
public:
  ...
  VariableBitset(const int lb, const int ub, const Vector<int> &values)
      : VariableImplementation() {
    initialise(lb, ub, values);
  };

  VariableBitset(const VariableBitset<WORD_TYPE> *x)
      : VariableImplementation() {
    if (x->is_range())
  ...
  virtual void initialise(const int lb, const int ub,
    ...
    // std::cout << "initialise domain with " << lb << ".." << ub << ": " <<
    // values << std::endl;
  ...
  std::string get_history() {
    ...
    int k = trail_.size - 1,
        n = domain.values.pos_words - domain.values.neg_words;
  ...
  void initialise_trail() {
    ...
    delta_abs =
        new WORD_TYPE *[domain.values.pos_words - domain.values.neg_words];
  ...
  void set_bound_history(const int lb, const int ub, const int level) {
    ...
    if (_DEBUG_HISTORY) {
      ...
      std::cout << "set bound history " << lb << ".." << ub << " @ " << level
                << std::endl;
    }
#endif
...
#ifdef _DEBUG_HISTORY
    if (_DEBUG_HISTORY) {
      std::cout << prev_lb << " -> " << lb << ".." << ub << " <- " << prev_ub
                << std::endl;
    }
#endif
    ...
    if (prev_lb < lb || prev_ub > ub) {
      ...
      if (i == l) {
        w = (BitSet::full << (lb & BitSet::CACHE)) &
            (BitSet::full >> (BitSet::CACHE - (ub & BitSet::CACHE)));
  ...
  // inline unsigned int get_reduction() const { return (trail_.back() ==
  // solver->level ? trail_.back(2) - domain.size : 0); }
  inline unsigned int get_reduction() const {
    return trail_.back(2) - domain.size;
  }
  ...
  /// Returns the minimum value in [1..infty] \\inter domain, min if there are
  /// none
  inline int get_min_pos() const {
    ...
    if (domain.max < 1)
      return INFTY;
    if (domain.values.fast_contain(1))
      return 1;
    return domain.values.next(1);
  }
  /// Returns the maximum value in [-infty..-1] \\inter domain, max if there are
  /// none
  ...
  /// Return the smallest value currently in the domain that is strictly greater
  /// than "v"
  inline int next(const int v) const {
    return (((domain.size != domain.max - domain.min + 1))
                ? domain.values.next(v)
                : (v < domain.min ? domain.min : (v < domain.max ? v + 1 : v)));
  }
  /// Return the smallest value currently in the domain that is strictly greater
  /// than "v"
  inline int prev(const int v) const {
    return (((domain.size != domain.max - domain.min + 1))
                ? domain.values.prev(v)
                : (v > domain.max ? domain.max : (v > domain.min ? v - 1 : v)));
  }
  ...
  inline bool intersect(const int lo, const int up) const {
    return (domain.min <= up && domain.max >= lo &&
            domain.values.intersect(lo, up));
  }
  /// Whether the domain is included in the interval [l..u]
  inline bool included(const int lo, const int up) const {
    return (domain.min >= lo && domain.max <= up);
  }
  /// Whether the domain is included in the interval [l..u]
  inline bool includes(const int lo, const int up) const {
    return (domain.min <= lo && domain.max >= up &&
            domain.values.includes(lo, up));
  }

  /// Whether the domain has a nonempty intersection with the interval [l..u]
  inline bool intersect(const Interval I) const {
    return (domain.min <= I.max && domain.max >= I.min &&
            domain.values.intersect(I.min, I.max));
  }
  /// Whether the domain is included in the interval [l..u]
  inline bool included(const Interval I) const {
    return (domain.min >= I.min && domain.max <= I.max);
  }
  /// Whether the domain is included in the interval [l..u]
  inline bool includes(const Interval I) const {
    return (domain.min <= I.min && domain.max >= I.max &&
            domain.values.includes(I.min, I.max));
  }

  /// Whether the domain has a nonempty intersection with the set s
  inline bool intersect(const BitSet &s) const {
    return domain.values.intersect(s);
  }
  /// Whether the domain is included in the set s
  inline bool included(const BitSet &s) const {
    return domain.values.included(s);
  }
  /// Whether the domain is included in the set s
  inline bool includes(const BitSet &s) const {
    return domain.values.includes(s);
  }

  /// Whether the domain has a nonempty intersection with the Variable x
  inline bool intersect(const VariableBitset *x) const {
    return x->intersect(domain.values);
  }
  /// Whether the domain is included in the Variable x
  inline bool included(const VariableBitset *x) const {
    return x->includes(domain.values);
  }
  /// Whether the domain is included in the Variable x
  inline bool includes(const VariableBitset *x) const {
    return x->included(domain.values);
  }
  ...
  void put_negation_in(BitSet &s) const {
    domain.values.negate(s);
  } // s.union_with(domain.values); }
  //@}
  ...
  inline Event remove(const int v) {
    ...
    // if(id==1) {
    // 	std::cout << event2str(removal) << " event ==> " << this << " = " <<
    // domain << std::endl;
    // }
  ...
  inline Event set_min(const int lo) {
    ...
    // if(id==1) {
    // 	std::cout << "set_min " << lo << " of " << this << " in " << domain <<
    // std::endl;
    // }
    ...
    if (lo == domain.max) {
      ...
      if (domain.values.contain(lo))
        domain.min = lo;
      else
        domain.min = domain.values.next(lo - 1);
      if (domain.size == 1)
        lower_bound |= VALUE_C;
    }

    // if(id==1) {
    // 	std::cout << event2str(lower_bound) << " event ==> " << this << " = " <<
    // domain << std::endl;
    // }
  ...
  inline Event set_max(const int up) {
    ...
    // std::cout << "x_set_max(" << up << ") " << domain << " " << domain.values
    // << std::endl;
    ...
    if (up == domain.min) {
      ...
      domain.size = domain.values.size();
      if (domain.values.contain(up))
        domain.max = up;
      else
        domain.max = domain.values.prev(up);
      if (domain.size == 1)
        upper_bound |= VALUE_C;
    }
  ...
  inline Event set_domain(const BitSet &s) {
    ...
    if (!domain.values.intersect(s))
      return FAIL_EVENT;
    if (domain.values.included(s))
      return NO_EVENT;
    ...
    domain.size = domain.values.size();
    if (!s.contain(domain.min)) {
      intersection |= LB_EVENT;
      domain.min = domain.values.next(domain.min);
    }
    if (!s.contain(domain.max)) {
      intersection |= UB_EVENT;
      domain.max = domain.values.prev(domain.max);
    }
    if (domain.min == domain.max)
      intersection |= VALUE_EVENT;
  ...
  //     /// Remove all values that do not appear in the current domain of the
  //     Variable "x" inline Event set_domain(Variable x) {
  //       if(domain.size == 1) {
  ...
  // 	    domain.size = domain.values.size();
  // 	    if(!domain.values.contain(domain.min)) { intersection |= LB_EVENT;
  // domain.min = domain.values.next(domain.min); }
  // 	    if(!domain.values.contain(domain.max)) { intersection |= UB_EVENT;
  // domain.max = domain.values.prev(domain.max); } 	    if(domain.min ==
  // domain.max) intersection |= VALUE_EVENT;
  ...
  inline Event remove_set(const BitSet &s) {
    ...
    // 	  domain.size = domain.values.size();
    // 	  if(s.contain(domain.min)) { setdifference |= LB_EVENT; domain.min =
    // domain.values.next(domain.min); } 	  if(s.contain(domain.max)) {
    // setdifference |= UB_EVENT; domain.max = domain.values.prev(domain.max); }
    ...
    return setdifference; // do nothing more for BOOL or CONSTANT or BITSET or
                          // RANGE
  }
  ...
  inline Event remove_interval(const int lo, const int up) {
    ...
    // std::cout << "REMOVE [" << lo << ".." << up << "] from " << domain <<
    // std::endl;
  ...
  inline void save() {
    ...
    // std::cout << "SAVE " << this << " at level " << solver->level <<
    // std::endl
    //  		<< trail_  ;
  ...
  virtual void debug_print() const {
    ...
    std::cout << "x" << id << " in " << domain << " " << trail_ << " "
              << std::endl;
...
public:
  ...
  /*!@name Constructors*/
  //@{
  VariableWord(const int lb, const int ub) { initialise(lb, ub); }
  ...
  void initialise_word(const int lb, const int ub) {
    ...
    std::fill(VariableBitset<WORD_TYPE>::domain.values.table + nw,
              VariableBitset<WORD_TYPE>::domain.values.table + pw, 0);
  }
...
public:
  ...
  VariableList(const int lb, const int ub, const Vector<int> &values)
      : VariableImplementation() {
    initialise(values);
  };
  ...
  virtual void initialise(const Vector<int> &values) {
    if (values.size) {
      ...
      for (unsigned int i = 1; i < values.size; ++i) {
        if (_initial_min > values[i])
          _initial_min = values[i];
        if (_initial_max < values[i])
          _initial_max = values[i];
      }
  ...
  /// Returns the minimum value in [1..infty] \\inter domain, min if there are
  /// none
  inline int get_min_pos() const { /*TODO*/
    exit(1);
  }
  /// Returns the maximum value in [-infty..-1] \\inter domain, max if there are
  /// none
  inline int get_max_neg() const { /*TODO*/
    exit(1);
  }
  ...
  /// Whether or not the Variable is currently an interval
  inline bool is_range() const {
    return (domain.back() - domain.head() + 1) == (int)(domain.size);
  }
  /// Whether or not the Variable is bound to a ground value
  inline bool is_ground() const { return domain.size == 1; }
  /// Whether or not the Variable is bound to a given ground value
  inline bool equal(const int v) const {
    return domain.size == 1 && domain.back() == v;
  }
  /// Whether the value "v" is currently contained in the domain
  inline bool contain(const int v) const {
    return (v <= _initial_max && v >= _initial_min && domain.contain(v));
  }

  /// Whether the domain has a nonempty intersection with the interval [l..u]
  inline bool intersect(const int lo, const int up) const { /*TODO*/
    exit(1);
  } // return (min <= up && max >= lo); }
  /// Whether the domain is included in the interval [l..u]
  inline bool included(const int lo, const int up) const { /*TODO*/
    exit(1);
  } // return (min >= lo && max <= up); }
  /// Whether the domain is included in the interval [l..u]
  inline bool includes(const int lo, const int up) const { /*TODO*/
    exit(1);
  } // return (min <= lo && max >= up); }

  /// Whether the domain has a nonempty intersection with the interval [l..u]
  inline bool intersect(const Interval I) const { /*TODO*/
    exit(1);
  } // return (min <= up && max >= lo); }
  /// Whether the domain is included in the interval [l..u]
  inline bool included(const Interval I) const { /*TODO*/
    exit(1);
  } // return (min >= lo && max <= up); }
  /// Whether the domain is included in the interval [l..u]
  inline bool includes(const Interval I) const { /*TODO*/
    exit(1);
  } // return (min <= lo && max >= up); }

  /// Whether the domain has a nonempty intersection with the set s
  // inline bool intersect(const BitSet& s) const { return (min <= s.max() &&
  // max >= s.min()); }
  inline bool intersect(const BitSet &s) const { /*TODO*/
    exit(1);
  } // return s.intersect(min, max); }
  /// Whether the domain is included in the set s
  inline bool included(const BitSet &s) const { /*TODO*/
    exit(1);
  } // return s.includes(min, max); }
  /// Whether the domain is included in the set s
  inline bool includes(const BitSet &s) const { /*TODO*/
    exit(1);
  } // return s.included(min, max); }

  /// Intersect its domain with a set s
  inline void intersect_to(BitSet &s) const { /*TODO*/
    exit(1);
  } // s.set_min(min); s.set_max(max); }
  /// Do the union of its domain with a set s
  inline void union_to(BitSet &s) const { /*TODO*/
    exit(1);
  } // s.fill(min,max);}
  /// Do the union of the negation of its domain with a set s
  void put_negation_in(BitSet &s) const { /*TODO*/
    exit(1);
  } // s.fill(-max,-min);}
  //@}
  ...
  /// Remove all values that do not appear in the current domain of the Variable
  /// "x"
  Event set_domain(Variable &x) {
    exit(1);
    // TODO
...
public:
  ...
  std::string get_history() {
    ...
    while (k > 0) {
      buf << " " << trail_[k] << ":";
      if (trail_[k - 1] == trail_[k - 2])
        buf << trail_[k - 2];
      else
        buf << "[" << trail_[k - 2] << "," << trail_[k - 1] << "]";
      k -= 3;
    }
    return buf.str();
  }
  ...
  void set_history(VariableBitmap *X) {
    ...
    display(std::cout);
    std::cout << " in [" << min << "," << max << "] at level " << solver->level
              << std::endl;
// std::cout << "current trail: " << trail_ << std::endl;
  ...
  /// Returns the magnitude of the pruning since last level
  // inline unsigned int get_reduction() const { return (trail_.back() ==
  // solver->level ? trail_.back(2) - trail_.back(3) - max + min : 0); }
  inline unsigned int get_reduction() const {
    return trail_.back(2) - trail_.back(3) - max + min;
  }
  /// Returns the first value in the domain
  ...
  /// than "v"
  inline int next(const int v) const {
    return (v < max ? (v >= min ? v + 1 : min) : v);
  }
  /// Return the smallest value currently in the domain that is strictly greater
  /// than "v"
  inline int prev(const int v) const {
    return (v > min ? (v <= max ? v - 1 : max) : v);
  }
  ...
  /// Intersect its domain with a set s
  inline void intersect_to(BitSet &s) const {
    s.set_min(min);
    s.set_max(max);
  }
  /// Do the union of its domain with a set s
  ...
  /// Do the union of the negation of its domain with a set s
  void put_negation_in(BitSet &s) const { s.fill(-max, -min); }
  //@}
...
public:
  ...
  Variable(const int lo, const int up, const Vector<int> &values,
           const int type = EXPRESSION);
  Variable(Variable X, bool h);
  Variable(const Variable &X);
  // Variable(const int lo, const int up, BitSet& values, const int
  // type=EXPRESSION);

  ~Variable() {
  } // std::cout << "del "; this->display(std::cout); std::cout << std::endl; }
  ...
  bool is_void() const {
    return (domain_type != CONST_VAR && variable == NULL);
  }
  ...
  void initialise_domain(const int lo, const int up, const Vector<int> &values,
                         const int type);
  ...
  bool is_initialised() const {
    return domain_type != CONST_VAR && variable->is_initialised();
  }
  bool
  is_set_var(); //{ return domain_type == EXPRESSION && expression->is_set(); }
  bool same_as(Variable &x) const {
    return domain_type == x.domain_type && variable == x.variable;
  }
  ...
  inline int id() const {
    return (domain_type == CONST_VAR ? -1 : variable->id);
  }
  // inline
  Solver *get_solver(); //  {
  ...
  /// Returns the minimum absolute value in [0..infty] \\inter domain, NOVAL if
  /// there are none
  int get_min_pos() const;
  /// Returns the minimum absolute value in [-infty..0] \\inter domain, NOVAL if
  /// there are none
...
public:
  ...
  inline int get_value(iterator it) {
    return (id < 0 ? (size_t)(it) / 4 : *it);
  }
  void update();
...
public:
  ...
  // all values between domain->size and delta_ptr (not included) are in the
  // delta
  ReversibleNum<int> delta;

  DeltaList() { domain = NULL; }

  DeltaList(VariableList *x) { initialise(x); }
...
public:
  ...
  DeltaBool() { domain = NULL; }

  DeltaBool(int *b, Environment *s) { initialise(b, s); }
...
public:
  ...
  DeltaRange(VariableRange *x) { initialise(x); }
...
public:
  ...
  DeltaBitset(VariableBitmap *x) { initialise(x); }
...
public:
  ...
  */
  ...
  Decision() { init_data(ASSIGNMENT, INFTY / 2); }
  ...
  inline bool make() {
    ...
    // std::cout << var << " in " << var.get_domain() << " : " << value() <<
    // std::endl;

    // if(_data_ == -1) return propagateRelation();
    switch (type()) {
    case REMOVAL:
      return !FAILED(var.remove(value()));
    case ASSIGNMENT:
      return !FAILED(var.set_domain(value()));
    case LOWERBOUND:
...
public:
  ...
  PrecedenceExpression(Variable X, const int of = 0, const int sp = 1);
...
public:
  ...
  DisjunctiveExpression(const Variable X, const Variable Y, const int p0 = 1,
                        const int p1 = 1);
...
public:
  ...
  ReifiedDisjunctiveExpression(Variable X, Variable Y, const int p0 = 1,
                               const int p1 = 1);
...
public:
  ...
  OccurrencesExpression(Vector<Variable> &args, const int first, const int last,
                        const int *lb, const int *ub, const int ct);
...
Variable Occurrences(Vector<Variable> &args, const int first, const int last,
                     const int *lb, const int *ub,
                     const int ct = BOUND_CONSISTENCY);
Variable Occurrences(Vector<Variable> &args, std::vector<int> &values,
...
//   CardinalityExpression(Vector< Variable >& args, const int d, const int p,
//   const int q); CardinalityExpression(Vector< Variable >& args, const int d,
//   const Vector< Tuple<2, int> >& c); virtual ~CardinalityExpression();
...
// Variable Cardinality(Vector< Variable >& args, const int d, const int p,
// const int q); Variable MultiCardinality(Vector< Variable >& args, const int
// d, const Vector< Tuple<2, int> >& c);
...
public:
  ...
  AtMostSeqCardExpression(Vector<Variable> &args, const int d, const int p,
                          const int q);
  AtMostSeqCardExpression(Vector<Variable> &args, const int d,
                          const Vector<Tuple<2, int>> &c);
...
Variable AtMostSeqCard(Vector<Variable> &args, const int d, const int p,
                       const int q);
Variable MultiAtMostSeqCard(Vector<Variable> &args, const int d,
                            const Vector<Tuple<2, int>> &c);
Variable AtMostSeqCardNaiveReason(Vector<Variable> &args, const int d,
                                  const int p, const int q);
Variable AtMostSeqCardSimplifiedReason(Vector<Variable> &args, const int d,
                                       const int p, const int q);
Variable AtMostSeqCardLeftExplanationReason(Vector<Variable> &args, const int d,
                                            const int p, const int q);

class AtMostSeqCardExpressionNaiveReason : public AtMostSeqCardExpression {

public:
  AtMostSeqCardExpressionNaiveReason(Vector<Variable> &args, const int d,
                                     const int p, const int q);
...
// left explanation
class AtMostSeqCardExpressionLeftExplanationReason
    : public AtMostSeqCardExpression {
...
public:
  enum AlgorithmType { GAC2001, GAC3, AC3, GAC4, Dynamic };
...
public:
  TableExpression(Vector<Variable> &args, const bool support = true,
                  const AlgorithmType ct = Dynamic);
  TableExpression(Vector<Variable> &args, Vector<const int *> &,
                  const bool support = true, const AlgorithmType ct = Dynamic);
  TableExpression(Vector<Variable> &args, Vector<const int *> *,
                  const bool support = true, const AlgorithmType ct = Dynamic);
...
Variable
Table(Vector<Variable> &args, const bool support = true,
      const TableExpression::AlgorithmType ct = TableExpression::Dynamic);
Variable
Table(Vector<Variable> &args, Vector<const int *> &, const bool support = true,
      const TableExpression::AlgorithmType ct = TableExpression::Dynamic);
Variable
Table(Vector<Variable> &args, Vector<const int *> *, const bool support = true,
      const TableExpression::AlgorithmType ct = TableExpression::Dynamic);
// Variable Table(VarArray& args, const TableExpression::AlgorithmType
// ct=TableExpression::Dynamic);
...
public:
  ...
  BoolSumExpression(Vector<Variable> &args, const Vector<int> &w, const int l,
                    const int u);
  BoolSumExpression(std::vector<Variable> &args, const std::vector<int> &w,
...
Variable BoolSum(std::vector<Variable> &args, const int l,
                 const int u = -INFTY);

// Variable BoolSum(Vector< Variable >& args, Vector< int >& w);
Variable BoolSum(Vector<Variable> &args, const Vector<int> &w, const int l,
                 const int u = -INFTY);
...
public:
  ...
  LinearExpression(Vector<Variable> &args, const int l, const int u,
                   const int o);
  LinearExpression(std::vector<Variable> &args, const int l, const int u,
                   const int o);
  LinearExpression(Vector<Variable> &args, Vector<int> &wgt, const int l,
                   const int u, const int o);
  LinearExpression(std::vector<Variable> &args, std::vector<int> &wgt,
                   const int l, const int u, const int o);
  LinearExpression(Vector<Variable> &args, std::vector<int> &wgt, const int l,
                   const int u, const int o);
...
Variable Sum(Vector<Variable> &args, const int l = -INFTY, const int u = INFTY,
             const int o = 0);
Variable Sum(std::vector<Variable> &args, const int l = -INFTY,
             const int u = INFTY, const int o = 0);
...
Variable Sum(std::vector<Variable> &args, std::vector<int> &wgts, Variable T,
             const int o = 0);
Variable Sum(Vector<Variable> &args, Vector<int> &wgts, const int l = -INFTY,
             const int u = INFTY, const int o = 0);
Variable Sum(std::vector<Variable> &args, std::vector<int> &wgts,
             const int l = -INFTY, const int u = INFTY, const int o = 0);
Variable Sum(Vector<Variable> &args, std::vector<int> &wgts,
             const int l = -INFTY, const int u = INFTY, const int o = 0);
...
public:
  ...
  StretchExpression(const Vector<Variable> &args, std::vector<int> &stype,
                    std::vector<int> &slb, std::vector<int> &sub,
                    std::vector<int> &t);
  StretchExpression(const std::vector<Variable> &args, std::vector<int> &stype,
                    std::vector<int> &slb, std::vector<int> &sub,
                    std::vector<int> &t);
...
Variable Stretch(const Vector<Variable> &X, std::vector<int> &typ,
                 std::vector<int> &lb, std::vector<int> &ub,
                 std::vector<int> &t);
Variable Stretch(const std::vector<Variable> &X, std::vector<int> &typ,
                 std::vector<int> &lb, std::vector<int> &ub,
                 std::vector<int> &t);
Variable Stretch(const Vector<Variable> &X, std::vector<int> &typ,
                 std::vector<int> &lb, std::vector<int> &ub);
...
//   /**
//      SetExpression are constraints gluing together Boolean variables (plus a
//      cardinality integer var) to define set variables
...
//   // num_args returns the number of arguments of the constraint, besides the
//   Bool and card var
//   // it is used for SetExpression standing for operation, for instance
//   SetIntersection will have
//   // two arguments
//   int num_args;
...
//   // bitset used to know if an element is variable (i.e., in ub and not not
//   in lb)
...
//   SetExpression(const int lelt, const int uelt, const int clb, const int
//   cub);
//   // SetExpression(const BitSet& lb, const BitSet& ub, const int clb, const
//   int cub); SetExpression(const Vector<int>& lb, const Vector<int>& ub, const
//   int clb, const int cub); void initialise_elements(); virtual
//   ~SetExpression();
...
public:
  /**
  OccExpression are constraints gluing together Boolean variables (plus a
  cardinality integer var) to define set variables
...
public:
  ...
  ValOccExpression(Vector<Variable> &args, const int val, const int lo,
                   const int up);

  virtual void encode();
};

Variable Occurrence(Vector<Variable> &X, const int value, const int lo = -INFTY,
                    const int up = INFTY);
Variable Occurrence(VarArray &X, const int value, const int lo = -INFTY,
                    const int up = INFTY);
...
public:
  ...
  VarOccExpression(Vector<Variable> &args, Variable x, const int lo,
                   const int up);

  virtual void encode();
};

Variable Occurrence(Vector<Variable> &X, Variable Y, const int lo = -INFTY,
                    const int up = INFTY);
Variable Occurrence(VarArray &X, Variable Y, const int lo = -INFTY,
                    const int up = INFTY);
...
public:
  ...
  SetOccExpression(Vector<Variable> &args, const BitSet &s, const int lo,
                   const int up);
  SetOccExpression(Vector<Variable> &args, const Vector<int> &s, const int lo,
                   const int up);
  SetOccExpression(Vector<Variable> &args, const std::vector<int> &s,
                   const int lo, const int up);

  virtual void encode();
};

Variable Occurrence(Vector<Variable> &X, const BitSet &s, const int lo = -INFTY,
                    const int up = INFTY);
Variable Occurrence(VarArray &X, const BitSet &s, const int lo = -INFTY,
                    const int up = INFTY);
Variable Occurrence(Vector<Variable> &X, const Vector<int> &s,
                    const int lo = -INFTY, const int up = INFTY);
Variable Occurrence(VarArray &X, const Vector<int> &s, const int lo = -INFTY,
                    const int up = INFTY);
Variable Occurrence(Vector<Variable> &X, const std::vector<int> &s,
                    const int lo = -INFTY, const int up = INFTY);
Variable Occurrence(VarArray &X, const std::vector<int> &s,
                    const int lo = -INFTY, const int up = INFTY);
...
public:
  ...
  IntOccExpression(Vector<Variable> &args, const Interval s, const int lo,
                   const int up);

  virtual void encode();
};

Variable Occurrence(Vector<Variable> &X, const Interval s,
                    const int lo = -INFTY, const int up = INFTY);
Variable Occurrence(VarArray &X, const Interval s, const int lo = -INFTY,
                    const int up = INFTY);
// Variable Occurrence(Vector<Variable>& X, const int l, const int u, const int
// lo=-INFTY, const int up=INFTY); Variable Occurrence(VarArray& X, const int l,
// const int u, const int lo=-INFTY, const int up=INFTY);
...
// Variable SetVariable(std::vector<int> lb, std::vector<int> ub, const int clb,
// const int cub); Variable SetVariable(BitSet lb, Bitset ub, const int clb,
// const int cub);
...
public:
  ...
  */
  ...
  // num_args returns the number of arguments of the constraint, besides the
  // Bool and card var it is used for SetExpression standing for operation, for
  // instance SetIntersection will have two arguments
  int num_args;
  ...
  // SetExpression(const BitSet& lb, const BitSet& ub, const int clb, const int
  // cub);
  SetExpression(const Vector<int> &lb, const Vector<int> &ub, const int clb,
                const int cub);
...
Variable SetVariable(const int lelt, const int uelt, const int clb = 0,
                     const int cub = INFTY);
...
public:
  ...
  void initialise(const int n, int lb = NOVAL, int ub = NOVAL,
                  int type = EXPRESSION) {

    // std::cout << "create var_array with " << n << " variables in [" << lb <<
    // ".." << ub << "]\n";
...
//   template < class WORD_TYPE >
//   /// Remove all values that do not appear in the current domain of the
//   Variable "x" Event VariableBitset< WORD_TYPE >::set_domain(Variable x) {
//     if(domain.size == 1) {
//       return(x.contain(domain.min) ? NO_EVENT : FAIL_EVENT);
//     } else {
//       int xsize = x.get_size();
...
//     /// Whether the domain has a nonempty intersection with the Variable x
//   template < class WORD_TYPE >
//   bool VariableBitset< WORD_TYPE >::intersect(Variable x) const { return
//   x.intersect(domain.values); }
//     /// Whether the domain is included in the Variable x
//   template < class WORD_TYPE >
//   bool VariableBitset< WORD_TYPE >::included(Variable x) const { return
//   x.includes(domain.values); }
//     /// Whether the domain is included in the Variable x
//   template < class WORD_TYPE >
//   bool VariableBitset< WORD_TYPE >::includes(Variable x) const { return
//   x.included(domain.values); }
...
//    // std::cout << "[" << s.next(min-1) << ".." << s.prev(max+1) << "]" <<
//    std::endl;
