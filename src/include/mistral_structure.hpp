...
public:
  ...
  inline DATA_TYPE &operator[](const int i) { return stack_[i]; }

  inline const DATA_TYPE operator[](const int i) const { return stack_[i]; }
...
public:
  ...
  TwoWatchedThing() {}
  TwoWatchedThing(const int n) { initialise(n); }
  ...
  void unwatch(const int ix1, const int w1) {
...
#ifdef _DEBUG_UNWATCH
      if (_DEBUG_UNWATCH) {
        std::cout << " and repoint its counterpart in " << o2 << "'s list"
                  << std::endl;
        std::cout << o2 << ":\n";
...
#ifdef _DEBUG_UNWATCH
      if (_DEBUG_UNWATCH) {
        std::cout << " and repoint its counterpart in " << o1 << "'s list"
                  << std::endl;
        std::cout << o1 << ":\n";
    ...
    // #ifdef _DEBUG_UNWATCH
    // 			if(_DEBUG_UNWATCH) {
    // 			std::cout << " stop watching " << w1 << " and " << w2 <<
    // " with
    // "
    // << watched_by[w1][ix1] << std::endl;
    //
    ...
    // #ifdef _DEBUG_UNWATCH
    // 			if(_DEBUG_UNWATCH) {
    // 			std::cout << "the other watched of " << l1 << " and " <<
    // l2
    // << " are, respectively, " << o2 << " and " << o1 << std::endl;
    //
    //
    // 			std::cout << o2 << ":\n";
    // 			std::cout << watched_by[o2] << std::endl;
    ...
    // 			std::cout << std::endl;
    //
    // 			std::cout << "the index of " << l1 << " in " << w1 <<"'s
    // list has changed to " << ix1 << std::endl;
    // std::cout << "the index of " << l2
    // << " in " << w2 <<"'s list has changed to " << ix2 << std::endl;
    // 		}
    // #endif
    //
    // 			//
    ...
    // #ifdef _DEBUG_UNWATCH
    // 			if(_DEBUG_UNWATCH) {
    // 			std::cout << " swap " << watched_by[w1][ix1] << " with "
    // << l1
    // << " and " << l2 << ", respectively" << std::endl << "=>\n"; std::cout <<
    // w1
    // << ":\n"; 			std::cout << watched_by[w1] <<
    // std::endl; std::cout
    // << watched_index[w1] << std::endl; 			std::cout <<
    // watched_other[w1] << std::endl; 			std::cout << std::endl;
    //
    // 			std::cout << w2 << ":\n";
    // 			std::cout << watched_by[w2] << std::endl;
    // 			std::cout << watched_index[w2] << std::endl;
...
template <class T> class TwoWayStack {
public:
  /*!@name Parameters*/
  //@{
  ...
  */
  ...
  void extend_stack(const unsigned int l = 0) {
    ...
    for (unsigned int i = 0; i < capacity; ++i) {
      new_index[i] =
          (index_[i] >= 0 ? (index_[i] + capacity - start) % capacity : -1);
    }
    ...
    std::fill((int *)new_stack + (capacity * f),
              (int *)new_stack + (capacity + increment) * f, 0);
  ...
  inline T &operator[](const int x) { return stack_[index_[x]]; }
...
template <class MAIN_TYPE, class AUX_TYPE> class BiStack {
public:
  /*!@name Parameters*/
  //@{
  ...
  void extend_stack(const unsigned int l = 0) {
    ...
    MAIN_TYPE *new_main_stack =
        (MAIN_TYPE *)malloc(capacity * sizeof(MAIN_TYPE));
    memcpy(new_main_stack, main_stack_,
           (capacity - increment) * sizeof(MAIN_TYPE));

    AUX_TYPE *new_aux_stack = (AUX_TYPE *)malloc(capacity * sizeof(AUX_TYPE));
    memcpy(new_aux_stack, aux_stack_,
           (capacity - increment) * sizeof(AUX_TYPE));
  ...
  std::ostream &display(std::ostream &os) const {
    ...
    os << "[";
    if (size)
      os << main_stack_[start] << "/" << aux_stack_[start];
    for (unsigned int i = 1; i < size; ++i)
      os << " " << main_stack_[(start + i) % capacity] << "/"
         << aux_stack_[(start + i) % capacity];
    os << "]";
    return os;
  }
  //@}
...
public:
  ...
  IntList() {}
  IntList(const int n, const bool filled = false) { initialise(n, filled); }
  ...
  bool contain(const int elt) const { return next[elt] != elt; }
  ...
  void verify(const char *msg) {
    ...
    if (rsize < size) {
      std::cout << "LIST SIZE INCORRECT!! (" << size << ") " << msg
                << std::endl;
      display(std::cout);
      std::cout << std::endl;
      exit(1);
...
template <class DATA_TYPE> class Vector2 {
public:
  /*!@name Parameters*/
  //@{
  DATA_TYPE *stack_;
  ...
  Vector2(const int n) {
    ...
    if (capacity)
      // stack_ = (DATA_TYPE*) malloc(capacity*sizeof(DATA_TYPE));
      stack_ = new DATA_TYPE[capacity]; //(DATA_TYPE*)
                                        // malloc(capacity*sizeof(DATA_TYPE));
    else
      stack_ = NULL;
  }
  //@}
  ...
  //       DATA_TYPE* new_stack = (DATA_TYPE*) realloc(stack_,
  //       new_capacity*sizeof(DATA_TYPE)); stack_ = new_stack;
  ...
  void resize(const unsigned int l) {
    if (capacity < l) {
      ...
      capacity = l;
      // DATA_TYPE* new_stack = (DATA_TYPE*) realloc(stack_,
      // capacity*sizeof(DATA_TYPE)); stack_ = new_stack;
...
public:
  ...
  virtual iterator get_reason_for(const Atom a, const int lvl,
                                  iterator &end) = 0;
...
template <class DATA_TYPE> class Array : public Explanation {
...
public:
  ...
  unsigned int **index_; // index[v][o] is the index of occurrence 'o' of value
                         // 'v' in 'values_'
  ...
  MultiSet(const int lb, const int ub, const int *occ = NULL,
           const int *mocc = NULL) {
    if (!occ) {
      int tmp_occ[ub - lb + 1];
  ...
  inline int head() { return *values_; }

  inline int back() { return values_[size - 1]; }
  ...
  // 	  else return_str += ",";
  // 	  return_str +=
  // ("("+toString((int)(occurrence_[v]))+"*"+toString(v)+")");
  // 	}
  //       }
  //       return_str += "}}";
  //       return return_str;
...
public:
  ...
  virtual void initialise(const int lb, const int ub, const int sz,
                          const bool full);
...
//       int idx = size;
//       while(idx && list_[idx-1] > elt) { // push every values greater than
//       elt above elt
...
//   {
//     assert((obj.size == 0) || ((unsigned int)(obj.back()->id - obj[0]->id +
//     1) == obj.size));
...
//     int rank = size;
//     while(idx && list_[rank-1]->id > elt->id) { // push every values greater
//     than elt above elt
...
class Solver;
template <class VAR_TYPE, class SIZE_TYPE> class VarStack {
public:
  /*!@name Parameters*/
  //@{
  /// list of values
  ...
  VarStack(Vector<VAR_TYPE> &obj, bool full = true) { initialise(obj, full); }
  ...
  void initialise(Solver *s) { size.initialise(s); }
  ...
  void initialise(Vector<VAR_TYPE> &obj, const bool full = true) {
    assert((obj.size == 0) ||
           ((unsigned int)(obj.back().id() - obj[0].id() + 1) == obj.size));
  ...
  inline bool safe_contain(const VAR_TYPE elt) const {
    return (elt.id() - offset <= (int)capacity &&
            (int)(index_[elt.id()]) < (int)size);
  }
  inline bool contain(const int elt) const {
  ...
  inline bool empty() const { return !(int)size; }
  ...
  inline VAR_TYPE &operator[](const unsigned int idx) { return list_[idx]; }
  //@}
  ...
  inline VAR_TYPE next() { return list_[size]; }

  inline VAR_TYPE pop() { return list_[--size]; }
  ...
  inline VAR_TYPE head() { return *list_; }

  inline VAR_TYPE back() { return list_[size - 1]; }
  ...
  inline void ordered_add(const VAR_TYPE elt) {
    ...
    while (idx &&
           list_[rank - 1].id() >
               elt.id()) { // push every values greater than elt above elt
      list_[rank] = list_[rank - 1];
      index_[list_[rank - 1].id()] = rank;
      --rank;
    }
  ...
  inline void revert_to(const int level) { size = level; }

  inline int index(const int elt_idx) {
    if (elt_idx > ((int)capacity - offset))
      return -1;
...
template <class CON_TYPE> class ConStack {
public:
  /*!@name Parameters*/
  //@{
  /// list of values
  ...
  ConStack(Vector<CON_TYPE> &obj, bool full = true) { initialise(obj, full); }
  ...
  inline bool contain(const int elt) const { return index_[elt] < size; }
  ...
  inline CON_TYPE &operator[](const unsigned int idx) { return list_[idx]; }
  //@}
  ...
  inline CON_TYPE next() { return list_[size]; }

  inline CON_TYPE pop() { return list_[--size]; }
  ...
  inline CON_TYPE head() { return *list_; }

  inline CON_TYPE back() { return list_[size - 1]; }
...
template <class DATA_TYPE> class Indexed {
...
public:
  ...
  Stack(Vector<DATA_TYPE> &obj, bool full = true) { initialise(obj, full); }

  virtual ~Stack() { delete[] list_; }
  ...
  inline DATA_TYPE pop() { return list_[--size].element; }
  ...
  inline DATA_TYPE head() { return list_->element; }
...
public:
  ...
  Node(DATA_TYPE e = (DATA_TYPE(0)), int p = -1, int n = -1) {
    elt = e;
    prev = p;
    next = n;
  }
  ...
  //     std::string getString() const {
  //       std::string return_str =
  //       "("+toString(prev)+"|"+toString(elt)+"|"+toString(next)+")"; return
  //       return_str;
  //     }
  std::ostream &display(std::ostream &os) const {
    // os << "(" << prev << "|" << elt << "|" << next << ")";
...
template <class DATA_TYPE, int NUM_HEAD> class MultiList {
...
public:
  ...
  inline Node<DATA_TYPE> &first(const int h) { return data.stack_[head[h]]; }
  ...
  void debug_print(std::ostream &os) const {
    for (unsigned int i = head[0]; i < data.size; ++i)
      os << "(" << data.stack_[i].prev << "|" << data.stack_[i].elt << "|"
         << data.stack_[i].next << ")";
...
public:
  ...
  static const unsigned int mantissa =
  ...
  Bitset<WORD_TYPE, FLOAT_TYPE> &
  operator=(const Bitset<WORD_TYPE, FLOAT_TYPE> &q) {
    if (!table)
      clone(q);
    else
      copy(q);
  ...
  void initialise(const int lb, const int ub, const WORD_TYPE p,
                  WORD_TYPE *pool = NULL) {
    neg_words = (lb >> EXP);
    pos_words = (ub >> EXP) + 1;
    if (pool == NULL)
  ...
  void iterate_into_b(const int size, int *buffer) {
    ...
    while (nval < size) {
      ...
      // cast into float, which will be coded as 1*2^exp, and 'exp' is precisely
  ...
  inline int minimum_element(int idx, WORD_TYPE v,
                             const int def = NOVAL) const {
  ...
  */
  ...
  inline void xor_with(const Bitset<WORD_TYPE, FLOAT_TYPE> *s) { xor_with(*s); }
...
#endif
    /*
    v = v - ((v >> 1) & (WORD_TYPE)~(WORD_TYPE)0/3); // temp v = (v &
    (WORD_TYPE)~(WORD_TYPE)0/15*3) + ((v >> 2) & (WORD_TYPE)~(WORD_TYPE)0/15*3);
    // temp v = (v + (v >> 4)) & (WORD_TYPE)~(WORD_TYPE)0/255*15; // temp return
    (WORD_TYPE)(v * ((WORD_TYPE)~(WORD_TYPE)0/255)) >> (sizeof(v) - 1) *
    CHAR_BIT; // count
    */
  }
  ...
  */
  ...
  /*!
  Return true iff the calling object intersect s (s is assumed to be a bitset in
  {0,..,31}) [O(N/32)]
  */
  ...
  inline bool fast_intersect(const Bitset<WORD_TYPE, FLOAT_TYPE> &s,
                             int &idx) const {
  ...
  */
  ...
  inline bool operator[](const int i) { return fast_contain(i); }
...
public:
  ...
  static InlinedBitset<WORD_TYPE, FLOAT_TYPE> *
  make_new(const int lb, const int ub, const WORD_TYPE p) {
...
public:
  ...
  bool is_initialised() { return (next != NULL); }
  void initialise(const int n) {
    next = new int[n + 1];
  ...
  inline int first() { return next[_head]; }
  ...
  /// Is the queue empty?
  inline bool empty() const { return _head == _tail; }
...
public:
  ...
  std::ostream &display(std::ostream &os) const {
    ...
    while ((unsigned int)(1 << n_level) <= data.size)
      ++n_level;
...
template <class T> class IndexedBinaryMaxHeap {

private:
  // starting at position 'rank' in the tree, move up to the root swapping
  // elements until this branch is sorted
  ...
  // starting at position 'rank' in the tree, move down to a leaf swapping
  // elements until this branch is sorted
  inline void sift_down(unsigned int rank) {
    ...
    while (descendant < heap.size) {
      ...
      ++descendant;
      if (descendant < heap.size &&
          value[heap[descendant]] > value[heap[largest]]) {
        largest = descendant;
      }
      if (largest == rank)
        break;
...
public:
  ...
  std::ostream &display(std::ostream &os) const {
    ...
    unsigned int n_level = 0;
    while ((unsigned int)(1 << n_level) <= heap.size)
      ++n_level;
...
public:
  ...
  BiInterval(const int n_min, const int n_max, const int p_min, const int p_max,
             const bool z);
...
std::ostream &operator<<(std::ostream &os,
                         const IndexedBinaryMaxHeap<DATA_TYPE> &x) {
  return x.display(os);
}
...
std::ostream &operator<<(std::ostream &os,
                         const BiStack<MAIN_TYPE, AUX_TYPE> &x) {
  return x.display(os);
}
...
std::ostream &operator<<(std::ostream &os,
                         const VarStack<DATA_TYPE, SIZE_TYPE> &x) {
  return x.display(os);
}
...
template <class DATA_TYPE, int NUM_HEAD>
std::ostream &operator<<(std::ostream &os,
                         const MultiList<DATA_TYPE, NUM_HEAD> &x) {
  return x.display(os);
}

template <class WORD_TYPE, class FLOAT_TYPE>
std::ostream &operator<<(std::ostream &os,
                         const Bitset<WORD_TYPE, FLOAT_TYPE> &x) {
  return x.display(os);
}
...
//   template < class DATA_TYPE, int NUM_HEAD >
//   std::ostream& operator<< (std::ostream& os, const MultiList< DATA_TYPE,
//   NUM_HEAD >* x) {
//     return x->display(os);
//   }
...
//   template < class DATA_TYPE, int NUM_HEAD >
//   std::ostream& MultiList< DATA_TYPE, NUM_HEAD >::display(std::ostream& os)
//   const {
//     //for(unsigned int i=0; i<data.size; ++i)
//     //os << data[i];
...
public:
  ...
  Graph(const int n, const int full_nodes = true,
        const int full_edges = false) {
  ...
  virtual void initialise(const int n, const bool full_nodes = true,
                          const bool full_edges = false) {

    // std::cout << "initialise " << n << " " << full_nodes << " " << full_edges
    // << std::endl;
    ...
    for (int i = 0; i < capacity; ++i) {
      // matrix[i].initialise(0, capacity-1, (_full ? BitSet::full :
      // BitSet::empt));
  ...
  int get_neighbor(const int x, const int i) const { return neighbor[x][i]; }
  ...
  bool is_matched(const int u) { return (matching[u] != -1); }
...
public:
  ...
  CompactGraph(const int n) { initialise(n); }
  ...
  void flip() {
    ...
    // std::cout << "flip graph" << std::endl << " 1/ clear nodes and edges" <<
    // std::endl;
  ...
  void add_node(const int x) { node.add(x); }
...
TempBipartiteGraph does not inherit from Graph since it uses very different data
structures.
...
// col_ids contains all adjacency vectors. The adjacency vector of a left-side
vertex v starts at index col_ptr[v] and ends at index col_ptr[v+1]-1. int
*col_ids; int *col_ptrs;

// If v is a left-side vertex, then match[v] is the right-side vertex matched
with v, and -1 otherwise. int *match;
...
TempBipartiteGraph(const int* col_ptrs_in, const int* col_ids_in, const int
n_in, const int m_in) { col_ids = col_ids_in; col_ptrs_in = col_ptrs_in; n =
n_in; m = m_in; match = (int*)malloc(sizeof(int)*n); row_match =
(int*)malloc(sizeof(int)*m); visited = (int*)malloc(sizeof(int)*m); stack =
(int*)malloc(sizeof(int)*n); colptrs = (int*)malloc(sizeof(int)*n); lookahead =
(int*)malloc(sizeof(int)*n); unmatched = (int*)malloc(sizeof(int)*n);
...
stack[0] = current_col; stack_last = 0; colptrs[current_col] =
col_ptrs[current_col];

while(stack_last > -1) {
stack_col = stack[stack_last];

eptr = col_ptrs[stack_col + 1];
for(ptr = lookahead[stack_col]; ptr < eptr && row_match[col_ids[ptr]] != -1;
ptr++){} lookahead[stack_col] = ptr + 1;
...
public:
  ...
  BipartiteGraph(const int n) { initialise(n); }

  void initialise(const int n, const bool full_nodes = true,
                  const bool full_edges = false) {
    capacity = n;
    node.initialise(0, capacity - 1, capacity, full_nodes);
    node_set.initialise(0, capacity - 1, (full_nodes ? ~0 : 0));
  ...
  void set_left(const int v) { left.add(v); }

  void set_right(const int v) { right.add(v); }
  ...
  void remove_and_save(const int v) {
    ...
    node.remove(v);
    // EH CHANGE?? for (int i=0; i<neighbor[v].size; i++)
    // neighbor[v][i].remove(v);
    for (unsigned int i = 0; i < neighbor[v].size; i++)
      neighbor[neighbor[v][i]].remove(v);
  ...
  void restore(const int v, const bool isleft) {
    ...
    for (unsigned int i = 0; i < neighbor[v].size; i++)
      neighbor[neighbor[v][i]].add(v);
    matching_is_max = false; // Maybe we should keep track of the old matching
                             // and restore it ?
  }
  ...
  void clear_visited() {
    for (size_t i = 0; i != rightvisited_toclear.size(); ++i)
      rightvisited[rightvisited_toclear[i]] = false;
    for (size_t i = 0; i != leftvisited_toclear.size(); ++i)
      leftvisited[leftvisited_toclear[i]] = false;
  ...
  void persistently_matched_left_vertices() {
    ...
    // Identify all vertices reacheable from an unmatched vertex via an
    // alternating path of even length (using breadth-first search)
    for (unsigned int i = 0; i < left.size; i++) {
      if (is_matched(left[i]))
        continue;
        ...
        while (!leftfrontier.empty()) {
          ...
          for (unsigned int q = 0; q < neighbor[u].size; q++) {
            ...
            if (rightvisited[v])
              continue;
            assert(
                is_matched(v)); // otherwise we would have an augmenting path !
    ...
    for (unsigned int i = 0; i < left.size; i++) {
      if (!leftaccessed[left[i]])
        lpmatched.add(left[i]);
    }
  }
  ...
  void find_matching() {
    int u, v;
    for (unsigned int fleft = 0;
         (fleft < left.size) && (nmatch < (int)(left.size)) &&
         (nmatch < (int)(right.size));
         fleft++) {
      if (is_matched(left[fleft]))
        continue;
  ...
  void matching_to_vertex_cover() {
    ...
    // Identify all vertices reacheable from an unmatched vertex via an
    // alternating path (using breadth-first search)
...
public:
  ...
  ReversibleGraph(const Graph &g) : Graph(g) { initialise_degree(); }
  ...
  void verify(const char *msg) {
    ...
    for (size_t x = 0; x < node.size; ++x) {
      ...
      if (neighbor[y].size != matrix[y].size()) {
        std::cout << msg << ": incosistency in representations: " << neighbor[y]
                  << " != " << matrix[y] << std::endl;
        exit(1);
      }
#endif
      ...
      for (size_t j = 0; j < neighbor[y].size; ++j) {
        if (!matrix[y].contain(neighbor[y][j])) {
          std::cout << msg << ": "
                    << "matrix[" << y << "] does not contain " << neighbor[y][j]
                    << " (" << matrix[y] << ")" << std::endl;
          exit(1);
        }
        ...
        if (!neighbor[neighbor[y][j]].contain(y)) {
          std::cout << msg << ": "
                    << "neighbor[" << neighbor[y][j] << "] does not contain "
                    << y << " (" << neighbor[neighbor[y][j]] << ")"
                    << std::endl;
          exit(1);
        }
      }

      // check that the vertices in [neighbor[y].size, original_size[y][ iif
      ...
      for (int j = neighbor[y].size; j < original_size[y]; ++j) {
        if (node.contain(neighbor[y][j])) {
          std::cout << msg << ": wrong dual! (" << neighbor[y][j]
                    << " should be at or after position " << original_size[y]
                    << ")" << std::endl;
          exit(1);
        }
      }
      ...
      for (int j = original_size[y]; j < capacity; ++j) {
        if (!node.contain(neighbor[y][j])) {
          std::cout << msg << ": wrong dual! (" << neighbor[y][j]
                    << " should be before position " << original_size[y] << ")"
                    << std::endl;
          exit(1);
        }
      }
    ...
    for (size_t x = 0; x < node.size; ++x) {
      ...
      if (!node_of_degree[d].contain(node[x])) {
        std::cout << *this << std::endl
                  << msg << ": node_of_degree[" << d << "] does not contain "
    ...
    if (lb != min_degree) {
      std::cout << *this << std::endl
                << msg << ": min_degree=" << min_degree << " (should be " << lb
                << ")" << std::endl;
      exit(1);
    }
    if (ub != max_degree) {
      std::cout << *this << std::endl
                << msg << ": max_degree=" << max_degree << " (should be " << ub
                << ")" << std::endl;
      exit(1);
    }
    ...
    if (ne != num_edges) {
      std::cout << *this << std::endl
                << msg << ": num_edges=" << num_edges << " (should be " << ne
                << ")" << std::endl;
      exit(1);
    }
    assert(ne == num_edges);
  ...
  void rem_and_update(const int x) {
    ...
    while (i--) {
      ...
      // std::cout << "remove " << x << " from neighbor[" << y << "] => move "
      // << y << " from "
      // 	<< node_of_degree[d] << " to " << node_of_degree[d-1] <<
      // std::endl;
      //
      node_of_degree[d].remove(y);
      node_of_degree[d - 1].add(y);

      // std::cout << "  ==> " << node_of_degree[d] << " and " <<
      // node_of_degree[d-1] << " ("
...
template <class GraphType> class VCAlgo {
...
public:
  ...
  VCAlgo(const GraphType &g, const int k) : _graph(g), _upper_bound(k) {
  ...
  void verify_trail(const char *msg) {
    ...
    for (int i = 0; i < _trail.size; ++i) {
      ...
      assert(_rank[x] == i);
      if (i)
        assert(_level[x] >= _level[_trail[i - 1]]);

      if (dec_idx < _decision.size && x == _decision[dec_idx]) {
        assert(_reason[x] == NO_REASON);
        if (i)
          assert(_level[x] == _level[_trail[i - 1]] + 1);
        else
          assert(_level[x] <= init_level + 1);
        ++dec_idx;
      } else {

        if (_reason[x] == NO_REASON) {
          std::cout << "ERROR: reason/trail of " << x << " (" << msg << ")"
                    << std::endl;
          print_trail(true);
        }
        ...
        if (_reason[x] != DEDUCTION) {
          ...
          // std::cout << " (" << msg << ") reason[" << x << "] = " <<
          // _reason[x] << std::endl;

          if (_reason[x] < 0) {
            std::cout << " (" << msg << ") reason[" << x << "] = " << _reason[x]
                      << std::endl;
          }
    ...
    for (int i = 0; i < _graph.node.size; ++i) {
      ...
      for (int j = 0; j < _graph.neighbor[x].size; ++j) {
        ...
        if (_cover.contain(y)) {
          std::cout << msg << " inconsistency graph/cover: " << y
                    << " cannot be both in the graph and in the cover "
                    << _cover << "\n"
                    << _graph << std::endl;
        }
      ...
      for (int j = 0; j < _graph.codegree(x); ++j) {
        ...
        if (!_cover.contain(y)) {
          std::cout << msg << " inconsistency: since " << x
                    << " is in the graph, " << y
                    << " must be either in the graph or in the cover " << _cover
    ...
    for (int x = 0; x < n; ++x) {
      for (int i = 0; i < _graph.original_size[x]; ++i) {
        ...
        if (!util_set.contain(x) and !util_set.contain(y)) {
          ...
          std::cout << "The edge (" << x << "," << y << ") is not covered by "
                    << util_set << std::endl;
          exit(1);
        }
      }
  ...
  int greedy_lower_bound(const int limit = -1) {
    ...
    while (covered < m) {
      ...
      if (++_lower_bound >= stop)
        break;

      covered += c;

      l = k - c;
      while (k > l && --degree_count[k] == 0)
        --k;
      for (i = k; --i > l;)
        --degree_count[i];
    }
  ...
  // pop from the trail
  inline int restore() { return _trail.pop(); }
  ...
  inline bool suboptimal() {
    ...
    if (_graph.num_edges) {
      ...
      if (_upper_bound < _graph.capacity) {
        _lower_bound =
            _cover.size + (_graph.size() - _graph.min_clique_cover_bitset());
      }
    }

#ifdef _TRACE_
    std::cout << "bounds = [" << _lower_bound << ".." << _upper_bound << "]"
              << std::endl;
#endif
  ...
  bool backjump() {
    ...
    // }
    // std::cout << std::endl;
    // std::cout << "nc: " << _graph.num_cliques << " / " << _graph.capacity <<
    // std::endl;
    ...
    for (i = _cover.size; i < _graph.capacity; ++i) {
      ...
      if (!_graph.node.contain(x)) {
        ...
        // std::cout << "i: " << i << ", x: " << x << "nc: " <<
        // _graph.num_cliques << " / " << _graph.capacity << std::endl;
    ...
    if (_DEBUG_BACKJUMPS) {
      if (_cover.empty()) {
        ...
        std::cout << " Extract a minimal conflict " << _explanation << std::endl
                  << "                 from cover ["
                  << _cover[_cover.size - 1]; //_cover << std::endl;
    ...
    if (_explanation.size == 0) {
      ...
      if (y != _explanation.capacity) {
        while (_level[y] == _level[x]) {
          // resolve x since there are at least one other literal from the same
          // level

          // std::cout << "           current conflict " << _explanation <<
          // std::endl;
...
#ifdef _DEBUG_BACKJUMPS
          if (_reason[x] < 0) {
            std::cout << "node " << x << " level " << _level[x] << " rank "
                      << _rank[x] << std::endl;
          ...
          // the previous resolved literal had the same reason, no need to
          // resolve twice
        ...
        // x is the last literal from this level and y is the literal of highest
        // level otherwise
        _explanation.remove(x);
...
#ifdef _DEBUG_BACKJUMPS
      if (_DEBUG_BACKJUMPS) {
        std::cout << "                            " << _explanation
                  << std::endl;
        std::cout << " backjump " << nbacktracks << " levels (of "
                  << _decision.size << ") and exclude " << x << std::endl
                  << std::endl;
      }
#endif
  ...
  void print_head() {
    std::cout << std::setw(3) << "#"
              ...
              << " " << std::setw(12) << "memory"
              << " " << std::endl
              << std::setfill('=') << std::setw(83) << "=" << std::endl
              << std::setfill(' ');
  }

  void print_summary() {
    double time_now = cpu_time();
    std::cout << std::setfill('=') << std::setw(83) << "=" << std::endl
              << std::setfill(' ');
    print_head();
    print_statistics();
    std::cout << std::setfill('=') << std::setw(83) << "=" << std::endl
              << std::setfill(' ') << std::setw(12)
              << (optimal ? "optimal " : "timeout ") << std::setw(9)
              << (int)(nb_conflicts / (time_now - time_start)) << " fails/s  "
              << std::setw(9) << (int)(nb_nodes / (time_now - time_start))
              << " nodes/s  "
              << "       memory: " << std::setw(9) << std::left << nogood_size
              << std::right << std::endl;
    //<< std::setfill('=') << std::setw(62) << "=" << std::endl <<
    // std::setfill(' ');

#ifdef _NOGOOD_STAT
    for (int i = 0; i < _graph.capacity; ++i) {
      if (backjump_sizes[i] || nogood_sizes[i]) {
        std::cout << std::right << std::setw(8) << nogood_sizes[i]
                  << " nogoods," << std::right << std::setw(8)
                  << backjump_sizes[i] << " backjumps of size " << std::left
  ...
  void print_statistics() {
    std::cout << std::setw(3) << nb_solutions << " | " << std::setw(6)
              << _upper_bound << " " << std::setw(6)
              << (_graph.capacity - _upper_bound) << " " << std::setw(8)
              << (int)((cpu_time() - time_start) * 1000) << " " << std::setw(5)
              << nb_restarts << " " << std::setw(10) << nb_conflicts << " "
              << std::setw(10) << nb_nodes << " " << std::setw(6)
              << (int)(nb_conflicts ? (double)nogood_size / (double)nb_conflicts
                                    : 0)
              << " " << std::setw(6)
              << (double)((int)(((nb_conflicts ? (double)backjump_size /
                                                     (double)nb_conflicts
                                               : 0) *
                                 10))) /
                     10.0
  ...
  bool run(const int limit, const int ub_limit) {
    ...
    while (feasible && (limit < 0 || nb_restart_conflicts < limit) &&
           _upper_bound > ub_limit) {
      ++nb_nodes;
      if (suboptimal()) {
...
public:
  ...
  // // so that when we remove it from one list, we can also remove it from the
  // second Vector<int> *watched_index; Vector<int> *watched_other;
  ...
  void verify_watchers(const char *msg) {
    for (size_t i = 0; i < node.size; ++i) {
      ...
      if (watchers.watched_by[w1].size != watchers.watched_index[w1].size ||
          watchers.watched_by[w1].size != watchers.watched_other[w1].size) {
        std::cout << msg << ": inconsistency in size of the watched lists of "
                  << w1 << ": " << std::endl;
      ...
      for (size_t ixw1 = 0; ixw1 < watchers.watched_by[w1].size; ++ixw1) {
        ...
        if (watchers.watched_other[w2][ixw2] != w1) {
          std::cout << msg << ":inconsistency in the watched lists (other) of "
                    << x << " (" << w1 << " & " << w2 << "): " << std::endl;
          std::cout << watchers.watched_by[w1] << " / "
                    << watchers.watched_by[w2] << std::endl;
          std::cout << watchers.watched_index[w1] << " / "
        ...
        if (watchers.watched_index[w2][ixw2] != (int)ixw1) {
          std::cout << msg << ": inconsistency in the watched lists (index) of "
                    << x << " (" << w1 << " & " << w2 << "): " << std::endl;
          std::cout << watchers.watched_by[w1] << " / "
                    << watchers.watched_by[w2] << std::endl;
          std::cout << watchers.watched_index[w1] << " / "
  ...
  // stupi implementation of dsatur, just as a witness that it does not seem to
  // help
  int min_clique_cover_bitset_sat(const int limit = -1) {
    ...
    while (!util_stack.empty()) {
      ...
      for (i = 1; i < util_stack.size; ++i) {
        y = util_stack[i];
        // if(saturation[x]<saturation[y] || (saturation[x]==saturation[y] &&
        // degree(x)>degree(y))) {
        if (degree(x) > degree(y) ||
            (degree(x) == degree(y) && saturation[x] < saturation[y])) {
          x = y;
        }
      }
  ...
  bool neighboring_clique(const int x, int &w1, int &w2) {
    ...
    if (_DEBUG_NEIGHCLIQUE) {
      std::cout << "check if " << x << "'s neighbors = " << neighbor[x]
    ...
    if (neighbor[x].size > 1) {
      ...
      for (i = neighbor[x].size; --i >= 0;) {
        ...
        if (_DEBUG_NEIGHCLIQUE) {
          std::cout << "      -> " << y << ": " << matrix[y]
                    << " is a super set of " << util_set << "?" << std::endl;
        }
#endif
  ...
  inline bool kernelize_buss(VCAlgo<ReversibleCompactGraph> &algo) {
    bool pruning = false;
    if (num_edges &&
        max_degree >= (algo._upper_bound - (int)(algo._cover.size))) {

      int *begr = algo._cover.begin();
      int offset = std::min(
          (int)(algo._cover.size),
          (max_degree - (algo._upper_bound - (int)(algo._cover.size))));
      ...
      do {
        ...
        // std::cout << util_vec.size << " " << algo._cover.size-offset <<
        // std::endl;

#ifdef _DEBUG_REDUCTION
        if (_DEBUG_REDUCTION) {
          std::cout << " add " << x << " (Buss rule): " << algo._cover
                    << std::endl;
        }
#endif
        ...
        rem_and_update(x);
        reason =
            (algo.use_backjump ? algo.explain_with(endr, begr) : NO_REASON);
      ...
      do {
        ...
        while (num_edges && min_degree <= 1) {
          ...
          if (_DEBUG_REDUCTION) {
            // #ifdef _DEBUG_KERNCLIQUE
            // 						if(_DEBUG_KERNCLIQUE) {
            std::cout << "  remove " << x << " b/c low degree (" << degree(x)
                      << "): " << co_neighbor[x] << std::endl;
          }
#endif

          rem_and_update(x);
          reason =
              (algo.use_backjump ? algo.explain_with(co_neighbor[x].end(),
                                                     co_neighbor[x].begin())
                                 : NO_REASON);
...
#ifdef _DEBUG_KERNCLIQUE
        if (_DEBUG_KERNCLIQUE) {
          std::cout << "  iteration for nodes of degree " << d
                    << ":"; //<< std::endl;
        }
#endif
        ...
        for (j = 0; j < (int)(node_of_degree[d].size);) {
          ...
          if (neighboring_clique(x, w1, w2)) {
            ...
            if (_DEBUG_REDUCTION) {
              ...
              // 						if(_DEBUG_KERNCLIQUE)
              // {
              std::cout << "\n  remove " << x << " b/c its neighborhood N-"
                        << co_neighbor[x] << "is a clique" << std::endl;
            }
#endif
  ...
  bool kernelize_cliques_incremental(VCAlgo<ReversibleCompactGraph> &algo) {
    ...
    if (_DEBUG_KERNCLIQUEINCR) {
      std::cout << "\nkernelize for the clique dominance (incremental, "
                   "checking the removed witnesses:";
    ...
    while (num_edges && algo._last_checked < (int)(algo._trail.size)) {
      ...
      while (num_edges && min_degree <= 1) {
        ...
        reason = (algo.use_backjump ? algo.explain_with(co_neighbor[x].end(),
                                                        co_neighbor[x].begin())
                                    : NO_REASON);
        rem_and_update(x);
        algo.exclude_from_cover(x, reason);
      ...
      // now take the first removed node y and check the nodes for which it was
      // witness
      ...
      if (_DEBUG_KERNCLIQUEINCR) {
        std::cout << "  * check non cliques witnessed by " << y << ": "
                  << watchers.watched_by[y] << " " << num_edges << " "
        ...
        if (node.contain(x)) {
          ...
          if (neighboring_clique(x, w1, w2)) {
            ...
            if (_DEBUG_REDUCTION) {
              ...
              // 							if(_DEBUG_KERNCLIQUEINCR)
              // {
              std::cout << "  remove " << x << " b/c its neighborhood N-"
                        << co_neighbor[x] << " is a clique" << std::endl;
            }
#endif
  ...
  bool kernelize_crowns(VCAlgo<ReversibleCompactGraph> &algo) {
    ...
    if (nmatch > algo._upper_bound - (int)(algo._cover.size))
      return false;
  ...
  bool kernelize_NT(VCAlgo<ReversibleCompactGraph> &algo) {
    ...
    for (size_t i = 0; i < H->left.size; i++) {
      ...
      if ((H->vcover.contain(v)) && (H->vcover.contain(v + capacity))) {
        ...
        algo.add_to_cover(v, NO_REASON);
      } else if ((!H->vcover.contain(v)) &&
                 (!H->vcover.contain(v + capacity))) {
    ...
    if (size() > 2 * (algo._upper_bound - (int)(algo._cover.size)))
      return false;
  ...
  bool kernelize_cliques_incremental_old(VCAlgo<ReversibleCompactGraph> &algo) {
    ...
    while (num_edges && algo._last_checked < (int)(algo._trail.size)) {
      ...
      while (num_edges && min_degree <= 1) {
        ...
        reason = (algo.use_backjump ? algo.explain_with(co_neighbor[x].begin(),
                                                        co_neighbor[x].end())
                                    : NO_REASON);
        rem_and_update(x);
        algo.exclude_from_cover(x, reason);
...
#ifdef _DEBUG_KERNCLIQUEINCR
      if (_DEBUG_KERNCLIQUEINCR) {
        std::cout << "  NOT HERE check non cliques witnessed by " << y << ": "
                  << watchers.watched_by[y] << std::endl;
      }
#endif
      ...
      while (num_edges && !watchers.watched_by[y].empty()) {
        ...
        if (node.contain(x)) {
          ...
          if (neighboring_clique(x, w1, w2)) {
            ...
            if (_DEBUG_KERNCLIQUEINCR) {
              std::cout << "  remove " << x
                        << " b/c its neighborhood is a clique" << std::endl;
            }
#endif

            reason =
                (algo.use_backjump ? algo.explain_with(co_neighbor[x].begin(),
                                                       co_neighbor[x].end())
                                   : NO_REASON);
            rem_and_update(x);
            algo.exclude_from_cover(x, reason);
  ...
  int my_fucking_clique_cover() {
    ...
    while (!node.empty()) {
      ...
      while (!node.empty()) {
        // start a clique with the vertex of lowest degree, then on next steps
        // select the node of highest degree
      ...
      // at this point the graph is empty, undo the removals from the last
      // clique, and remove only the vertices of the clique
  ...
  int min_clique_cover_list() {
    ...
    while (node.size) {
      ...
      ++num_cliques;
      neighbor_clique(x, node, start_clique, start_candidates, end_candidates,
                      true);
...
#ifdef _DEBUG_CLIQUECOV
      std::cout << "pick next node: " << x << "(" << saturation[x] << "/"
                << degree(x) << ")";
#endif
...
#elif _SAT_DEG_
        if (saturation[y] > saturation[x] ||
            (saturation[y] == saturation[x] && degree(y) < degree(x))) {
#elif _DEG_SAT_
        if (degree(y) < degree(x) ||
            (degree(y) == degree(x) && saturation[y] > saturation[x])) {
#elif _DEG_PLUS_SAT_
  ...
  // if false, a maximal clique with a subset of neighbors of x is at the back
  // of V and all vertices at the front (until "end_candidates") are not
  // neighbors of the node at index "start_clique"
  bool
  neighbor_clique(const int x, IntStack &V, int &start_clique,
                  int &start_candidates, int &end_candidates,
    ...
    if (_DEBUG_ADJCC) {
      if (maximize)
        std::cout << std::endl
                  << "  find a maximal clique in " << V << " around " << x
                  << std::endl;
      else
        std::cout << std::endl
                  << "  check if " << x << "'s neighborhood in " << V
                  << " is a clique" << std::endl;
    }
#endif
    ...
    for (int i = degree(x) - 1; start_candidates < end_candidates && i >= 0;
         --i) {
    ...
    while (start_candidates > 0 && (maximize || full_clique)) {
      ...
      // [candidates]-end_candidate-[not-clique]-start_clique-[clique] with "a"
      // in clique and V empty

#ifdef _DEBUG_ADJCC
      if (_DEBUG_ADJCC) {
        std::cout << "  move " << a
                  << " to the clique and intersect with its neighbors = "
                  << neighbor[a] << std::endl
                  << "  (2) [";
        for (int i = 0; i < start_candidates; ++i) {
          if (i)
      ...
      // we go through a's neighbors and add them into V (i.e. put them first in
      // [candidates])
      start_candidates = 0;
      d = capacity + 1;
      ...
      for (int i = degree(a) - 1; start_candidates < end_candidates && i >= 0;
           --i) {
  ...
  void clique_dominance_incremental(VCAlgo<ReversibleCompactGraph> &algo) {
    ...
    while (algo._last_checked < (int)(algo._trail.size)) {
      ...
      for (i = watchers.watched_by[y].size - 1; i >= 0; ++i) {
        ...
        if (neighbor_clique(x, node, start_clique, start_candidates,
          ...
          rem_and_update(x);
          algo.exclude_from_cover(x, algo.explain_with(co_neighbor[x].begin(),
                                                       co_neighbor[x].end()));
          ...
          // x's neighborhood is not a clique, and
          // node[start_clique],node[end_candidates] is a witness
      ...
      while (!util_stack.empty()) {
        ...
        if (neighbor_clique(x, node, start_clique, start_candidates,
          ...
          if (_DEBUG_CLIQUEDOM) {
            ...
            // std::cout << "start clique = " << start_clique << " size " <<
            // node.size << std::endl;
...
#ifdef _DEBUG_CLIQUEDOM
              if (_DEBUG_CLIQUEDOM) {
                std::cout << "remove " << node[j] << " from " << util_stack
                          << std::endl;
              }
#endif
  ...
  void verify(const char *msg) {
    ...
    for (size_t i = 0; i < node.size; ++i) {
      ...
      for (size_t j = 0; j < neighbor[x].size; ++j) {
        ...
        if (neighbor[y][nb_index[x][j]] != x) {
          // std::cout << std::endl;
          // display(std::cout);
          std::cout << msg << ": inconsistency for edge " << x << "," << y
                    << std::endl;
          exit(1);
        }
    ...
    if (node.size > 0) {
      if (lb != min_degree) {
        std::cout << *this << std::endl
                  << msg << ": min_degree=" << min_degree << " (should be "
                  << lb << ")" << std::endl;
        exit(1);
      }
      if (ub != max_degree) {
        std::cout << *this << std::endl
                  << msg << ": max_degree=" << max_degree << " (should be "
                  << ub << ")" << std::endl;
        exit(1);
      }
    }
  ...
  void add_and_update(const int x) {
    ...
    while (i--) {
      ...
      if (min_degree == d && node_of_degree[d].empty()) {
        min_degree = d + 1;
        // std::cout << "increase min degree (" << d << " => " << (d+1) <<") b/c
        // " << "node_of_degree[" << d << "] = " << node_of_degree[d] << " is
        // empty" << std::endl;
      }
  ...
  // 		// returns true if the neighbors of x in V form a clique
  // 		// if true, the clique is at the back of V starting at
  // "start_clique"
  // 		// if false, a maximal clique with a subset of neighbors of x is
  // at the back of V and all vertices at the front
  // 		// (until "end_candidates") are not neighbors of the node at
  // index
  // "start_clique" 		bool neighbor_clique(const int x, IntStack& V,
  // int& start_clique, 		int& start_candidates, 		int&
  // end_candidates, 		const bool maximize) {
  //
  // #ifdef _DEBUG_ADJCC
  // 			std::cout << std::endl << "  check if " << x << "'s
  // neighborhood in " << V << " is a clique" << std::endl; #endif
  //
  ...
  // 					end_candidates = start_candidates-1;
  //
  // 					// set a as the candidate of lowest
  // degree 					a = c;
  // 					// remove it from [candidates]
  // 					V.move(a, --start_candidates);
  // 					// move it to [clique]
  ...
  //
  // 					// at this point we have
  // [candidates]-end_candidate-[not-clique]-start_clique-[clique]
  // 					// with "a" in clique and V empty
  //
  // #ifdef _DEBUG_ADJCC
  // 					std::cout << "move " << a << " to the
  // clique and intersect with its neighbors = " << neighbor[a] << std::endl
  // 						<< "  [";
  // 					for(int i=0; i<start_candidates; ++i) {
  // 						if(i) std::cout << " ";
  // 						std::cout << V[i];
  ...
  // #endif
  //
  // 					// we go through a's neighbors and add them
  // into V (i.e. put them first in [candidates]) start_candidates = 0;
  // d=capacity+1;
  // 				}
  // 			} while(start_candidates>0 && (maximize ||
  // full_clique));
  //
  //
  // 			return full_clique;
  ...
  //
  // #ifdef _DEBUG_CLIQUEDOM
  // 				std::cout << std::endl << util_stack ; //<<
  // std::endl
  // << "check " << x ; #endif
  //
  // 				//size_before = node.size;
  //
  // 				if(neighbor_clique(x, node, start_clique,
  // start_candidates, end_candidates, maximize)) {
  //
  // #ifdef _DEBUG_CLIQUEDOM
  // 					std::cout << "-> clique:" ;
  // 					for(int j=start_clique; j<node.size;
  // ++j) { 						std::cout << " " <<
  // node[j];
  // 					}
  // 					std::cout << std::endl;
  ...
  // // 					if(neighbor_clique(x, node,
  // start_clique, end_candidates, false)) {
  // // 						// the neighbors of x form a
  // clique, we can remove it
  // //
  // // 						// first restore the set
  // of nodes
  // // 						node.size = size_before;
  // //
  // // 						rem_and_update(x);
...
public:
  ...
  VertexCoverAlgo(const Graph &g, const int k) : _graph(g), _upper_bound(k) {
  ...
  bool kernelize_buss() {
    ...
    if (_graph.num_edges >
        _graph.max_degree * (_upper_bound - (int)(_cover.size))) {
      return false;
    }
  ...
  bool kernelize_cliques() {
    ...
    while (_graph.num_edges && !fix_point) {
      fix_point = true;
      // std::cout << _graph.min_degree << " <> " << _graph.max_degree <<
      // std::endl;
      d = _graph.min_degree;
      while (_graph.num_edges && d <= _graph.max_degree) {
        while (_graph.num_edges && _graph.min_degree <= 1) {
          // std::cout << "there are nodes of degree <= 1 (" <<
          // _graph.min_degree << ")" << std::endl;

          // std::cout << _graph.node_of_degree[_graph.min_degree].size <<
          // std::endl;

          x = _graph.node_of_degree[_graph.min_degree][0];

          // std::cout << "remove " << x << " and add " << _graph.neighbor[x] <<
          // std::endl; remove_and_save(x, NO_REASON); save(x, NO_REASON);
          ...
          if (use_backjump) {
            ...
            if (_DEBUG_BACKJUMPS) {
              std::cout << std::endl
                        << "   -> explains " << x << " and its neighbors "
                        << _graph.neighbor[x] << std::endl;
            }
#endif
          }
        ...
        for (j = 0; j < (int)(_graph.node_of_degree[d].size);) {
          ...
          // std::cout << " examine " << x << " current neighbors = " <<
          // util_set << " " << _graph.neighbor[x] << std::endl;
          for (i = _graph.neighbor[x].size; --i >= 0;) {
            y = _graph.neighbor[x][i];
            util_set.remove(y);
            if (!util_set.included(_graph.matrix[y])) {
              // std::cout << " not a clique b/c of " <<  y << " neighbors: " <<
              // (_graph.matrix[y]) << std::endl;
              break;
            }
            util_set.add(y);
          }
          if (i < 0) {
            // std::cout << " -> it's a clique! remove " << x << " and add " <<
            // _graph.neighbor[x] << std::endl;
            //  N(x) is a clique
  ...
  bool kernelize_crowns() {
    ...
    if (_graph.nmatch > (int)(_upper_bound - _cover.size))
      return false;
  ...
  bool kernelize_NT() {
    ...
    for (int i = 0; i < _graph.size(); i++) {
      for (size_t j = 0; j < _graph.neighbor[_graph.node[i]].size; j++) {
        H->add_undirected(_graph.node[i],
                          _graph.neighbor[_graph.node[i]][j] + _graph.capacity);
      }
    }
    ...
    for (size_t i = 0; i < H->left.size; i++) {
      ...
      if ((H->vcover.contain(v)) && (H->vcover.contain(v + _graph.capacity))) {
        ...
        add_to_cover(v, NO_REASON);
                 ...
                 (!H->vcover.contain(v + _graph.capacity))) {
        save(v, NO_REASON);
    ...
    // check the number of vertices
    if (_graph.size() > 2 * (_upper_bound - (int)(_cover.size)))
      return false;
    ...
    for (d = _graph.min_degree; d <= _graph.max_degree; ++d) {
      for (j = _graph.node_of_degree[d].size; --j >= 0;) {
        ...
        if (best < 0) {
          best = num_cliques;
          ++num_cliques;
          // while(smallest_clique<num_cliques &&
          // candidates[smallest_clique].empty()) ++smallest_clique;
        } else {
          // if(++lb==limit) break;
          ++lb;
        }
    ...
    if (_explanation.size == 0) {
      ...
      if (y != _explanation.capacity) {
        while (_level[y] == _level[x]) {
          ...
          if (_DEBUG_BACKJUMPS) {
#ifdef _STORE_NOGOOD
            std::cout << " resolve " << x << " with its explanation " << reason
                      << std::endl;
#else
            std::cout << " resolve " << x << " with its explanation";
            for (int k = 0; k < rsize; ++k)
      ...
      while (--nbacktracks > 0) {
        ...
        // std::cout << "add " << z << " into " << std::endl << _graph <<
        // std::endl; std::cout << "add " << z << " into G" << std::endl;
      ...
      // std::cout << "last decision = " << z << " / nogood = " << x <<
      // std::endl;
...
#ifdef _DEBUG_COLORING
        std::cout << " => " << _coloring[x] << " smallest in " << color_list
                  << std::endl;
#endif

        color_list.fill();

        if (max_col < _coloring[x])
          max_col = _coloring[x];
      }
    }
    ...
    for (size_t i = 0; i < _graph.node.size; ++i) {
      ...
      for (int j = _graph.neighbor[x].size; j < _graph.capacity - 1; ++j) {
        ...
        if (_graph.node.contain(y) && _coloring[x] == _coloring[y]) {
          ...
          std::cout
              << " " << x << " and " << y
              << " cannot be in the same clique since they aren't neighbors!"
              << std::endl;
          exit(1);
        }
      }
  ...
  inline int undo() {
    ...
    //
    // while(!_cover.empty() && _level[_cover.back()] >= l) {
    // 	std::cout << "pop " << _cover.back() << " " << _level[_cover.back()] <<
    // std::endl;
    // 	//std::cout.flush();
    // 	x = _cover.pop();
    // }
  ...
  //
  // 				lb =
  // cliques_lower_bound(_upper_bound-_cover.size);
  //
  ...
  inline bool suboptimal(const bool dominance) {
    ...
    if (_graph.num_edges) {
      ...
      // 	std::cout << "overhead= " << (_lower_bound-_upper_bound) <<
      // std::endl;
      // }
    }
...
inline bool apply_buss() {
  ...
  if (_graph.max_degree >= (_upper_bound - (int)(_cover.size))) {
    ...
    do {
      ...
      // std::cout << "buss " << _graph.min_degree << " .. " <<
      // _graph.max_degree << std::endl;
...
// std::cout << std::endl;
  ...
  inline void reset() {
    ...
    while ((int)(_decision.size) > init_level) {
      ...
      // print_trail();
      //
      // std::cout << "add " << (x < 0 ? -x-1 : x) << " to" << std::endl <<
      // _graph << std::endl;
      //
      _graph.add_and_update((x < 0 ? -x - 1 : x));
  ...
  bool run(const bool bmax = true, const bool dominance = true,
           const int limit = -1, const int ub_limit = -1,
           const bool randomized = false);

  void solve(const bool bmax = true, const bool use_bkjp = true,
             const bool dominance = true, const bool kernelize = false,
             const bool randomized = false, const int base = -1,
