...
#define FILTER1(var, method)                                                   \
  ...
  if (event_type[(var)] == FAIL_EVENT)                                         \
    wiped = FAILURE(var);                                                      \
  else if (event_type[(var)] != NO_EVENT && !changes.contain(var))             \
    changes.add(var);
...
* For each variable in their scope, a constraint has a pointer to the list they
belong to for that variable [on]
- If the constraint should not be called on any modification of this variable,
the pointer is null
- If the constraint should be called on value events, it points to the value
list of the given variable
- If the constraint should be called on range events, it points to the range
list of the given variable
- If the constraint should be called on domain events, it points to the domain
list of the given variable

* Again for each variable in the scope, it keeps its current index in the given
list [index]
- If the constraint is not currently in the list of var $i, the value of
index[$i] should be -1
- If the constraint is currently posted in the list of var $i, the value of
index[$i] should be its rank in the list
...
This is this pointer which is added to the list of triggers of a variables (via
[on]) This is important when propagating: when scanning the list of triggers for
a given variable, it indicates the rank of the variable in the constraint's
scope, allowing to use the version of propagate() informed about the caller.

* A constraint has flag to indicate whether it achieves at least a level of
propagation equal to NFC1 [enforce_nfc1]
- If it is set to true, then the constraint will be automatically relaxed from
its trigger list in the last active variable when there is only one of them
- If it is set to false, it is never relaxed

* The set of "active" variables is kept in a specific, however non-virtual
attribute "active"
...
When the solver process a value event on a variable, it notifies it to all its
constraints.
...
then: the constraint is relaxed from that variable.
...
That is, when backtracking upon this level, the restore() method of $c will be
called. Some information about what should be undone is passed trough the [data]
field of $c.

1/ First, it contains the index of the variable
2/ Second, it can be complemented with the flags [ACTIVITY], [RELAXED] or
[POSTED].

FixedArityConstraints:

* notify_assignment() changes the value of the set [active], hence it turn on
the flag [ACTIVITY] if activity becomes a singleton, the constraint is relaxed
from the last remaining variable and the flag [RELAXED] is set before saving.

* When relaxing the flag [RELAXED] is set

* When posting the flag [POSTED] is set

* Restore: the constrint is posted/relaxed from the corresponding variable
according to the flag If the flag [ACTIVITY] was set, then a/ For Binary
constraints, activity is set back to {0,1} b/ For Ternary constraints, activity
is reversed using a specific mechanism
...
public:
  ...
  // what to put on the list: a pointer to the constraint and the rank of the
  // variable (so that it knows who triggered it)
  Constraint *self;
...
#ifdef _DEBUG_RELAX
    if (_DEBUG_RELAX) {
      std::cout << "relax [" << id << "] from " << _scope[var] << ": "
                << on[var] << " -> ";
    }
#endif
  ...
  void un_relax_from(const int var) { index[var] = on[var]->post(self[var]); }
...
template <int ARITY> class ValTuple {
public:
...
public:
  ...
  FixedArityConstraint(Variable x, Variable y, Variable z)
      : ConstraintImplementation() {
    for (int i = 0; i < ARITY; ++i)
      support[i] = NULL;
    // std::cout << "there" << std::endl;
  ...
  FixedArityConstraint(std::vector<Variable> &scp)
      : ConstraintImplementation() {
    for (int i = 0; i < ARITY; ++i)
      support[i] = NULL;
    // std::cout << "there" << std::endl;
  ...
  virtual void consolidate() {
    ...
    // std::cout << _scope << std::endl << scope[0] << " " << scope[1] <<
    // std::endl;
  }
      ...
      if (index[i] >= 0) {
        if (active & (1 << i)) {
          if (scope[i].is_ground()) {
            ...
            display(std::cout);
            std::cout << std::endl;
            std::cout << "Warning: " << scope[i] << " = "
                      << scope[i].get_domain() << " is ground and active!!"
                      << std::endl;
            print_active();
            std::cout << " :: ";
  ...
  virtual void print_active() {
    ...
    //       while(x) {
    //
    // if(x==7) std::cout << "{" << scope[0] << "," << scope[1] << "," <<
    // scope[2] << "}" ; else if(x==6) std::cout << "{"  << scope[1] << "," <<
    // scope[2] << "}" ; else if(x==5) std::cout << "{" << scope[0] << "," <<
    // scope[2] << "}" ; else if(x==3) std::cout << "{" << scope[0] << "," <<
    // scope[1] << "}" ; else if(x==4) std::cout << "{" << scope[2] << "}" ;
  ...
  void relax_from(const int var) {
    ...
    if (index[var] >= 0) {
      ...
      if (_DEBUG_RELAX) {
        std::cout << "[" << std::setw(4) << id << "](" << name()
                  << "): f relax from " << scope[var] << std::endl;
      }
#endif
...
public:
  ...
  virtual PropagationOutcome checker_propagate() {
    return BinaryConstraint::propagate();
  }
  virtual PropagationOutcome bound_checker_propagate() {
    return BinaryConstraint::bound_propagate();
  }
  ...
  virtual PropagationOutcome checker_propagate(const int changed_idx,
                                               const Event evt) {
    return BinaryConstraint::propagate(changed_idx, evt);
  }
  virtual PropagationOutcome bound_checker_propagate(const int changed_idx,
                                                     const Event evt) {
    return BinaryConstraint::bound_propagate(changed_idx, evt);
  }
  ...
  inline void notify_assignment(const int var) {
    ...
    if (_DEBUG_RELAX) {
      std::cout << "[" << std::setw(4) << id << "](" << name()
                << "): notify assignment of " << scope[var] << std::endl;
    }
#endif
    ...
    if (active & (1 << var)) {
      ...
      if (enforce_nfc1 && size_byte[active] == 1) {
        ...
        if (index[last] >= 0) {
          ...
          if (_DEBUG_RELAX) {
            std::cout << "[" << std::setw(4) << id << "](" << name()
                      << "): f relax from " << scope[last] << std::endl;
          }
#endif
          c.data |= RELAXED;
          un_post_from(last);
  ...
  void restore(const int rtype) {
    ...
    if (_DEBUG_BACKTRACK) {
      ...
      while (--lvl >= 0)
        std::cout << " ";
      std::cout << "[" << std::setw(4) << id << "](" << name() << "): restore"
                << std::endl;
    }
#endif
    ...
    // #ifdef _DEBUG_RELAX
    //      std::cout << "[" << std::setw(4) << id << "](" << name() << "):
    //      reset active: " ; print_active(); std::cout << std::endl;
    // #endif
...
public:
  ...
  TernaryConstraint(Variable x, Variable y, Variable z)
      : FixedArityConstraint<3>(x, y, z) {
    lvl = 3;
  }
  TernaryConstraint(Vector<Variable> &scp) : FixedArityConstraint<3>(scp) {
    lvl = 3;
  }
  TernaryConstraint(std::vector<Variable> &scp) : FixedArityConstraint<3>(scp) {
    lvl = 3;
  }
  ...
  Whenever a variable is assigned, store the current state into the next slot
  and save.
  ...
  */
  ...
  inline void notify_assignment(const int var) {
    ...
    //   std::cout << "SHOULD NOT NOTIFY TWICE (" << scope[var] << " in " <<
  ...
  void update(const int changed_idx, const Event evt) {
    ...
    if (ASSIGNED(evt) && assign(changed_idx)) {
      ...
      if (_DEBUG_RELAX) {
        std::cout << "[" << std::setw(4) << id << "](" << name()
                  << "): ff notify assignment of " << scope[changed_idx]
                  << std::endl;
      }
#endif
...
//   evt) { return FAILURE(0); } virtual PropagationOutcome bound_propagate() {
//   return FAILURE(0); } virtual PropagationOutcome bound_propagate(const int
//   changed_idx, const Event evt) { return FAILURE(0); } virtual
//   PropagationOutcome checker_propagate(const int changed_idx, const Event
//   evt)  { return FAILURE(0); } virtual PropagationOutcome
//   bound_checker_propagate(const int changed_idx, const Event evt) { return
//   FAILURE(0); }
...
public:
  ...
  virtual int get_type() {
    return (PUSHED * pushed()) | (POSTPONED * postponed()) |
  ...
  /// An idempotent constraint should not be called on events triggered by
  /// itself. To forbid that, the lists 'events' and 'changes' are merged during
  /// its propagation, events will be added to the events list of the constraint
  ...
  inline void set_idempotent() {
    if (idempotent()) {
      ...
      events.initialise(0, changes.index_capacity - 1, changes.index_capacity,
                        false);
    }
  }

  virtual void print_active() { std::cout << active << std::endl; }
  ...
  inline void un_relax() {
...
#ifdef _DEBUG_RELAX
    if (_DEBUG_RELAX) {
      std::cout << "[" << std::setw(4) << id << "](" << name() << "): UNRELAX "
                << std::endl;
    }
#endif
#ifdef _DEBUG_RELAX
    if (_DEBUG_RELAX) {
      ...
      // on[14] << std::endl; std::cout << on[14] << std::endl;
    }
#endif
  ...
  inline void un_post() {
...
#ifdef _DEBUG_RELAX
    if (_DEBUG_RELAX) {
      std::cout << "[" << std::setw(4) << id << "](" << name() << "): UNPOST "
                << std::endl;
    }
#endif
#ifdef _DEBUG_RELAX
    ...
    for (int i = active.size; --i >= 0;) {
      ...
      if (index[var] >= 0) {
        ...
        if (_DEBUG_RELAX) {
          std::cout << "[" << std::setw(4) << id << "](" << name()
                    << "): g1 relax from " << scope[var] << std::endl;
        }
#endif
...
#ifdef _DEBUG_RELAX
    if (_DEBUG_RELAX) {
      std::cout << "[" << std::setw(4) << id << "](" << name() << "): RELAX "
                << std::endl;
    }
#endif
#ifdef _DEBUG_RELAX
  ...
  virtual PropagationOutcome checker_propagate() {
    return GlobalConstraint::propagate();
  }
  virtual PropagationOutcome bound_checker_propagate() {
    return GlobalConstraint::bound_propagate();
  }
  ...
  inline void notify_assignment(const int var) {
    ...
    if (_DEBUG_RELAX) {
      std::cout << "[" << std::setw(4) << id << "](" << name()
                << "): notify assignment of " << scope[var]
                << " => "; //<< std::endl;
    }
#endif
#ifdef _DEBUG_RELAX
  ...
  inline void notify_other_event(const int var, const Mistral::Event evt) {

    // if(ASSIGNED(evt) && active.contain(var)) {

    // 	// std::cout << active << " -> " << scope[var] << " in " <<
  ...
  inline void notify_first_event(const int var, const Mistral::Event evt) {
  ...
  // returns true iff the constraint explains the conflict. That is,
  // weight_conflict is overwritten
...
public:
  ...
  ConstraintEqual(Variable x, Variable y) : BinaryConstraint(x, y) {}
  ConstraintEqual(Vector<Variable> &scp) : BinaryConstraint(scp[0], scp[1]) {}
  ConstraintEqual(std::vector<Variable> &scp)
      : BinaryConstraint(scp[0], scp[1]) {}
  virtual Constraint clone() {
    return Constraint(new ConstraintEqual(scope[0], scope[1]) // , type
    );
  }
...
public:
  ...
  ConstraintNotEqual(Variable x, Variable y) : BinaryConstraint(x, y) {}
  ...
  ConstraintNotEqual(std::vector<Variable> &scp)
      : BinaryConstraint(scp[0], scp[1]) {}
  virtual Constraint clone() {
    return Constraint(new ConstraintNotEqual(scope[0], scope[1]) // , type
    );
  }
  virtual void initialise();
  virtual void mark_domain();
  ...
  virtual Constraint get_negation(const int var, Variable x) {
    return Constraint(
        new ConstraintEqual((var ? scope[0] : x), (var ? x : scope[1])));
  }
  virtual bool absorb_negation(const int var) {
    return (scope[0].get_min() == 0 && scope[1].get_min() == 0 &&
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(
        new PredicateEqual(scope[0], scope[1], scope[2], spin) // , type
    );
  }
  virtual void initialise();
  virtual bool rewritable() { return true; }
...
public:
  ...
  PredicateConstantEqual(Variable x, Variable y, const int val = 0,
                         const int sp = 1)
      : BinaryConstraint(x, y) {
    value = val;
    spin = sp;
  }
  PredicateConstantEqual(Vector<Variable> &scp, const int val = 0,
                         const int sp = 1)
      : BinaryConstraint(scp) {
    value = val;
    spin = sp;
  }
  PredicateConstantEqual(std::vector<Variable> &scp, const int val = 0,
                         const int sp = 1)
      : BinaryConstraint(scp) {
    value = val;
    spin = sp;
  ...
  virtual Constraint clone() {
    return Constraint(
        new PredicateConstantEqual(scope[0], scope[1], value, spin) // , type
    );
  }
  virtual Constraint get_negation(const int var, Variable x) {
...
public:
  ...
  PredicateNot(Variable x, Variable y) : BinaryConstraint(x, y) {}
  PredicateNot(Vector<Variable> &scp) : BinaryConstraint(scp) {}
  PredicateNot(std::vector<Variable> &scp) : BinaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateNot(scope[0], scope[1]));
  }
  virtual Constraint get_negation(const int var, Variable x) {
    return Constraint(
        new ConstraintEqual((var ? scope[0] : x), (var ? x : scope[1])));
  ...
  /**@name Solving*/
  //@{
  virtual int check(const int *sol) const { return ((sol[0] > 0) == sol[1]); }
  virtual PropagationOutcome propagate();
  virtual PropagationOutcome propagate(const int changed_idx, const Event evt);
  virtual bool rewritable() { return true; }
  virtual RewritingOutcome rewrite();
...
public:
  ...
  PredicateNeg(Variable x, Variable y) : BinaryConstraint(x, y) {}
  PredicateNeg(Vector<Variable> &scp) : BinaryConstraint(scp) {}
  PredicateNeg(std::vector<Variable> &scp) : BinaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateNeg(scope[0], scope[1]));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 1; }
...
public:
  ...
  PredicateIntervalMember(Variable x, Variable y, const int lb = -INFTY,
                          const int ub = +INFTY, const int sp = 1)
      : BinaryConstraint(x, y) {
    spin = sp;
    lower_bound = lb;
    upper_bound = ub;
  }
  PredicateIntervalMember(Vector<Variable> &scp, const int lb = -INFTY,
                          const int ub = +INFTY, const int sp = 1)
      : BinaryConstraint(scp) {
    spin = sp;
    lower_bound = lb;
  ...
  PredicateIntervalMember(std::vector<Variable> &scp, const int lb,
                          const int ub, const int sp = 1)
      : BinaryConstraint(scp) {
    spin = sp;
    lower_bound = lb;
    upper_bound = ub;
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateIntervalMember(
        scope[0], scope[1], lower_bound, upper_bound, spin));
  }
  virtual Constraint get_negation(const int var, Variable x) {
    return Constraint(new PredicateIntervalMember(scope[0], x, lower_bound,
                                                  upper_bound, !spin));
  }
  virtual void initialise();
  virtual void mark_domain();
  ...
  //@{
  virtual int check(const int *sol) const {
    return (((sol[0] >= lower_bound) && (sol[0] <= upper_bound)) ==
            (sol[1] ^ spin));
  }
  virtual PropagationOutcome propagate();
  virtual PropagationOutcome propagate(const int changed_idx, const Event evt);
...
public:
  ...
  PredicateSetMember(Variable x, Variable y, const BitSet &vals,
                     const int sp = 1)
      : BinaryConstraint(x, y) {
    spin = sp;
    values = vals;
  }
  PredicateSetMember(Vector<Variable> &scp, const BitSet &vals,
                     const int sp = 1)
      : BinaryConstraint(scp) {
    spin = sp;
    values = vals;
  }
  PredicateSetMember(std::vector<Variable> &scp, const BitSet &vals,
                     const int sp = 1)
      : BinaryConstraint(scp) {
    spin = sp;
    values = vals;
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateSetMember(scope[0], scope[1], values, spin));
  }
  virtual Constraint get_negation(const int var, Variable x) {
  ...
  //@{
  virtual int check(const int *sol) const {
    return ((values.contain(sol[0]) == (sol[1] ^ spin)));
  }
  virtual PropagationOutcome propagate(const int changed_idx, const Event evt);
  virtual PropagationOutcome propagate();
  // virtual RewritingOutcome rewrite();
...
public:
  ...
  ConstraintLess(const int ofs = 0) : BinaryConstraint() { offset = ofs; }
  ...
  virtual Constraint clone() {
    return Constraint(new ConstraintLess(scope[0], scope[1], offset) // , type
    );
  }
  virtual void initialise();
  virtual void mark_domain();
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(
        new PredicateLess(scope[0], scope[1], scope[2], offset) // , type
    );
  }
  virtual Constraint get_negation(const int var, Variable x) {
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(
        new PredicateUpperBound(scope[0], scope[1], bound) // , type
    );
  }
  virtual Constraint get_negation(const int var, Variable x);
  // {
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(
        new PredicateLowerBound(scope[0], scope[1], bound) // , type
    );
  }
  virtual Constraint get_negation(const int var, Variable x) {
...
public:
  ...
  PredicateAnd(Vector<Variable> &scp) : TernaryConstraint(scp) {}
  PredicateAnd(Variable x, Variable y, Variable z)
      : TernaryConstraint(x, y, z) {}
  PredicateAnd(std::vector<Variable> &scp) : TernaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateAnd(scope[0], scope[1], scope[2]));
  }
  // virtual Constraint get_negation(const int var, Variable x) {
  //   return Constraint( new PredicateUpperBound( scope[0], x, bound+1 ) );
  // }
...
public:
  ...
  ConstraintAnd(Vector<Variable> &scp) : BinaryConstraint(scp) {}
  ConstraintAnd(Variable x, Variable y) : BinaryConstraint(x, y) {}
  ConstraintAnd(std::vector<Variable> &scp) : BinaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new ConstraintAnd(scope[0], scope[1]));
  }
  virtual void initialise();
  // virtual void mark_domain();
  virtual int idempotent() { return 1; }
...
public:
  ...
  ConstraintNotAnd(Vector<Variable> &scp) : BinaryConstraint(scp) {}
  ConstraintNotAnd(Variable x, Variable y) : BinaryConstraint(x, y) {}
  ConstraintNotAnd(std::vector<Variable> &scp) : BinaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new ConstraintNotAnd(scope[0], scope[1]));
  }
  virtual void initialise();
  // virtual void mark_domain();
  virtual int idempotent() { return 1; }
...
public:
  ...
  PredicateOr(Vector<Variable> &scp) : TernaryConstraint(scp) {}
  PredicateOr(Variable x, Variable y, Variable z)
      : TernaryConstraint(x, y, z) {}
  PredicateOr(std::vector<Variable> &scp) : TernaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateOr(scope[0], scope[1], scope[2]));
  }
  virtual Constraint get_negation(const int var, Variable x) {
    return Constraint((
        var < 2 ? new PredicateLess(x, (var ? scope[0] : scope[1]), scope[2], 0)
                : NULL
...
public:
  /**@name Constructors*/
  //@{
  ConstraintOr(Vector<Variable> &scp) : BinaryConstraint(scp) {}
  ConstraintOr(Variable x, Variable y) : BinaryConstraint(x, y) {}
  ConstraintOr(std::vector<Variable> &scp) : BinaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new ConstraintOr(scope[0], scope[1]));
  }
  virtual Constraint get_negation(const int var, Variable x) {
...
public:
  ...
  ConstraintLex(Vector<Variable> &scp) : GlobalConstraint(scp) { priority = 1; }
  ConstraintLex(std::vector<Variable> &scp) : GlobalConstraint(scp) {
    priority = 1;
  }
  ...
  virtual int check(const int *sol) const {
    return (((!sol[2] && !sol[3]) > (sol[0] == sol[1]) ||
             (sol[2] < sol[3]) > (sol[0] < sol[1]) || sol[2] > sol[3]));
  }
  virtual PropagationOutcome propagate();
  // virtual RewritingOutcome rewrite();
  //@}
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateOffset(scope[0], scope[1], offset));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 1; }
...
public:
  ...
  PredicateAdd(Variable x, Variable y, Variable z)
      : TernaryConstraint(x, y, z) {}
  PredicateAdd(Vector<Variable> &scp) : TernaryConstraint(scp) {}
  PredicateAdd(std::vector<Variable> &scp) : TernaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateAdd(scope[0], scope[1], scope[2]) // , type
    );
  }
  virtual void initialise();
  virtual void mark_domain();
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateFactor(scope[0], scope[1], factor));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 1; }
...
public:
  ...
  PredicateSquare(Vector<Variable> &scp) : BinaryConstraint(scp) {}
  PredicateSquare(Variable x, Variable y) : BinaryConstraint(x, y) {}
  PredicateSquare(std::vector<Variable> &scp) : BinaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateSquare(scope[0], scope[1]));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 1; }
...
public:
  ...
  PredicateAbs(Vector<Variable> &scp) : BinaryConstraint(scp) {}
  PredicateAbs(Variable x, Variable y) : BinaryConstraint(x, y) {}
  PredicateAbs(std::vector<Variable> &scp) : BinaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateAbs(scope[0], scope[1]));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 1; }
  ...
  virtual int check(const int *sol) const { return (abs(sol[0]) != sol[1]); }
  virtual PropagationOutcome propagate(const int changed_idx, const Event evt);
  virtual PropagationOutcome propagate();
  // virtual RewritingOutcome rewrite();
  //@}
...
public:
  ...
  PredicateMod(Variable x, Variable y, Variable z)
      : TernaryConstraint(x, y, z) {}
  PredicateMod(Vector<Variable> &scp) : TernaryConstraint(scp) {}
  PredicateMod(std::vector<Variable> &scp) : TernaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateMod(scope[0], scope[1], scope[2]));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 0; }
  ...
  virtual int check(const int *sol) const {
    return (!sol[1] || sol[2] != __modulo_fct__(sol[0], sol[1]));
  }
  virtual PropagationOutcome propagate(const int changed_idx, const Event evt);
  virtual PropagationOutcome propagate();
  virtual RewritingOutcome rewrite();
...
public:
  ...
  PredicateCMod(Variable x, Variable y, Variable z)
      : TernaryConstraint(x, y, z) {}
  PredicateCMod(Vector<Variable> &scp) : TernaryConstraint(scp) {}
  PredicateCMod(std::vector<Variable> &scp) : TernaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateCMod(scope[0], scope[1], scope[2]));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 0; }
  ...
  PropagationOutcome filter();
  virtual int check(const int *sol) const {
    return (!sol[1] || sol[2] != sol[0] % sol[1]);
  }
  virtual PropagationOutcome propagate(const int changed_idx, const Event evt);
  virtual PropagationOutcome propagate();
  virtual RewritingOutcome rewrite();
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateModConstant(scope[0], scope[1], modulo));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 0; }
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateCModConstant(scope[0], scope[1], modulo));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 0; }
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateDivConstant(scope[0], scope[1], quotient));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 0; }
...
public:
  ...
  PredicateDiv(Variable x, Variable y, Variable z)
      : TernaryConstraint(x, y, z) {}
  PredicateDiv(Vector<Variable> &scp) : TernaryConstraint(scp) {}
  PredicateDiv(std::vector<Variable> &scp) : TernaryConstraint(scp) {}
  virtual Constraint clone() {
    return Constraint(new PredicateDiv(scope[0], scope[1], scope[2]));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 0; }
...
public:
  ...
  PredicateMul(Vector<Variable> &scp) : GlobalConstraint(scp) { priority = 1; }
  // PredicateMul(Variable x, Variable y, Variable z)
  //   : GlobalConstraint(x,y,z) {}
  PredicateMul(std::vector<Variable> &scp) : GlobalConstraint(scp) {
    priority = 1;
  }
  // virtual Constraint clone() { return Constraint(new PredicateMul(scope[0],
  // scope[1], scope[2])); }
  ...
  //
  PropagationOutcome revise_multiplication(const int X, const int Y,
                                           const int Z);
  //
  PropagationOutcome prune(const int lb_neg, const int ub_neg, const int lb_pos,
                           const int ub_pos, const bool pzero, const int Z);
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new ConstraintDisjunctive(
        scope[0], scope[1], processing_time[0], processing_time[1]));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual int idempotent() { return 1; }
...
public:
  ...
  ConstraintReifiedDisjunctive(Variable x, Variable y, Variable z, const int p0,
                               const int p1);
  ConstraintReifiedDisjunctive(Vector<Variable> &scp, const int p0,
                               const int p1);
  ConstraintReifiedDisjunctive(std::vector<Variable> &scp, const int p0,
                               const int p1);

  virtual Constraint clone() {
    return Constraint(new ConstraintReifiedDisjunctive(
        scope[0], scope[1], scope[2], processing_time[0], processing_time[1]));
  }
  virtual Constraint get_negation(const int var, Variable x) {
    return Constraint(new ConstraintReifiedDisjunctive(
        scope[0], scope[1], x, processing_time[0], processing_time[1]));
  }
  virtual void initialise();
  virtual void mark_domain();
...
//     /**@name Parameters*/
//     //@{
//     // // given the rank $i$ of a tuple in $table$, and a variable $j$, the
//     id of this tuple in the set $support_of[j]$
//     // tuple<ARITY> *table2support;

//     // // given a variable $j$ and a value $k$ in the set $support_of[j]$,
//     the id of the corresponding tuple in $table$
//     // int **support2table;

//     // for each variable and each value, the set of support. In fact, the
//     rank of the support in $tabl$ is stored in the set.
//     // since support are partitioned between values, all set for a given
//     variable share the same $index_$ ReversibleSet *support_of[ARITY];
...
//     virtual Constraint clone() { return Constraint(new
//     PredicateTuple(scope)); } virtual void initialise(); virtual int
//     idempotent() { return 1;} virtual int postponed() { return 1;} virtual
//     int pushed() { return 1;}
//     //virtual bool absorb_negation(const int var) { return true; }
...
//   for(int i=0; i<ARITY; ++i) {
//     support_of[i] = new ReversibleSet[scope[i].get_initial_max() -
//     scope[i].get_initial_min() + 1]; support_of[i] -=
//     scope[i].get_initial_min(); values[i].initialise(0,scope[i].get_size());
...
/**@name Parameters*/
//@{
// for each variable and each value, the set of support. In fact, the rank of
// the support in $tabl$ is stored in the set. since support are partitioned
// between values, all set for a given variable share the same $index_$
#ifdef _OS_PRUNING
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new ConstraintGAC2001(scope));
  }
  virtual void initialise();
  // virtual int idempotent() { return 1;}
  // virtual int postponed() { return 1;}
...
//   /**@name Parameters*/
//   //@{
//   // last "gap" from the tightest bound to the total, used to know what cause
//   pruning/failure
//   //int gap;
//   int total;
//   int lower_bound;
...
//   virtual Constraint clone() { return Constraint(new
//   ConstraintBoolSumEqual(scope, total)); } virtual void initialise(); virtual
//   void mark_domain(); virtual int idempotent() { return 1;} virtual int
//   postponed() { return 1;} virtual int pushed() { return 1;}
//   //virtual bool absorb_negation(const int var) { return true; }
...
//   end); virtual void initialise_activity(double *lact, double *vact, double
//   norm);
...
public:
  ...
  ConstraintBoolSumInterval(std::vector<Variable> &scp, const int l,
                            const int u);
  virtual Constraint clone() {
    return Constraint(
...
public:
  ...
  ConstraintWeightedBoolSumInterval(Vector<Variable> &scp, const int L = 0,
                                    const int U = 0);
  ConstraintWeightedBoolSumInterval(Vector<Variable> &scp, Vector<int> &coefs,
                                    const int L = 0, const int U = 0);
  ConstraintWeightedBoolSumInterval(std::vector<Variable> &scp,
                                    std::vector<int> &coefs, const int L = 0,
                                    const int U = 0);
  // ConstraintWeightedBoolSumInterval(Vector< Variable >& scp, const int l,
  // const int u); ConstraintWeightedBoolSumInterval(std::vector< Variable >&
  // scp, const int l, const int u);
  virtual Constraint clone() {
    return Constraint(
        new ConstraintWeightedBoolSumInterval(scope, lower_bound, upper_bound));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual bool explained() { return true; }
...
public:
  ...
  // ReversibleNum<int> max_;
  //  points to the unassigned variable with maximum coefficient (in absolute
  //  value)
  ...
  /**@name Constructors*/
  //@{
  ConstraintIncrementalWeightedBoolSumInterval() : GlobalConstraint() {
    priority = 1;
  }
  ...
  ConstraintIncrementalWeightedBoolSumInterval(std::vector<Variable> &scp,
                                               ...
                                               const int L = 0,
                                               const int U = 0);
  // ConstraintIncrementalWeightedBoolSumInterval(Vector< Variable >& scp, const
  // int l, const int u);
  // ConstraintIncrementalWeightedBoolSumInterval(std::vector< Variable >& scp,
  // const int l, const int u);
  virtual Constraint clone() {
    return Constraint(new ConstraintIncrementalWeightedBoolSumInterval(
...
public:
  ...
  PredicateWeightedBoolSum(Vector<Variable> &scp, Vector<int> &coefs,
                           const int o = 0);
  PredicateWeightedBoolSum(std::vector<Variable> &scp, std::vector<int> &coefs,
                           const int o = 0);
  // PredicateWeightedBoolSum(Vector< Variable >& scp, const int l, const int
  // u); PredicateWeightedBoolSum(std::vector< Variable >& scp, const int l,
  // const int u);
  virtual Constraint clone() {
    return Constraint(new PredicateWeightedBoolSum(scope, weight, offset));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual bool explained() { return true; }
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateBoolSum(scope, offset));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual bool explained() { return true; }
...
public:
  ...
  /**@name Constructors*/
  //@{
  PredicateWeightedSum(Vector<Variable> &scp, const int L = 0, const int U = 0);
  PredicateWeightedSum(Vector<Variable> &scp, Vector<int> &coefs,
                       const int L = 0, const int U = 0);
  PredicateWeightedSum(std::vector<Variable> &scp, std::vector<int> &coefs,
                       const int L = 0, const int U = 0);
  virtual ~PredicateWeightedSum();
  virtual Constraint clone() {
    return Constraint(new PredicateWeightedSum(scope, weight));
  }
  virtual bool rewritable() { return true; }
  virtual int idempotent() { return 1; }
  virtual int postponed() { return 1; }
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new ConstraintParity(scope, target_parity));
  }
  // virtual bool rewritable() { return true; }
  virtual bool explained() { return true; }
  virtual int idempotent() { return 1; }
...
public:
  ...
  // used to store the explanation when "get_reason_for()" is called
  Vector<Literal> explanation;
  // we need this for the explanation to check if the maximum cardinality of all
  // subsequences at position i is equal to p.
  ...
  ConstraintMultiAtMostSeqCard(Vector<Variable> &scp, const int k, const int d,
                               const int *p, const int *q);
  ConstraintMultiAtMostSeqCard(std::vector<Variable> &scp, const int k,
                               const int d, const int *p, const int *q);
  void initialise_struct(const int k = 0, const int d = 0, const int *p = NULL,
                         const int *q = NULL);
  virtual Constraint clone() {
    return Constraint(new ConstraintMultiAtMostSeqCard(scope, _k, _d, _p, _q));
  }
  virtual void initialise();
  virtual void mark_domain();
  virtual bool explained() { return true; }
  ...
  virtual iterator get_reason_for(const Atom a, const int lvl, iterator &end);
  void greedy_assign_for_explanation(Vector<Variable> &X, int __size,
                                     int __rank);
...
/**********************************************
 * AtMostSeqCard Constraint
 * The same constraint as ConstraintMultiAtMostSeqCard with the exeption of
 *generating default nogoods without reductions
 **********************************************/
//
///
...
public:
  ...
  ConstraintNaiveMultiAtMostSeqCard(Vector<Variable> &scp, const int k,
                                    const int d, const int *p, const int *q);
  // ConstraintNaiveMultiAtMostSeqCard(std::vector< Variable >& scp, const int
  // k, const int d, const int* p, const int* q);
  virtual iterator get_reason_for(const Atom a, const int lvl, iterator &end);
};

//Simplified Explanation
class ConstraintSimplifiedExplanationMultiAtMostSeqCard
    : public ConstraintMultiAtMostSeqCard {

public:
  /**@name Constructors*/
  //@{
  // ConstraintNaiveMultiAtMostSeqCard();
  ConstraintSimplifiedExplanationMultiAtMostSeqCard(Vector<Variable> &scp,
...
class ConstraintLeftExplanationMultiAtMostSeqCard
    : public ConstraintMultiAtMostSeqCard {
...
public:
  ...
  //@{
  // ConstraintNaiveMultiAtMostSeqCard();
  ConstraintLeftExplanationMultiAtMostSeqCard(Vector<Variable> &scp,
                                              const int k, const int d,
                                              const int *p, const int *q);
  // ConstraintNaiveMultiAtMostSeqCard(std::vector< Variable >& scp, const int
  // k, const int d, const int* p, const int* q);
  virtual iterator get_reason_for(const Atom a, const int lvl, iterator &end);
  void simple_greedy_assign_for_explanation(Vector<Variable> &X, int __rank,
                                            int index_a);
};
...
public:
  ...
  //@{
  ConstraintStretch();
  ConstraintStretch(std::vector<Variable> &scp, std::vector<int> &stype,
                    std::vector<int> &slb, std::vector<int> &sub,
                    std::vector<int> &t);
  ConstraintStretch(Vector<Variable> &scp, std::vector<int> &stype,
                    std::vector<int> &slb, std::vector<int> &sub,
                    std::vector<int> &t);
  virtual Constraint clone() {
    return Constraint(
        new ConstraintStretch(scope, stype, slb, sub, transition_list));
  }
  void init_struct(std::vector<int> &type, std::vector<int> &lb,
                   std::vector<int> &ub, std::vector<int> &T);
  virtual void initialise();
  virtual void mark_domain();
  virtual bool explained() { return false; }
  virtual int idempotent() { return 1; }
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateVertexCover(scope, _G));
  }
  virtual int idempotent() { return 1; }
  virtual int postponed() { return 1; }
  virtual int pushed() { return 1; }
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateFootrule(scope));
  }
  virtual int idempotent() { return 1; }
  virtual int postponed() { return 1; }
  virtual int pushed() { return 1; }
...
public:
  ...
  /**@name Solving*/
  //@{
  void react_to(PropagationOutcome &wiped, const int changed_idx,
                const Event evt);
  virtual int check(const int *sol) const;
  virtual PropagationOutcome propagate();
  //@}
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateElement(scope, offset));
  }
  virtual int idempotent() { return 1; }
  virtual int postponed() { return 1; }
  virtual int pushed() { return 1; }
...
public:
  ...
  virtual Constraint clone() {
    return Constraint(new PredicateBoolElement(scope, offset));
  }
  virtual int idempotent() { return 1; }
  virtual int postponed() { return 1; }
  virtual int pushed() { return 1; }
...
public:
  ...
  ConstraintCliqueNotEqual(std::vector<Variable> &scp,
                           const int except = INT_MIN);
  ConstraintCliqueNotEqual(Variable *scp, const int n);
  virtual Constraint clone() {
    return Constraint(new ConstraintCliqueNotEqual(scope) // , type
    );
  }
  virtual void initialise();
  virtual void mark_domain();
  ...
  virtual PropagationOutcome propagate();
  // virtual PropagationOutcome propagate(const int changed_idx, const Event
  // evt); virtual RewritingOutcome rewrite();
  //@}
...
private:
  ...
  int expl_note;
  // used to store
  //  -- which interval order was used to find the Hall interval (least
  //  significant bit)
  //  -- at which rank was is discovered (rest)
  // When computing an explanation, suppose that it was when exploring the
  // maxsorted intervals an at rank r
  //  -> then maxsorted[r]->max is the max of the Hall interval AND it is a HI
  //  of the subset of variables whose rank is <= r
  //     we then use the following array:
  // Vector<int> count_bound;
  std::vector<int> other_bounds;
  // let m = maxsorted[r]->max
  // for each pos s in the interval [0, m] we store how many times an interval
  // finishing before or at m starts at s then, going from m toward 0, we
  // compute how many of these intervals start at a pos s or before (call this
  // b[s]) IF s+b[s]>m+1 THEN [s, m] is a Hall interval if the Hall interval was
  // found on minsorted intervals, then we do the same thing in reverse order
...
public:
  ...
  virtual void mark_domain();
  virtual Constraint clone() {
    return Constraint(new ConstraintAllDiff(scope) // , type
    );
  }
  virtual void initialise();
...
private:
  ...
  int *potentialStableSets; // links elements that potentialy belong to same
                            // stable set
  int *newMin;
  AD_Interval *iv;
  AD_Interval **minsorted;
  ...
  partialSum *l;
  partialSum *u;
  partialSum *initializePartialSum(const int firstValue, int count,
                                   const int *elements);
  ...
  int filterLowerMax();
  int filterUpperMax();
  int filterLowerMin(int *tl, int *c, int *stableAndUnstableSets,
                     int *stableInterval, int *potentialStableSets,
                     int *newMin);
  int filterUpperMin(int *tl, int *c, int *stableAndUnstableSets,
...
public:
  ...
  ConstraintOccurrences(Vector<Variable> &scp, const int firstDomainValue,
                        const int lastDomainValue, const int *minOccurrences,
                        const int *maxOccurrences);
  ConstraintOccurrences(std::vector<Variable> &scp, const int firstDomainValue,
                        const int lastDomainValue, const int *minOccurrences,
                        const int *maxOccurrences);
  ...
  virtual Constraint clone() {
    return Constraint(
        new ConstraintOccurrences(scope, l->firstValue + 3, l->lastValue - 2,
                                  l->sum + 3, u->sum + 3) // , type
    );
  }
  virtual void initialise();
 ...
 ***********************************************/
/*! \class ConstraintPreemptiveNoOverlap
\brief  NoOverlap Constraint
*/
class Instance;
// class ConstraintPreemptiveNoOverlap : public GlobalConstraint {
...
//   /**@name Constructors*/
//   //@{
//   ConstraintPreemptiveNoOverlap() : GlobalConstraint() { priority = 2; }
//   ConstraintPreemptiveNoOverlap(Vector<Variable> &scp,
//                                 const std::vector<int> &duration);
//   virtual Constraint clone() {
//     return Constraint(
//         new ConstraintPreemptiveNoOverlap(scope, duration) // , type
//     );
//   }
...
//   virtual int postponed() { return 1; }
//   virtual int pushed() { return 1; }
//   virtual ~ConstraintPreemptiveNoOverlap();
//   //@}
...
class ConstraintPreemptiveNoOverlapEdge : public GlobalConstraint {
...
public:
  ...
  /**@name Constructors*/
  //@{
  ConstraintPreemptiveNoOverlapEdge() : GlobalConstraint() { priority = 2; }
  ConstraintPreemptiveNoOverlapEdge(Vector<Variable> &scp,
                                    const std::vector<int> &duration);
  virtual Constraint clone() {
    return Constraint(
        new ConstraintPreemptiveNoOverlapEdge(scope, duration) // , type
    );
  }
  size_t numVars() { return duration.size(); }
  virtual void initialise();
  ...
  virtual int postponed() { return 1; }
  virtual int pushed() { return 1; }
  virtual ~ConstraintPreemptiveNoOverlapEdge();
  //@}
...
public:
  ...
  /**@name Constructors*/
  //@{
  ConstraintPreemptiveNoOverlapHall() : GlobalConstraint() { priority = 2; }
  ConstraintPreemptiveNoOverlapHall(Vector<Variable> &scp,
                                    const std::vector<int> &duration);
  virtual Constraint clone() {
    return Constraint(new ConstraintPreemptiveNoOverlapHall(scope, duration) //
                      ,
                      type);
  }
  size_t numVars() { return duration.size(); }
  ...
  virtual int postponed() { return 1; }
  virtual int pushed() { return 1; }
  virtual ~ConstraintPreemptiveNoOverlapHall();
  //@}
